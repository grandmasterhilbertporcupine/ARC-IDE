...
export function makeThreadListEntry(
  overrides: MakeThreadArgs &
    Partial<Pick<ThreadListEntry, "lastRequestedModel">>,
): ThreadListEntry {
  return {
    ...makeThread(overrides),
    lastRequestedModel: overrides.lastRequestedModel ?? null,
    activity: {
    ...
    runtime: {
      displayStatus: overrides.status ?? "idle",
