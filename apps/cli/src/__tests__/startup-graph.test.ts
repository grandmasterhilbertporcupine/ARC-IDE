...
describe("bb startup module graph", () => {
  ...
  describe("split dist/index.js", () => {
    ...
    it.each([
      { mode: "implicit", exitCode: 0 },
      { mode: "implicit", exitCode: 7 },
      { mode: "explicit", exitCode: 0 },
      { mode: "explicit", exitCode: 7 },
    ])(
      "drains compressed plugin output and preserves exit $exitCode in $mode routing",
      async ({ mode, exitCode }) => {
        const stdout = `${"ARC Δ agent output\n".repeat(20_000)}complete\n`;
        const stderr = exitCode === 0 ? "" : "Fixture check failed\n";
        ...
        const server = createServer(async (request, response) => {
          ...
          response.end(gzipSync(JSON.stringify({ exitCode, stdout, stderr })));
        });
        await new Promise<void>((resolvePromise) =>
          server.listen(0, "127.0.0.1", resolvePromise),
        );
        ...
        const args =
          mode === "implicit"
            ? ["fixture", "inspect", "fixture-1"]
            : ["plugin", "run", "fixture-plugin", "inspect", "fixture-1"];
        try {
          const run = execFileAsync(process.execPath, [distEntry, ...args], {
            cwd: cliRoot,
            env,
            windowsHide: true,
            timeout: 15_000,
          ...
          expect(invocations).toEqual([["inspect", "fixture-1"]]);
          expect(requests).toEqual([
            ...(mode === "implicit" ? ["/api/v1/plugins/contributions"] : []),
            "/api/v1/plugins/fixture-plugin/cli",
          ]);
        } finally {
