...
describe("bb thread open command output", () => {
  ...
  it("requires an explicit thread id outside an ARC thread", async () => {
    stubThreadOpenApi({});
  ...
  it("documents the current-thread or explicit-thread command shape", async () => {
    ...
    expect(help).toContain(
      "Open an ARC thread, optionally with a file in its panel",
    );
  ...
  it("treats an explicit --split target as a thread id inside an ARC thread", async () => {
    vi.stubEnv("BB_THREAD_ID", "thread-current");
    const { openThread } = stubThreadOpenApi({});
  ...
  it("opens a file for an explicit split target inside an ARC thread", async () => {
    vi.stubEnv("BB_THREAD_ID", "thread-current");
    const { openThread } = stubThreadOpenApi({});
