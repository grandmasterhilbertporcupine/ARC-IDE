...
describe("bb thread action command output", () => {
  ...
  it("preserves the worker sender context when retrying saved addressed work", async () => {
    ...
    const retryPost = vi.fn(async () => ({
      ...
      experimental_addressed: {
        runId: "retained-run",
        status: "started",
        summary: "Retained request",
      },
    }));
    stubServerApi({ "v1.threads.:id.retry-addressed.$post": retryPost });
    await runCommand(
      [
        "thread",
        "retry-addressed",
