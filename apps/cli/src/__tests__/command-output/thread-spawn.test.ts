...
describe("bb thread spawn command output", () => {
  ...
  it("preserves the worker sender context for addressed spawn without parenting the new thread", async () => {
    vi.stubEnv("BB_THREAD_ID", "owned-worker");
    const directory = await mkdtemp(join(tmpdir(), "arc-addressed-cli-"));
    const path = join(directory, "recipients.json");
    const addressing = {
      ...
      recipients: [
        {
          ...
          entityId: "team-a",
          versionId: 1,
          scopeKey: "project:proj-1",
          label: "Team",
        },
      ],
    };
    await writeFile(path, JSON.stringify(addressing));
    try {
      ...
      expect(post).toHaveBeenCalledWith({
        json: expect.objectContaining({
          senderThreadId: "owned-worker",
          experimental_addressing: addressing,
        }),
      });
      expect(post).not.toHaveBeenCalledWith({
        json: expect.objectContaining({ parentThreadId: expect.anything() }),
      });
    } finally {
  ...
  it("forwards a paired execution context and plugin owner in the initial request", async () => {
    const post = vi.fn(async () =>
      fixtures.makeThread({
        id: "bound-thread",
        projectId: "proj-1",
        providerId: "codex",
        originPluginId: "arc",
        experimental_executionContextId: "snapshot-1",
      }),
    );
    stubServerApi({ "v1.threads.$post": post });
    await runCommand(
