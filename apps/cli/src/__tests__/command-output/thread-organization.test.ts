...
describe("bb thread organization commands", () => {
  ...
  it("forwards explicit project-scoped content search and preserves requested model JSON", async () => {
    const body = {
      active: {
        total: 1,
        results: [
    ...
    vi.stubEnv("BB_PROJECT_ID", "project-env");
    await runCommand(
      [
        "thread",
        "search",
    ...
    expect(search).toHaveBeenLastCalledWith({
      query: { query: "needle", limitPerGroup: "3", projectId: "project-1" },
    });
    expect(collectLogPayloads(vi.mocked(console.log)).join("\n")).toContain(
      '"lastRequestedModel"',
    );
    await runCommand(["thread", "search", "needle", "--json"], register);
    expect(search).toHaveBeenLastCalledWith({
      query: {
        query: "needle",
    ...
    await expect(
      runCommand(
        ["thread", "search", "needle", "--project", "bad/path"],
        register,
      ),
