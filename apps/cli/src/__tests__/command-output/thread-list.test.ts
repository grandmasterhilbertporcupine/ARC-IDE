...
describe("bb thread list command output", () => {
  ...
  it("prints the retained requested provider and model in table and JSON output", async () => {
    const thread = fixtures.makeThreadListEntry({
      id: "thread-model",
      projectId: "proj-1",
  ...
  it("bb thread list renders archived status in the shared borderless table", async () => {
    ...
    expect(collectLogPayloads(vi.mocked(console.log))).toEqual([
      "",
      "ID                 Title  Project  Status           Requested model\n-----------------  -----  -------  ---------------  ---------------\nthread-archived-1  -      Alpha    idle (archived)  -              ",
      "",
    ]);
  });
