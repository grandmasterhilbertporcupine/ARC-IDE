...
describe("plugin execution context flags", () => {
  it("requires the context and owner together and preserves ordinary CLI origin", () => {
    expect(resolveSpawnExecutionContext({})).toEqual({ origin: "cli" });
    expect(
      resolveSpawnExecutionContext({
    ...
    for (const options of [
      { executionContext: "ctx-1" },
      { originPlugin: "arc" },
      { executionContext: " ", originPlugin: "arc" },
    ]) {
      expect(() => resolveSpawnExecutionContext(options)).toThrow(
        "must be provided together",
      );
    }
    expect(() =>
      resolveSpawnExecutionContext({
        executionContext: "x".repeat(257),
        originPlugin: "arc",
      }),
    ).toThrow("at most 256");
  });
...
describe("looksLikePath", () => {
  it.each(["C:\\Work Space\\项目", "\\\\server\\share\\project", "C:\\"])(
    "recognizes native Windows paths: %s",
    (path) => {
