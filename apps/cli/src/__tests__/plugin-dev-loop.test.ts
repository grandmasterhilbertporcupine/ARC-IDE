...
describe("createPluginDevLoop", () => {
  ...
  it("does not reload after disposal while a source scan is pending", async () => {
    const { deps } = makeDeps({ hasApp: false });
    let resolveChanges: (paths: readonly string[]) => void = () => {};
    ...
    await vi.advanceTimersByTimeAsync(300);
    loop.dispose();
    resolveChanges(["server.ts"]);
    await loop.settled();
    expect(deps.reloadPlugin).not.toHaveBeenCalled();
  });

  it("retains the watcher after a source scan error without disposing the plugin", async () => {
    const { deps, lines } = makeDeps({ hasApp: false });
    const filterChanges = vi.fn(async (paths: readonly string[]) => paths);
    filterChanges.mockRejectedValueOnce(
      new Error("source is temporarily unreadable"),
    );
    const loop = createPluginDevLoop({ ...deps, filterChanges });
    loop.handleChange("server.ts");
    await vi.advanceTimersByTimeAsync(300);
    await loop.settled();
    expect(deps.reloadPlugin).not.toHaveBeenCalled();
    expect(lines[0]).toContain(
      "source read failed: source is temporarily unreadable",
    );
    loop.handleChange("server.ts");
    await vi.advanceTimersByTimeAsync(300);
    await loop.settled();
    expect(deps.reloadPlugin).toHaveBeenCalledOnce();
    loop.dispose();
  });
});
