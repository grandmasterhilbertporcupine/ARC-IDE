...
describe.sequential("bb plugin new dependency install", () => {
  ...
  it("installs through npm in a workspace path with spaces and shell metacharacters", async () => {
    const nestedDir = join(workDir, "plugin work & (tools)");
    await mkdir(nestedDir);
    process.chdir(nestedDir);
    await installFakeNpm(nestedDir);

    await runPluginNew(["@acme/bb-plugin-portable"]);

    expect(
      await stat(
        join(nestedDir, "bb-plugin-portable", "node_modules", "typescript"),
  ...
  it("pins the scaffold to ARC's SDK version", async () => {
  ...
  it("warns, without failing, when ARC's SDK version is not on npm yet", async () => {
    ...
    expect(warnings).toContain(
      `@get-bb/plugin-sdk ${PLUGIN_SDK_VERSION} — ARC's SDK version — was not found on npm`,
    );
