...
export function registerBrowserCommands(
  ...
  scoped(
    browser
      .command("targets <leaseId>")
      .description("List native page targets available to an existing lease"),
  ).action(
    action(async (leaseId: string, options: ScopeOptions) => {
      ...
      const targets = await experimental_listDesktopBrowserTargets(connection);
      console.log(JSON.stringify({ targets }));
    }),
  );
  scoped(
    browser
      .command("evaluate <leaseId>")
      .description(
        "Run bounded page JavaScript through existing leased native CDP control",
      ),
  )
    .requiredOption(
      "--expression <javascript>",
      "Page expression, up to 20000 characters",
    )
    .option(
      "--target <id>",
      "Exact page target ID; optional only when the lease has one page",
    )
    .option(
      "--timeout-ms <ms>",
      "Evaluation timeout, 1000–30000 milliseconds",
      "10000",
    )
    .action(
      action(
        async (
          ...
          const result = await experimental_evaluateDesktopBrowser({
            connection,
