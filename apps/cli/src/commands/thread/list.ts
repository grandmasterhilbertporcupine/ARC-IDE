...
function printThreadTable(
  ...
  const rows = threads.map((thread) => [
    ...
    thread.lastRequestedModel === null
      ? "-"
      : `${thread.lastRequestedModel.providerId}/${thread.lastRequestedModel.model}`,
  ]);
  ...
  const modelWidth = Math.max(15, ...rows.map((row) => row[4].length));
  const table = renderBorderlessTable(
    {
      head: ["ID", "Title", "Project", "Status", "Requested model"],
      colWidths: [idWidth, titleWidth, projectWidth, statusWidth, modelWidth],
    },
    rows,
  );
