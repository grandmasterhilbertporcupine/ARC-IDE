...
interface PostThreadMessageArgs {
  addressing?: import("@bb/domain").ExperimentalAddressing;
  getUrl: () => string;
  threadId: string;
  message: string;
...
export function registerActionsCommands(
  ...
  parent
    ...
    .option(
      ...
      "JSON send operation and pinned agent/team recipients; routes through their coordinator",
    )
      ...
      action(
        async (id: string, message: string, opts: ThreadTellCommandOptions) => {
          const response = await postThreadMessage({
            addressing: await readAddressingFile(opts.addressingFile),
            getUrl,
            threadId: id,
            message,
  ...
  parent
    .command("retry-addressed <id>")
    .description(
      "Retry a saved addressed Send with its original recipients and attachments",
    )
    .requiredOption("--operation-id <id>", "Original Send operation UUID")
    .option("--json", "Print machine-readable JSON output")
    .action(
      action(
        async (id: string, opts: { operationId: string; json?: boolean }) => {
          const result = await createCliBbSdk(
            getUrl(),
          ).threads.experimental_retryAddressed({
            threadId: id,
            operationId: opts.operationId,
            senderThreadId: resolveSenderThreadId(id),
          });
          if (!outputJson(opts, { threadId: id, ...result })) {
            if (result.delivery === "sent" && result.experimental_addressed)
              console.log(
                `${result.experimental_addressed.summary}\nRun: ${result.experimental_addressed.runId}`,
              );
            else
              console.log(
                `Addressed retry on thread ${id}: ${result.delivery}`,
              );
          }
        },
      ),
...
async function postThreadMessage(
  ...
  const response = await sdk.threads.send({
    ...(args.addressing ? { experimental_addressing: args.addressing } : {}),
