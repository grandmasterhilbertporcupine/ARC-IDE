...
import { readAddressingFile } from "./addressing.js";
import {
  PERSONAL_PROJECT_ID,
...
export function resolveSpawnExecutionContext(args: {
  executionContext?: string;
  originPlugin?: string;
}):
  | { origin: "cli" }
  | {
      ...
      experimental_executionContextId: string;
    } {
  if (args.executionContext === undefined && args.originPlugin === undefined) {
    return { origin: "cli" };
  }
  const contextId = args.executionContext?.trim();
  const pluginId = args.originPlugin?.trim();
  if (!contextId || !pluginId) {
    throw new Error(
      "--execution-context and --origin-plugin must be provided together with non-empty values.",
    );
  }
  if (contextId.length > 256) {
    throw new Error("--execution-context must be at most 256 characters.");
  }
  return {
    origin: "plugin",
    originPluginId: pluginId,
    experimental_executionContextId: contextId,
  };
}
...
export function registerSpawnCommand(
  ...
  parent
    ...
    .option(
      ...
      "JSON send operation and pinned agent/team recipients; creates an addressed conversation",
    )
    .option(
      "--execution-context <id>",
      "Bind an immutable plugin execution context before the first turn; requires --origin-plugin",
    )
    .option(
      "--origin-plugin <id>",
      "Owning plugin for --execution-context; both flags are required",
    )
    ...
    .action(
      action(async (opts: ThreadSpawnCommandOptions) => {
        const executionContext = resolveSpawnExecutionContext(opts);
        ...
        try {
          ...
          const addressing = await readAddressingFile(opts.addressingFile);
          const senderThreadId = addressing
            ? resolveContextThreadId()
            : undefined;
