...
export function registerOpenCommand(
  ...
  parent
    .command("open")
    .description("Open an ARC thread, optionally with a file in its panel")
    .usage("[id] [path] [options]")
    .argument("[id]", "Thread ID. Omit inside an ARC thread.")
...
function resolveThreadOpenTarget(
  ...
  if (first === undefined) {
    throw new Error(
      "Missing thread ID. Pass <threadId> [path], or run inside an ARC thread.",
    );
  }
