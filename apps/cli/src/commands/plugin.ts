...
function warnIfSdkPinIsStale(pin: string | null): void {
  ...
  console.warn(
    `This plugin pins @get-bb/plugin-sdk ${pin}; ARC's SDK is ${PLUGIN_SDK_VERSION} — \`bb plugin types\` updates the pin.`,
  );
}
...
async function requirePluginManifest(
  ...
  if (typeof manifest.bb?.server !== "string") {
    console.error(
      `${rootDir} is not an ARC plugin — package.json has no "bb.server" entry.`,
    );
    process.exit(1);
  }
  return manifest;
...
async function warnIfSdkVersionUnpublished(): Promise<void> {
  ...
  if (status === "unknown") {
    console.warn(
      `Warning: could not reach the npm registry to verify that @get-bb/plugin-sdk ${PLUGIN_SDK_VERSION} — ARC's SDK version — is published.`,
    );
    console.warn(
  ...
  console.warn(
    `Warning: @get-bb/plugin-sdk ${PLUGIN_SDK_VERSION} — ARC's SDK version — was not found on npm.`,
  );
  console.warn(
  ...
  return new Promise((resolveRun, reject) => {
    ...
    const stop = (error: Error) => {
      ...
      try {
        ...
        reject(Object.assign(new Error(error.message, { cause }), output()));
      }
    };
    const timeout =
      options.timeoutMs === undefined
        ? undefined
        : setTimeout(
            () => stop(new Error("npm registry probe timed out")),
            options.timeoutMs,
          );
    const collect = (chunks: Buffer[]) => {
      ...
      return (chunk: Buffer) => {
        if (failure !== null) return;
        chunks.push(chunk.subarray(0, outputLimit - bytes));
        bytes += chunk.length;
        if (bytes > outputLimit) stop(new Error("npm output limit exceeded"));
      };
    };
    ...
    child.once("close", (code, signal) => {
      ...
      if (failure !== null || code !== 0) {
        reject(
          Object.assign(
            failure ?? new Error(`npm exited with ${signal ?? code}`),
            output(),
          ),
        );
      } else {
...
function installPlanSummary(plan: PluginCatalogInstallPlan): string {
  if (plan.kind === "bundled") {
    return `Installing ${plan.displayName}, bundled with ARC (${plan.source})`;
  }
  if (plan.official) {
    return `Installing ${plan.displayName} from the ${plan.marketplaceDisplayName} marketplace (${plan.source})`;
  }
  const author =
  ...
  return [
    ...
    `  marketplace: ${plan.marketplaceDisplayName} — a third-party marketplace`,
    `  author: ${author}`,
...
export function registerPluginCommands(
  ...
  const plugin = program
    .command("plugin")
    .description("Manage ARC plugins")
    .enablePositionalOptions();

  plugin
    .command("search <query>")
    .description(
      "Search every plugin the store lists: the plugins bundled with the app, the upstream Community catalog (bb-community), and any third-party marketplace added on this host. The Marketplace column names the source",
    )
      ...
      action(
        async (
          ...
          if (!opts.json) {
            console.log(summary);
            console.log(
              "Plugins are full-trust code running inside the ARC server. " +
                "They can read all local ARC data, including other plugins' secrets.",
            );
          }
          if (!opts.yes) {
            if (!process.stdin.isTTY) {
  ...
  plugin
    .command("types [path]")
    .description(
      "Sync a plugin's @get-bb/plugin-sdk surface to the running ARC version (default: cwd): repin the npm devDependency and the type-only devDependencies of the packages ARC shims at runtime (sonner, vaul, the portal radix families, ...) for plugins that depend on the package, or rewrite the vendored types/ declarations for plugins that still carry them",
    )
    .option(
      "--check",
        ...
        if (layout.kind === "package") {
          if (opts.check) {
            ...
            if (pending.pin !== null || pending.movedFromDependencies) {
              console.error(
                pending.pin === null
                  ? 'Move "@get-bb/plugin-sdk" from dependencies to devDependencies — ARC provides its runtime (`bb plugin types` does it for you).'
            ...
            for (const shim of pending.shimmedTypePins) {
              console.error(
                shim.movedFromDependencies
                  ? `Move "${shim.name}" from dependencies to devDependencies at ${shim.to} — ARC shims it at runtime and never bundles it (\`bb plugin types\` does it for you).`
                  : `Set "${shim.name}" to ${shim.to} in devDependencies — the version ARC shims at runtime (\`bb plugin types\` does it for you).`,
              );
            }
            process.exit(1);
          }
          ...
          if (changed === null) {
            console.log(
              `@get-bb/plugin-sdk is already pinned to ${PLUGIN_SDK_VERSION} — ARC's SDK version${hasApp ? ", and the runtime-shimmed packages are at ARC's versions" : ""}.`,
            );
            console.log(
              "The declarations are in node_modules/@get-bb/plugin-sdk/bundled-types/ — read them for exact signatures.",
            );
    ...
    .action(
      action(async (path: string | undefined) => {
        ...
        const loop = createPluginDevLoop({
          pluginId: entry.id,
          filterChanges: await createPluginSourceChangeFilter(rootDir),
  ...
  plugin
    ...
    .action(
      action(async (id: string, args: string[]) => {
        process.exitCode = await runPluginCliCommand(getUrl(), id, args ?? []);
      }),
    );
