...
function parseSurfaceStyle(value: string): AppSurfaceStyle {
  const parsed = appSurfaceStyleSchema.safeParse(value);
  if (!parsed.success) {
    throw new Error(
      `Invalid surface style '${value}'. Expected one of: default, liquid-glass.`,
    );
  }
  return parsed.data;
}
...
export function registerThemeCommands(
  ...
  const theme = program
    .command("theme")
    .description("Manage the app theme, surface style, and favicon color");
  ...
  theme
    ...
    .action(
      action(async (opts: JsonOutputOptions) => {
        ...
        console.log(`Surface style: ${catalog.active.surfaceStyle}`);
        ...
        const updated =
          opts.faviconColor === undefined
            ...
            : await sdk.theme.set({
                ...
                surfaceStyle: (await sdk.theme.get()).surfaceStyle,
              });
    ...
    .action(
      action(async (color: string, opts: JsonOutputOptions) => {
        ...
        const updated = await sdk.theme.set({
          ...
          surfaceStyle: active.surfaceStyle,
        });
        if (outputJson(opts, updated)) return;
  ...
  const style = theme
    .command("style")
    .description(
      "Manage the surface style without changing the palette or favicon",
    );

  style
    .command("set <style>")
    .description("Set the surface style (default or liquid-glass)")
    .option("--json", "Print machine-readable JSON output")
    .action(
      action(async (value: string, opts: JsonOutputOptions) => {
        const surfaceStyle = parseSurfaceStyle(value);
        const sdk = createCliBbSdk(getUrl());
        const active = await sdk.theme.get();
        const updated = await sdk.theme.set({
          themeId: active.themeId,
        ...
        if (outputJson(opts, updated)) return;
        console.log(`Surface style set to ${updated.surfaceStyle}`);
      }),
    );
  ...
  style
    .command("reset")
    .description(
      "Reset the surface style without changing the palette or favicon",
    )
    .option("--json", "Print machine-readable JSON output")
    .action(
      action(async (opts: JsonOutputOptions) => {
        ...
        const updated = await sdk.theme.set({
          themeId: active.themeId,
          faviconColor: active.faviconColor,
          surfaceStyle: defaultAppSurfaceStyle,
        });
        if (outputJson(opts, updated)) return;
        console.log(`Surface style reset to ${updated.surfaceStyle}`);
      }),
    );

  theme
    .command("show")
    .description("Show the active theme, surface style, and favicon color")
    ...
    .action(
      action(async (opts: ThemeShowCommandOptions) => {
        ...
        console.log(`Surface style: ${active.surfaceStyle}`);
