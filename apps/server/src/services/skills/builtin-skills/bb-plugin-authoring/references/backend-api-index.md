...
The Context embedding exports describe the fixed offline host model and its
validated inputs, limits, generation status, token counts, vectors and failures.
`ExperimentalHostRpcContext.experimental_contextEmbeddings` exposes the host
operations. Status initializes a helper; count/embed require its exact
`expectedGeneration` and do not start a replacement helper. These contracts are
also re-exported from `@get-bb/plugin-sdk`.
