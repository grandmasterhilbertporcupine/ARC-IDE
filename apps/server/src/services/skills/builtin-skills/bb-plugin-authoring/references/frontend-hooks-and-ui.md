...
experimental_openFilePreview(options), experimental_openFileExternally(options) }`.
  ...
  `openUrl` owns HTTP(S) only and returns false for schemes ARC
...
- Use a vendored ARC prompt icon-button recipe for native-matching action chrome
...
component kit is removed. The app module still exports focused ARC capability
...
- `bb plugin new` pre-vendors button, card, input, checkbox, dialog (plus
  ...
  ARC. Import via the `@/*` alias: `import { Button } from
...
@bb/table` — the ARC registry carries the full stock set (~44 items:
  accordion, alert-dialog, calendar, chart, command, form, sheet, table,
  …), generated from the ARC app's own component source, so vendored code is
  version-matched to your ARC by construction. Edit the copies freely; they
...
renderers never had the `experimental_Original` alias. ARC removes these aliases
in bb 0.42.
