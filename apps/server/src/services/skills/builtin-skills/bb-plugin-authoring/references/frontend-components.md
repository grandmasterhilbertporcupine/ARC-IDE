...
- `UrlLink` — a real anchor whose ordinary HTTP(S) activation
  ...
  internal ARC routes in SPA history, preserves modifier clicks, copying,
...
- `experimental_FileLink` — a real anchor for an explicit live file target:
  ...
  current surface's shared ARC preview. Its lazy context menu offers the
...
className?, draftKey? }` — the `default*` props are SEEDS, not controlled
  ...
  project". That choice submits ARC's personal-project id in `projectId` (not
