...
host-dependent: ARC asks that provider's bridge for `provider/health` and lists
...
the same name, the earlier registration wins and ARC logs the conflict. A
...
credentials and sensitive URLs with `secret: true`; ARC passes the real value
...
while the proxy is usable, or `null` otherwise. ARC uses it only when the
