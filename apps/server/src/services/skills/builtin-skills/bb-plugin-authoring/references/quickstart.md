...
- `bb.branding` (required) — declare `bb.branding.icon` as either the plugin's
  canonical ARC icon name, such as `Zap`, or a plugin-relative compact SVG path
  ...
  ARC validates path-shaped SVGs (well-formed XML with an `<svg>` root, no
  ...
  colors are ignored.
  ARC reuses this icon on roomy surfaces when no logo override is declared.
  ...
  reject the invalid paths and SVGs described above. Every SVG ARC serves
  ...
  logo is required. ARC uses a declared logo where space permits, such as roomy
...
- `bb.branding.experimental_icons` (optional) — the plugin's own icon
  ...
  `bb.providers.register` `icon`. ARC serves each file hashed from
...
- `engines.bb` — optional compatibility string checked during runtime and
  update selection against the ARC app version.
- `engines.bbPluginSdk` — optional SDK compatibility string. The scaffold uses
  the repository SDK version. ARC validates this range before use. Absent means
...
- Default to `bb-plugin-hello` for the package name. Scoped names such as
  ...
  package-name component minus the `bb-plugin-` prefix. ARC lowercases it,
