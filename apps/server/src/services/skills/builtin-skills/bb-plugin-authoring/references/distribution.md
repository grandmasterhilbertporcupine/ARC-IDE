...
Plugins included with ARC are separate from the upstream Community catalog.
The upstream marketplace maintainers decide which submissions to accept.
Bundled plugins ship inside ARC and install from that local copy with no
network fetch.
...
Community marketplace requires one, and a plugin that is only installed from
...
`"overview": "./overview/<plugin-id>.md"`. A bundled ARC plugin uses the same
