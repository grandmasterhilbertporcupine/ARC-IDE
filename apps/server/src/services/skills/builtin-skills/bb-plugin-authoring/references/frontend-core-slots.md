...
bundled JavaScript/TypeScript in the ARC app shell without a React slot. It is
...
- `experimental_appOverlay` → `{}` (deliberately no props). An additive,
  ...
  `{ id, component }`. ARC mounts every registration once per app window
  ...
  `useComposerView`, remain limited to that surface. ARC supplies no chrome,
...
- `navPanel` → `{ subPath: string }` — owns the whole route at
  ...
  ARC automatically wraps every plugin page in the same host-owned App panel
  ...
  Terminal itself. ARC owns the desktop split, compact drawer, header/panel
  ...
  content tab closes an otherwise empty panel; if fixed tabs remain, ARC falls
  ...
  `experimental_target: { validate(value): value is Target }`; ARC checks JSON
...
- `experimental_sidebarFooter.register` → managed host-rendered icon items in
  ...
  returns nothing. ARC keeps only one disclosure open across all plugins.
...
- `experimental_sidebarNavigation` → replaces the bounded navigation controls
  ...
  the quick palette. No inline search field or query state exists. ARC keeps the
...
- `fileOpener` → `{ path: string, source, Original }` — register as a viewer/editor
  ...
  by default. Users can pin ARC's preview or a specific opener per extension
...
projectId, experimental_hostId? }` (nullable fields). The optional host ID
  ...
  `Original` is ARC's preview bound to this file; render it to
