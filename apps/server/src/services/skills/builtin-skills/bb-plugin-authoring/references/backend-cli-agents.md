...
bb.agents.registerTool({
  ...
  // Optional row presentation (grammar v3). Without it, ARC shows its normal
...
// selects this plugin's own ids when ARC resolves a thread/session config.
...
approval — falls back to ARC's standard `Running tool …` / `Ran tool …`
...
Ordering is standard ARC instructions, selected tools' static snippets,
...
takes effect only when the provider session is next started/resumed; ARC never
...
`prompt` can be `null`.
## Pinned skill directories

An owning execution-context plugin may return `experimental_skillSnapshots` from `bb.agents.configure`. Each entry is `{name, description, files:[{path, contentBase64, executable}]}` and must include a standard `SKILL.md` whose YAML name and description match the entry. Paths are relative to that skill directory. A snapshot carries supporting files as well as the Markdown instructions; it must fit 128 files and 1 MiB. The whole selection must fit 32 unique skill names and 4 MiB.

This experimental field is rejected for ordinary unbound contributions. ARC validates and stages exact bytes through its existing skill-tree transport and requires the provider's skill configuration capability before starting the agent. Assigned names replace matching ARC-supplied catalog entries, while shared and project skills remain available. A skill is instruction content, not an additional permission or a guarantee that the model invokes it. Keep immutable versions in the owning plugin's storage and return the version bound to the admitted session.
