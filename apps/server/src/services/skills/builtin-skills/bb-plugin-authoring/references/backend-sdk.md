...
### bb.experimental_turns

`prepare`, `getPreparation`, `startPrepared`, and `interrupt` reserve one immutable
owned input on an existing conversation. Supply operationId, projectId, threadId,
executionContextId, exact environment and execution tuple, and resolved nonempty
input without dynamic mentions. Release requires the retained expectedRevision
and an idle thread; `prepared_turn_busy` is retryable without changing admission.
The queue stays separate from manual inputs and retains one actual client request.
No Send-now, queue edit/group, retry or message edit may replay that operation.
Owner loss before dispatch requires explicit reconciliation; pre-dispatch cancel
is terminal. After allocation the operation never creates another client request.
Conditional interrupt targets exact native identity and leaves unknown outcomes
unsettled until accepted and terminal events prove the result.

The owning `bb.agents.configure` callback alone receives `experimental_turnContext`
with ownerPluginId, operationId, executionContextId and clientTurnRequestId. The
thread origin and permanent execution context are unchanged. Native plugin tools
receive `experimental_invocation` with exact providerThreadId, turnId, callId and
`ownedTurn`. Ownership includes foreign plugins; null means proven unowned.
Missing, mixed or stale native mappings reject before tool invocation. The fake
host supports `experimental_preparedTurns`; non-native test contexts may omit
invocation identity.

### bb.experimental_threads

Server-bound `prepare`, `getPreparation` and `startPrepared` reserve a worker
under the actual plugin owner and a caller-owned operationId. Seal explicit
title, project, executionContextId, provider execution, environment, input and
`turnPolicy: "single" | "conversation"`. Preparation provisions without a
provider turn; release requires the exact returned revision and environment.
Identical retries observe one retained reservation/dispatch receipt. A single
worker rejects extra sends, retries, Send-now, edits, clear and manual
compaction. Stop/deletion retains its tombstone; stale owner generations and
uncertain native starts require reconciliation, never blind replay. Observe
the retained clientTurnRequestId through native turn/input/accepted and the
matching turn/completed scope using bb.sdk.threads.events.list.

### Private server-plugin calls

`bb.rpc.experimental_registerInternal(contract, handlers)` and
`bb.rpc.experimental_client({pluginId, contract})` use Standard Schema with
registrations outside public RPC routes. Call `client.call(method, input,
{signal})`; core supplies each handler's actual `{callerPluginId, signal}` and
fences both plugin generations across async validation and disposal. Errors
do not establish whether a native effect occurred. Keep durable effect identity
and observe retained receipts before retrying.
