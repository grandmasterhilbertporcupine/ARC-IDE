...
the word plugin. Do not add the word ARC unless the product name holds it.
...
An App Store listing has two parts. ARC holds both in one field.

**The hook is the first sentence.** ARC clamps the description to two lines in a
...
```json
{
  "id": "notes",
  "displayName": "Notes",
  "description": "Keeps project notes beside each ARC thread.",
...
entry can also use a supported ARC host icon name.
...
ARC masks SVG icons with the surrounding text color. Use a single-color SVG for
...
Install the plugin in ARC first. Capture the real plugin surface. Do not draw a
...
the session. Drive ARC with that tool. Open the plugin surface. Capture the
image.
