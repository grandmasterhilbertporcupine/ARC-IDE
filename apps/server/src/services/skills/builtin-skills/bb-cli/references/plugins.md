...
- An ARC plugin is a TypeScript package running inside the ARC server, extending
...
- **ARC plugin catalog** (store under `/api/v1/plugin-catalog`):
  - The reserved **Included with ARC marketplace** has the name `bb-official`. It
    ...
    `bb marketplace list`. Its plugins appear in the first Browse shelf, Included with ARC, and in their category shelves. It can be neither added nor
    removed.
  - The store lists the **Community marketplace** catalog: a manifest
  ...
  - `bb plugin search <query> [--json]` — search the catalog by id,
    ...
    **Installs** column appears once the curated
    ...
    entries. Included with ARC entries use the count for the same plugin id. With
...
- **Third-party marketplaces** (routes under `/api/v1/marketplaces`):
  - `bb marketplace add <source>` — add a marketplace from an https manifest
    ...
    directory before it sends the request. ARC validates the
    ...
    ARC ignores unknown v2 fields, except in npm and git source objects. ARC
