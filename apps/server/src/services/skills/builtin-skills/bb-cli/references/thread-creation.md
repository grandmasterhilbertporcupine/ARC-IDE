...
- Plugins with immutable execution contexts can use paired
  `--execution-context <id> --origin-plugin <id>` flags (experimental). Persist
  the context through the owning plugin first. Its configuration callback must
  validate the context for the actual project and thread before provider dispatch;
  missing, disabled, or failed owner configuration stops dispatch. The identifier
  is nonempty and at most 256 characters, is not a credential, and does not inherit
  into forks or children. SDK callers use `experimental_executionContextId` with
  `origin: "plugin"` and `originPluginId`; plugin-bound SDK calls enforce their own
  plugin identity.
...
- `bb updates` runs the default `bb updates status` action. It aggregates ARC and provider
...
- Use `bb project attachment upload <project-id> --client-file <path>` when the
  bytes live on the CLI machine, including when the CLI and ARC server are on
...
- Top-level `customModels` in the same `config.json` registers extra picker
  ...
  This list has no set/unset CLI surface. Edit the JSON and restart ARC.
