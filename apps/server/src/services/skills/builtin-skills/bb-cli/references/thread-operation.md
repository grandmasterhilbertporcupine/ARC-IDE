...
- Recover a saved failed addressed Send with `bb thread retry-addressed <id> --operation-id <original-UUID>` or `sdk.threads.experimental_retryAddressed({threadId,operationId,senderThreadId?})`. The server loads its original recipients, prompt and attachments; repeated clicks or retries preserve one operation. The CLI carries `BB_THREAD_ID` when present; SDK agent callers must carry the same sender context. Owned workers cannot send or retry coordinated work and must use their run's admitted messaging tools. The timeline's **Retry saved Send** uses the same route.

- `bb thread spawn` and `bb thread tell` accept `--addressing-file <path>` to send work to published agent/team revisions through one coordinator. The JSON shape is `{ "operationId": "<UUID>", "recipients": [{ "pluginId": "arc", "kind": "agent", "entityId": "<id>", "versionId": 1, "scopeKey": "project:<project-id>", "label": "Agent name" }] }`. Library pins use `scopeKey: "library"`. Keep the same operation ID and input for retries; choose a fresh UUID for a new request. Addressed work cannot use `--send-at`. The SDK uses `experimental_addressing` on `threads.spawn` and `threads.send`, plus `senderThreadId` for agent callers. Addressed CLI spawn preserves `BB_THREAD_ID` without changing the new conversation's parent; ordinary spawn does not accept `senderThreadId`.
...
- Use `bb thread search <query> [--project <id>] [--limit <1-50>]` to search
  titles and conversation content. Omit `--project` to search all projects;
  scoped totals and limits apply within that project. List and search JSON
  include `lastRequestedModel: {providerId, model} | null` from the latest
  retained execution request, not a claim about the provider's actual model. Use
...
- Use `bb thread open <path>` inside an ARC thread to open a Markdown, HTML, or
  other workspace file for the user in the ARC IDE's thread panel.
...
[thread-id]` to change a matching open pane in every connected ARC app window.
  Inside an ARC thread, omit the ID to use `BB_THREAD_ID`. The command reports
