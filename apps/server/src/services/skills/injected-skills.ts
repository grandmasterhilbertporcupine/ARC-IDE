...
interface SkillTreeRegistryLimits {
  maxBytes: number;
  maxTrees: number;
  maxEntries: number;
}

const MAX_SKILL_TREE_BYTES = 10 * 1024 * 1024;
const MAX_SKILL_TREE_ENTRIES = 1_000;

export class SkillTreeRegistry {
  readonly #rootsByHash = new Map<string, string>();
  readonly #manifestsByHash = new Map<string, SkillTreeManifest>();
  readonly #limits: SkillTreeRegistryLimits;
  #retainedBytes = 0;
  #retainedEntries = 0;

  constructor(
    limits: SkillTreeRegistryLimits = {
    ...
    for (const value of Object.values(limits)) {
      if (!Number.isSafeInteger(value) || value <= 0) {
        throw new Error("Skill tree registry limits must be positive integers");
      }
    }
    this.#limits = { ...limits };
  }

  register(manifest: SkillTreeManifest, sourceRootPath: string): void {
    const byteCount = manifest.entries.reduce(
      (total, entry) => total + entry.bytes.length,
      0,
    );
    if (
      byteCount > MAX_SKILL_TREE_BYTES ||
      manifest.entries.length > MAX_SKILL_TREE_ENTRIES
    ) {
      throw new Error(
        `Skill tree cannot be registered: ${sourceRootPath}. Reduce it to at most ${MAX_SKILL_TREE_ENTRIES} files and ${MAX_SKILL_TREE_BYTES} bytes.`,
      );
    }
    if (hashSkillTreeEntries(manifest.entries) !== manifest.treeHash) {
      throw new Error(`Skill tree manifest hash mismatch: ${sourceRootPath}`);
    }
    if (this.#manifestsByHash.has(manifest.treeHash)) {
      this.#rootsByHash.set(manifest.treeHash, sourceRootPath);
      return;
    }
    if (
      this.#retainedBytes + byteCount > this.#limits.maxBytes ||
      this.#manifestsByHash.size + 1 > this.#limits.maxTrees ||
      this.#retainedEntries + manifest.entries.length > this.#limits.maxEntries
    ) {
      throw new Error(
        `Skill tree registry capacity exhausted while registering ${sourceRootPath}. Existing skill snapshots remain available. Restart the server to clear retained snapshots; in development, restart the dev server.`,
      );
    }
    const snapshot = copySkillTreeManifest(manifest);
    this.#manifestsByHash.set(manifest.treeHash, snapshot);
    this.#rootsByHash.set(manifest.treeHash, sourceRootPath);
    this.#retainedBytes += byteCount;
    this.#retainedEntries += manifest.entries.length;
  }

  readManifest(treeHash: string): SkillTreeManifest | undefined {
    const manifest = this.#manifestsByHash.get(treeHash);
    return manifest === undefined ? undefined : copySkillTreeManifest(manifest);
  }
...
function copySkillTreeManifest(manifest: SkillTreeManifest): SkillTreeManifest {
  return {
...
function readSkillCandidate(
  ...
  args.skillTreeRegistry.register(manifest, args.candidatePath);
  return {
    kind: "tree",
    sourceType: args.sourceType,
    name: frontmatter.data.name,
