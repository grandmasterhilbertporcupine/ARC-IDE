...
export function appendClientTurnEvent(
  ...
  if (args.type === "client/turn/requested") {
    assertPreparedThreadMutable(deps.db, args.threadId);
    assertPreparedTurnHistoryMutable(deps.db, args.threadId);
  }
...
export function appendClientTurnEventInTransaction(
  ...
  if (args.type === "client/turn/requested") {
    assertPreparedThreadMutable(db, args.threadId);
    assertPreparedTurnHistoryMutable(db, args.threadId);
  }
  return appendBuiltClientTurnEvent(
...
export function appendPreparedClientTurnRequestedEventWithNotificationInTransaction(
  ...
  assertPreparedThreadRequest(db, args.threadId, args.requestId);
  if (getThreadTurnPreparationByRequest(db, args.requestId) === null)
    assertPreparedTurnHistoryMutable(db, args.threadId);
