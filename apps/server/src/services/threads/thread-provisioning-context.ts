...
export const threadProvisionContextSchema = z.object({
  request: threadProvisionCommonPayloadSchema,
  state: z.object({
    environmentId: z.string().nullable(),
    provisionEventSequence: z.number().int().nullable(),
    provisioningId: z.string().min(1),
    stage: z.enum(threadProvisioningStageValues),
    workspaceReadyEventSequence: z.number().int().nullable(),
  }),
});
...
interface CreateMetadataPendingContextArgs {
  ...
  clientRequestId: ClientTurnRequestId | null;
...
export function createMetadataPendingContext(
  ...
  return {
    request: {
      ...
      ...(args.experimental_addressing
        ? { experimental_addressing: args.experimental_addressing }
        : {}),
    },
    state: {
      environmentId: null,
