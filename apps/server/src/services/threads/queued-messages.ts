...
import { getPreparedTurnQueueIds, hasUnsettledPreparedTurn } from "@bb/db";
import type {
  PromptInput,
...
async function sendClaimedQueuedMessageForIdleProviderThread(
  ...
  if (
    getPreparedTurnQueueIds(
      deps.db,
      args.queuedMessages.map((row) => row.id),
    ).size > 0 ||
    hasUnsettledPreparedTurn(deps.db, args.thread.id)
  )
    return null;
  if (args.mode !== "auto") {
    return null;
...
async function sendClaimedQueuedMessageForThread(
  ...
  for (const message of args.queuedMessages)
    resolveMessageSenderThreadId(deps, {
      senderThreadId: message.senderThreadId ?? undefined,
