...
interface SendThreadMessageArgs {
  preparedQueuedMessageId?: string | null;
...
export function resolveMessageSenderThreadId(
  ...
  if (
    ownerControlledParentNotification(
      deps.db,
      senderThread.id,
      args.targetThread.id,
    ) !== null
  )
    throw new ApiError(
      409,
...
export async function sendThreadMessage(
  ...
  assertPreparedQueueDispatch(
    deps.db,
    args.thread.id,
    args.preparedQueuedMessageId ? [args.preparedQueuedMessageId] : [],
  );
...
async function sendThreadMessageWithoutContextClear(
  ...
  const ownedTurnContext = preparedTurnConfiguration(
    deps.db,
    args.preparedQueuedMessageId ?? null,
    requestId,
  );
  if (
    ownedTurnContext !== null &&
    (mode !== "start" ||
    ...
    throw new ApiError(
      ...
      "Owned turns require a fresh input on the existing ready conversation",
    );
  ...
  const beforeAppendInTransaction: SendThreadMessageTransactionPreflight = ({
    ...
    recordPreparedDispatch(tx, {
      threadId: thread.id,
      queuedMessageId: args.preparedQueuedMessageId ?? null,
      requestId,
    ...
    recordPreparedTurnDispatch(tx, {
      threadId: thread.id,
      queueId: args.preparedQueuedMessageId ?? null,
      requestId,
      input,
      execution,
    });
  ...
  const initiator: ThreadTurnInitiator =
    args.retryOf !== undefined || ownedTurnContext !== null
      ? "system"
      : senderThreadId
        ? "agent"
        : "user";
  ...
  if (mode === "start") {
    const commandArgs = {
      experimental_turnContext: ownedTurnContext,
      thread,
      fork: null,
      input,
      ...(inputGroups !== undefined ? { inputGroups } : {}),
