...
import { readAddressedProvision } from "./addressed-dispatch.js";
...
export function loadActiveThreadProvisionContext(
  ...
  const context =
    ...
    readAddressedProvision(deps.db, threadId)?.context ??
    null;
  if (
    !thread ||
...
export function failThreadProvisioning(
  ...
  const preparation = getThreadPreparation(deps.db, args.thread.id);
  if (preparation?.state === "provisioning") {
    deps.db
