...
import {
  ...
  assertPreparedTurnRequestMutable,
  events,
...
function resolveEditableTurnCandidate(
  ...
  assertPreparedTurnHistorySuffixMutable(db, thread.id, requestRow.sequence);
  assertPreparedTurnRequestMutable(db, request.requestId);
  if (
...
export async function editThreadMessage(
  ...
  assertPreparedThreadMutable(deps.db, args.thread.id);
