...
export async function acceptThreadSendRequest(
  ...
  assertAddressedSend(args.payload);
  if (args.payload.experimental_addressing) {
    const result = await dispatchAddressedMessage(
      deps,
      args.thread,
      args.payload,
    );
    return { ok: true, delivery: "sent", experimental_addressed: result };
  }
