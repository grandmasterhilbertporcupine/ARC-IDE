...
import { stageAssignedSkillSnapshots } from "../skills/assigned-skills.js";
...
interface ResolveThreadRuntimeCommandConfigArgs {
  experimental_turnContext?:
    | import("@get-bb/plugin-sdk").ExperimentalOwnedTurnIdentity
    | null;
...
export async function resolveThreadRuntimeCommandConfig(
  ...
  const conditionalConfiguration = await resolvePluginAgentConfiguration({
    context: {
      experimental_turnContext: args.experimental_turnContext ?? null,
      thread: {
        id: args.thread.id,
        ...
        experimental_executionContextId:
          args.thread.experimental_executionContextId ?? null,
      },
      project: {
        id: project.id,
        kind: project.kind,
  ...
  const assignedSources = stageAssignedSkillSnapshots(
    deps.skillTreeRegistry,
    conditionalConfiguration.assignedSkillSnapshots,
  );
  const assignedNames = new Set(assignedSources.map((source) => source.name));
  const inheritedSkillSources = resolveSkillCatalog(deps, {
    projectSkillSources,
  ...
  const injectedSkillSources = [
    ...inheritedSkillSources.filter(
      (source) => !assignedNames.has(source.name),
    ),
    ...assignedSources,
  ];
  ...
  for (const contribution of dynamicToolContributions) {
    ...
    if (contribution.pluginId === null) {
      ...
      instructionSections.push(
        `The following instructions come from the ARC plugin "${contribution.pluginId}" for its tool "${contribution.tool.name}":`,
  ...
  for (const contribution of listPluginInstructionContributions()) {
    ...
    instructionSections.push(
      `The following instructions come from the ARC plugin "${contribution.pluginId}":`,
      text,
    );
  }
  for (const contribution of conditionalConfiguration.dynamicInstructions) {
    instructionSections.push(
      `The following dynamic instructions come from the ARC plugin "${contribution.pluginId}":`,
      contribution.text,
    );
  }
