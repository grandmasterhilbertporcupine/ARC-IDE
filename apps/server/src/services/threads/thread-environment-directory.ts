...
export const UPDATE_ENVIRONMENT_DIRECTORY_TOOL: DynamicTool = {
  ...
  description:
    "Move this ARC thread to a different working directory for subsequent turns. Use this when the user asks to switch to a new checkout, worktree, or local directory. The path must be an absolute existing directory on the current host. The tool reuses this project's existing ARC environment for that host/path, otherwise it creates an unmanaged environment after validating the path. Another project may hold its own environment for the same directory; that is allowed, except for an ARC-managed worktree owned by another project, which this tool refuses. After a successful switch, stop the current turn because the running provider cwd will not change until the next turn.",
  inputSchema: {
    type: "object",
    properties: {
      path: {
...
function validateDirectoryPath(path: string): string | null {
  ...
  const failure = getProjectPathValidationMessage(path);
  if (failure === PROJECT_PATH_ROOT_MESSAGE) {
...
export async function handleUpdateEnvironmentDirectoryToolCall(
  ...
  assertPreparedTurnHistoryMutable(deps.db, args.thread.id);
  ...
  const normalizedPath = normalizeProjectPathInput(input.data.path);
