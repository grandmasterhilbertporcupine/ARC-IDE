...
import { assertPreparedQueueDispatch } from "./prepared-thread-state.js";
import {
  getPreparedTurnQueueIds,
  hasUnsettledPreparedTurn,
  preparedTurnQueueCanDispatch,
} from "@bb/db";
...
async function runDispatchAttempt(
  ...
  const queueIds =
    args.source.kind === "drain"
      ? args.source.claimed.map((row) => row.id)
      : [];
  if (!preparedTurnQueueCanDispatch(deps.db, queueIds))
    throw new ApiError(
      409,
      "prepared_turn_revoked",
      "This owned turn is no longer released",
    );
  const ownedTurn = getPreparedTurnQueueIds(deps.db, queueIds).size > 0;
  ...
  if (ownedTurn && sendNow)
    throw new ApiError(
      409,
      "prepared_turn_owned",
      "Send now cannot release an owned turn",
    );
  if (
    hasUnsettledPreparedTurn(deps.db, thread.id) ||
    (ownedTurn && thread.status !== "idle")
  )
  ...
  await sendThreadMessage(deps, {
    preparedQueuedMessageId: claimed?.length === 1 ? claimed[0]!.id : null,
    environment,
    payload: ownedTurn
      ? { ...resolvedPayload, mode: "start" }
      : resolvedPayload,
    thread,
