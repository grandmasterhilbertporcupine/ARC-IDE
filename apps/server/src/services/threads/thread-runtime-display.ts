...
interface ToThreadListEntryResponseFromLatestSessionArgs {
  ...
  lastRequestedModel: ThreadListEntry["lastRequestedModel"];
  hostConnected: boolean;
  latestSession: HostDaemonSessionRow | null;
  now?: number;
...
export function toThreadListEntryResponses(
  ...
  const requestedModelByThreadId = new Map(
    listLastRequestedModelsByThreadIds(deps.db, {
      threadIds: args.threads.map((thread) => thread.id),
    }).map((row) => [row.threadId, row.model]),
  );
  return args.threads.map((thread) => {
    const model = requestedModelByThreadId.get(thread.id) ?? null;
    return toThreadListEntryResponseFromLatestSession({
      lastRequestedModel:
        model === null ? null : { providerId: thread.providerId, model },
      activity: activityByThreadId.get(thread.id) ?? EMPTY_THREAD_ACTIVITY,
...
function toThreadListEntryResponseFromLatestSession(
  ...
  return {
    ...thread,
    activity: args.activity,
    lastRequestedModel: args.lastRequestedModel,
