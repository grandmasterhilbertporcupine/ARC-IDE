...
import {
  assertAddressedSend,
  assertAddressedActorThread,
  addressedCreateHash,
  findAddressedCreation,
  readAddressedProvision,
  reserveAddressedCreation,
  finishAddressedProvision,
} from "./addressed-dispatch.js";
import {
...
import {
  beginPreparedThreadProvision,
  failPreparedThreadCreation,
} from "./prepared-thread-provision.js";
...
interface CreateProvisioningThreadArgs {
  preparation?: { threadId: string; assertCurrent(): void };
...
async function createPendingThreadAndAttemptFirstDispatch(
  ...
  args: CreateProvisioningThreadArgs & {
    ...
    addressedRequestHash: string | null;
  },
) {
  args.preparation?.assertCurrent();
  const thread = createThreadRecord(deps, {
    ...(args.preparation ? { threadId: args.preparation.threadId } : {}),
  ...
  try {
    ...
    if (args.request.experimental_addressing) {
      assertAddressedSend(args.request);
      if (args.fork || args.preparation)
        throw new ApiError(
          422,
          "addressed_fork_unsupported",
          "Start addressed work in a new conversation rather than a fork or owned preparation.",
        );
      if (!args.addressedRequestHash)
        throw new Error("Addressed creation fingerprint is missing");
      reserveAddressedCreation(
        deps.db,
        thread.id,
        args.request.experimental_addressing.operationId,
        args.addressedRequestHash,
      );
      ...
      if (!outcome.applied)
        throw new Error("Addressed workspace preparation could not start");
      const context = requestThreadProvision(deps, {
        thread: getThreadSafe(deps, thread.id),
        ...
        titleProvided: true,
        experimental_addressing: args.request.experimental_addressing,
      });
    ...
    if (args.preparation) {
      args.preparation.assertCurrent();
      beginPreparedThreadProvision(deps, {
        thread,
        execution,
        environmentIntent: args.environmentIntent,
        input: args.request.input,
...
const addressedCreates = new Map<
  string,
  { hash: string; task: Promise<Thread> }
>();
...
export async function createThreadFromRequest(
  ...
  const addressing = request.experimental_addressing;
  if (!addressing && request.senderThreadId !== undefined)
    throw new ApiError(
      422,
      "invalid_request",
      "senderThreadId on creation requires experimental_addressing",
    );
  if (!addressing) return createThreadFromRequestInner(deps, request, options);
  for (const actorThreadId of [
    request.senderThreadId,
    request.parentThreadId,
    request.sourceThreadId,
    request.startedOnBehalfOf?.senderThreadId,
  ])
    assertAddressedActorThread(deps.db, actorThreadId);
  if (request.experimental_executionContextId || options.preparation)
    throw new ApiError(
      409,
      "addressed_worker_denied",
      "Owned worker preparations cannot start coordinated addressed work.",
    );
  const key = `${request.projectId}:${addressing.operationId}`;
  const hash = addressedCreateHash(request);
  const current = addressedCreates.get(key);
  if (current) {
  ...
  const task = createThreadFromRequestInner(deps, request, options);
  addressedCreates.set(key, { hash, task });
  try {
    return await task;
  } finally {
    addressedCreates.delete(key);
  }
}

async function createThreadFromRequestInner(
  deps: ThreadCreateDeps,
  rawRequestInput: ThreadCreateServiceRequestInput,
  options: ThreadCreateOptions = {},
) {
  assertAddressedSend(rawRequestInput);
  const addressedRequestHash = rawRequestInput.experimental_addressing
    ? addressedCreateHash(rawRequestInput)
    : null;
  if (rawRequestInput.experimental_addressing) {
    const previous = findAddressedCreation(
      deps.db,
      rawRequestInput.projectId,
      rawRequestInput.experimental_addressing.operationId,
    );
    if (previous) {
      const record = readAddressedProvision(deps.db, previous.id)!;
      const fingerprint = addressedRequestHash;
      if (record.requestHash !== fingerprint)
        throw new ApiError(
          409,
          "addressed_operation_conflict",
          "This send operation already belongs to different input. Start a new message.",
        );
      if (record.state === "failed" && previous.environmentId)
        await finishAddressedProvision(deps, previous.id);
      return getThreadSafe(deps, previous.id);
    }
  }
  ...
  if (rawRequestInput.experimental_executionContextId !== undefined) {
    if (rawRequestInput.origin !== "plugin") {
      throw new ApiError(
        400,
        "invalid_request",
        'experimental_executionContextId requires origin "plugin"',
      );
    }
    const parsed =
      createThreadRequestSchema.shape.experimental_executionContextId.safeParse(
        rawRequestInput.experimental_executionContextId,
      );
    if (!parsed.success) {
      throw new ApiError(
        400,
        "invalid_request",
        "experimental_executionContextId must be a non-empty identifier of at most 256 characters",
      );
    }
    rawRequestInput = {
      ...rawRequestInput,
      experimental_executionContextId: parsed.data,
    };
  }
  const requestInput = {
    ...rawRequestInput,
  ...
  const pluginMentionContext = requestInput.experimental_addressing
    ? []
