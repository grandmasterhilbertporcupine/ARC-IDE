...
import { settlePreparedEnvironment } from "./prepared-thread-provision.js";
import { finishAddressedProvision } from "./addressed-dispatch.js";
import { appendSystemErrorEvent } from "./thread-events.js";
...
async function startThreadIfEnvironmentReady(
  ...
  if (args.context.request.clientRequestId === null) {
    settlePreparedEnvironment(deps, args.thread.id, args.environment);
    forgetActiveThreadProvisionContext(args.thread.id);
    return;
  }

  if (args.context.request.experimental_addressing) {
    ...
    forgetActiveThreadProvisionContext(args.thread.id);
    try {
      await finishAddressedProvision(deps, args.thread.id);
    } catch (error) {
      appendSystemErrorEvent(deps, {
        threadId: args.thread.id,
        environmentId: args.environment.id,
        code: "addressed_dispatch_failed",
        message: "Addressed work could not start",
