...
interface PluginRuntimeContext {
  ...
  validateRpcValue: (
    schema: import("@get-bb/plugin-sdk").StandardSchemaV1,
    value: unknown,
    phase: "input" | "output",
  ) => Promise<unknown>;
  normalizeRpcJsonResult: (value: unknown) => import("@bb/domain").JsonValue;
...
export function createPluginRuntime(context: PluginRuntimeContext) {
  ...
  function checkEngineRange(manifest: PluginManifest): string | undefined {
    ...
    if (!semver.satisfies(version, manifest.bbEngineRange)) {
      return `requires ARC ${manifest.bbEngineRange}, this is ${version.version}`;
    }
    return undefined;
  }
  ...
  async function loadOne(row: InstalledPluginRow): Promise<string | null> {
    ...
    const handle = createPluginApi({
      ...
      preparedThreads: deps.preparedThreads
        ? (signal) => deps.preparedThreads!(row.id, signal)
        : undefined,
      preparedTurns: deps.preparedTurns
        ? (signal) => deps.preparedTurns!(row.id, signal)
        : undefined,
      async callInternalRpc(args) {
        const target = loaded.get(args.pluginId);
        const requireCurrent = () => {
          if (
            loaded.get(row.id)?.handle !== handle ||
            loaded.get(args.pluginId) !== target ||
            !target ||
            handle.lifecycleSignal.aborted ||
            target.handle.lifecycleSignal.aborted
          ) {
            throw new Error("Internal plugin RPC generation is unavailable");
          }
        };
        requireCurrent();
        if (!target)
          throw new Error("Internal plugin RPC target is unavailable");
        const handler = target.handle.internalRpcHandlers.get(args.method);
        if (!handler)
          throw new Error(`Unknown internal RPC method ${args.method}`);
        const callerContract = readRpcMethodContract(
          args.method,
          args.contract[args.method],
        );
        const signal = AbortSignal.any([
          handle.lifecycleSignal,
          target.handle.lifecycleSignal,
          ...(args.signal ? [args.signal] : []),
        ]);
        const outcome = await invokeWrapped(
          args.pluginId,
          `internal rpc ${args.method}`,
          async () => {
            signal.throwIfAborted();
            const callerInput = await context.validateRpcValue(
              callerContract.input,
              args.input,
              "input",
            );
            const input = await context.validateRpcValue(
              handler.inputSchema,
              context.normalizeRpcJsonResult(callerInput),
              "input",
            );
            requireCurrent();
            signal.throwIfAborted();
            const result = await handler.handler(input as never, {
              callerPluginId: row.id,
              signal,
            });
            const output = await context.validateRpcValue(
              handler.outputSchema,
              result,
              "output",
            );
            const callerOutput = await context.validateRpcValue(
              callerContract.output,
              context.normalizeRpcJsonResult(output),
              "output",
            );
            requireCurrent();
            signal.throwIfAborted();
            return context.normalizeRpcJsonResult(callerOutput);
          },
        );
    ...
    try {
      ...
      if (/ERR_DLOPEN_FAILED|\.node/.test(message)) {
        message += " (native dependencies are not supported in ARC plugins)";
      }
      if (previous !== undefined) {
