import { experimental_assignedSkillSnapshotsSchema } from "@get-bb/plugin-sdk";
...
import {
  ...
  experimentalRecipientIdentitySchema,
  derivePluginId,
  formatPluginThemeId,
  isNamespacedGlyph,
...
import {
  ...
  buildPluginHost,
  createPluginDevLoop,
  createPluginSourceChangeFilter,
...
export interface PluginService {
  ...
  dispatchAddressed(
    context: import("@get-bb/plugin-sdk").ExperimentalAddressedDispatchContext,
...
export function normalizeRpcJsonResult(value: unknown): JsonValue {
...
function normalizeMentionSearchItems(
  ...
  return result.map((item, index) => {
    const typed = item as {
      ...
      subtitle?: unknown;
      icon?: unknown;
      experimental_recipient?: unknown;
    } | null;
    if (
    ...
    return {
      ...
      ...(typed.experimental_recipient === undefined
        ? {}
        : {
            experimental_recipient: experimentalRecipientIdentitySchema.parse(
              typed.experimental_recipient,
            ),
          }),
      title: typed.title,
      subtitle:
...
interface NormalizedPluginAgentConfiguration {
  assignedSkillSnapshots: import("@get-bb/plugin-sdk").ExperimentalAssignedSkillSnapshot[];
  toolIds: string[];
...
export function createPluginService(deps: PluginServiceDeps): PluginService {
  ...
  return {
      ...
      if (deps.watchBuiltinPluginSources) {
        for (const bundled of bundledPlugins) {
          ...
          const loop = createPluginDevLoop({
            pluginId: row.id,
            filterChanges: await createPluginSourceChangeFilter(
              bundled.rootDir,
            ),
            targets: async () => {
    ...
    async resolveAgentConfiguration({ context, skillIdsByPlugin }) {
      ...
      const ownerId = context.origin.pluginId;
      const turnContext = context.experimental_turnContext ?? null;
      if (
        turnContext !== null &&
        !loaded.get(turnContext.ownerPluginId)?.handle
          .agentConfigurationProvider
      )
        throw new Error(
          "Turn execution-context owner configuration is unavailable",
        );
      if (
        executionContextId !== null &&
        (ownerId === null ||
          !loaded.get(ownerId)?.handle.agentConfigurationProvider)
      ) {
        throw new Error("Execution-context owner configuration is unavailable");
      }
      ...
      const assignedSkillSnapshots: import("@get-bb/plugin-sdk").ExperimentalAssignedSkillSnapshot[] =
        [];
      ...
      for (const [pluginId, plugin] of [...loaded.entries()].sort(([a], [b]) =>
        ...
        const outcome = await invokeWrapped(pluginId, "agent configure", () =>
          normalizePluginAgentConfiguration({
            boundExecutionContext:
              (executionContextId !== null && pluginId === ownerId) ||
              turnContext?.ownerPluginId === pluginId,
            knownSkillIds,
            knownToolIds,
            pluginId,
            value: provider({
              ...context,
              experimental_turnContext:
                turnContext?.ownerPluginId === pluginId ? turnContext : null,
              thread: {
                ...context.thread,
                experimental_executionContextId:
                  pluginId === ownerId ? executionContextId : null,
              },
            }),
          }),
        );
        if (!outcome.ok) {
          if (
            ...
            throw new Error(
              `Execution-context owner ${pluginId} configuration failed: ${outcome.error}`,
            );
          }
        ...
        assignedSkillSnapshots.push(...outcome.value.assignedSkillSnapshots);
    ...
    async dispatchAddressed(context) {
      const parsed = experimentalAddressingSchema.parse({
        operationId: context.operationId,
        recipients: context.recipients,
      });
      const pluginId = parsed.recipients[0]!.pluginId;
      const lookup = wireLookup(
        pluginId,
        (plugin) => plugin.handle.addressedDispatch.handler ?? undefined,
      );
      if (lookup.outcome !== "found")
        throw new Error(`Addressed coordinator "${pluginId}" is unavailable`);
      const outcome = await invokeWrapped(pluginId, "addressed dispatch", () =>
        lookup.value({ ...context, recipients: parsed.recipients }),
      );
      if (!outcome.ok)
        throw new Error(`Addressed dispatch failed: ${outcome.error}`);
      const result = experimentalAddressedDispatchResultSchema.parse(
        outcome.value,
      );
      if (result.path && !result.path.startsWith(`/plugins/${pluginId}/`))
        throw new Error(
          "Addressed run link must belong to its coordinator plugin",
        );
      return result;
    },
