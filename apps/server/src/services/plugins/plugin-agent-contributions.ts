...
export async function dispatchPluginAddressedMessage(
  context: import("@get-bb/plugin-sdk").ExperimentalAddressedDispatchContext,
) {
  if (!contributions?.dispatchAddressed)
    throw new Error(
      "The addressed coordinator is unavailable. Enable its plugin and retry this message.",
    );
  return contributions.dispatchAddressed(context);
}
...
export async function resolvePluginAgentConfiguration(args: {
  ...
  if (!active?.resolveAgentConfiguration) {
    if (
      args.context.thread.experimental_executionContextId != null ||
      args.context.experimental_turnContext != null
    ) {
