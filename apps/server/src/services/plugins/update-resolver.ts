...
export function evaluateCompatibility(args: {
  ...
  if (!appVersion) {
    throw new Error(`cannot parse running ARC version "${args.appVersion}"`);
  }
  ...
  if (args.bbRange !== undefined) {
    if (semver.validRange(args.bbRange) === null) {
      ...
      bbProblems.push({
        ...
        required: args.bbRange,
        actual: appVersion.version,
        message: `requires ARC ${args.bbRange}, running ARC is ${appVersion.version}`,
      });
    }
  }
...
async function resolveGitRangeUpdate(args: {
  ...
  for (const candidate of candidates) {
    ...
    if (probes >= MAX_GIT_CANDIDATE_PROBES) {
      return {
        outcome: "unavailable",
        detail: `no release of ${args.url} matching ${args.intent.range} runs on this ARC version within the newest ${MAX_GIT_CANDIDATE_PROBES} releases`,
      };
    }
    probes += 1;
...
> {
  ...
  for (const candidate of candidates) {
    if (probes >= MAX_GIT_CANDIDATE_PROBES) {
      return {
        outcome: "unavailable",
        detail: `no release of ${args.url} matching ${args.range} runs on this ARC version within the newest ${MAX_GIT_CANDIDATE_PROBES} releases`,
      };
    }
    probes += 1;
  ...
  return {
    ...
    detail:
      firstProblem ??
      `no release of ${args.url} matching ${args.range} runs on this ARC version`,
  };
}
