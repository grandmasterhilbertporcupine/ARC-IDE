...
export interface PluginRpcHandler {
  ...
  handler: (
    input: never,
    context?: import("@get-bb/plugin-sdk").ExperimentalInternalRpcContext,
  ) => unknown;
}
...
export interface PluginApiHandle {
  ...
  internalRpcHandlers: Map<string, PluginRpcHandler>;
  ...
  addressedDispatch: { handler: ExperimentalAddressedDispatchHandler | null };
  activate(): void;
  ...
  return {
    ...
    threads: {
      ...
      spawn(args: ThreadSpawnArgs) {
        if (
          args.experimental_executionContextId !== undefined &&
          ((args.origin !== undefined && args.origin !== "plugin") ||
            (args.originPluginId !== undefined &&
              args.originPluginId !== pluginId))
        ) {
          throw new Error(
            "Execution-context threads must be owned by the spawning plugin",
          );
        }
        const origin = args.origin ?? "plugin";
...
export function createPluginApi(options: {
  ...
  preparedThreads?: (
    signal: AbortSignal,
  ) => import("@get-bb/plugin-sdk").ExperimentalPreparedThreads;
  preparedTurns?: (
    signal: AbortSignal,
  ) => import("@get-bb/plugin-sdk").ExperimentalPreparedTurns;
  ...
  let preparedThreadsApi:
    | import("@get-bb/plugin-sdk").ExperimentalPreparedThreads
    | undefined;
  const getPreparedThreads = () => {
    if (!options.preparedThreads)
      throw new Error("Prepared threads are unavailable");
    return (preparedThreadsApi ??= options.preparedThreads(lifecycle.signal));
  };
  let preparedTurnsApi:
    | import("@get-bb/plugin-sdk").ExperimentalPreparedTurns
    | undefined;
  const getPreparedTurns = () => {
    if (!options.preparedTurns)
      throw new Error("Prepared turns are unavailable");
    return (preparedTurnsApi ??= options.preparedTurns(lifecycle.signal));
  };
  ...
  const internalRpcHandlers = new Map<string, PluginRpcHandler>();
  ...
  function assertActive(): void {
    assertLive();
    if (!activated || lifecycle.signal.aborted) {
      throw new PluginContextStaleError(pluginId);
    }
  }
  ...
  function registerRpc(
    contract: PluginRpcContract,
    handlers: object,
    target: Map<string, PluginRpcHandler>,
  ): void {
    assertLive();
    if (
    ...
    experimental_client({ pluginId: targetId, contract }) {
      ...
      return {
        async call(method, input, callOptions) {
          assertActive();
          if (!options.callInternalRpc) {
            throw new Error("Internal plugin RPC is unavailable");
          }
          const result = await options.callInternalRpc({
            pluginId: targetId,
            contract,
            method,
            input,
          ...
          assertActive();
          return result as import("@get-bb/plugin-sdk").PluginRpcResult<
  ...
  const api: BbPluginApi = {
    ...
    experimental_turns: {
      prepare(request, callOptions) {
        assertActive();
        return getPreparedTurns().prepare(request, callOptions);
      },
      getPreparation(request, callOptions) {
        assertActive();
        return getPreparedTurns().getPreparation(request, callOptions);
      },
      startPrepared(request, callOptions) {
        assertActive();
        return getPreparedTurns().startPrepared(request, callOptions);
      },
      interrupt(request, callOptions) {
        assertActive();
        return getPreparedTurns().interrupt(request, callOptions);
      },
    },
    experimental_threads: {
      prepare(request, callOptions) {
        ...
        if (!options.preparedThreads)
          throw new Error("Prepared threads are unavailable");
        return getPreparedThreads().prepare(request, callOptions);
      },
      getPreparation(request, callOptions) {
        assertActive();
        if (!options.preparedThreads)
          throw new Error("Prepared threads are unavailable");
        return getPreparedThreads().getPreparation(request, callOptions);
      },
      startPrepared(request, callOptions) {
        assertActive();
        if (!options.preparedThreads)
          throw new Error("Prepared threads are unavailable");
        return getPreparedThreads().startPrepared(request, callOptions);
      },
    },
    realtime,
    background,
  ...
  return {
    ...
    withdraw() {
      lifecycle.abort(new PluginContextStaleError(pluginId));
      internalRpcHandlers.clear();
    },
    ...
    activate() {
      ...
      activated = true;
      if (options.preparedThreads) getPreparedThreads();
      if (options.preparedTurns) getPreparedTurns();
