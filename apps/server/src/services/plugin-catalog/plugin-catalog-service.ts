...
import { loadPluginBrandingAsset } from "../plugins/app-bundle.js";
import {
...
export function createPluginCatalogService(deps: {
  ...
  function catalogForDisplay(
    ...
    return {
      ...
      ...(marketplaceName === CURATED_MARKETPLACE_NAME
        ? { description: BUNDLED_CURATED_MARKETPLACE.description }
        : {}),
    };
  }
  ...
  function catalogOf(row: PluginMarketplaceRow): MarketplaceManifest | null {
    try {
      ...
      return catalogForDisplay(row.name, catalog);
    } catch (error) {
  ...
  function buildReservedCollectionIndex(
    ...
    for (const row of orderedMarketplaces()) {
      ...
      const catalog =
        override === undefined
          ? catalogOf(row)
          : catalogForDisplay(row.name, override);
  ...
  ): Promise<PluginCatalogIcon | null> {
    const useLogo = manifest.branding.icon === undefined;
    const path = useLogo
      ? manifest.branding.logo?.lightPath
      : manifest.branding.compactIconPath;
    if (path === undefined) return null;
    try {
      const asset = await loadPluginBrandingAsset(
        manifest.id,
        path,
        useLogo ? "logo" : "icon",
      );
      ...
      bundled === undefined
        ...
        : bundledIconAsset === null
          ...
          : {
              ...
              iconTinted: manifest?.branding.compactIconPath !== undefined,
            };
    const compatibility =
      manifest === null
        ? null
