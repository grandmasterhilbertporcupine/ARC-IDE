...
import { finishAddressedProvision } from "../threads/addressed-dispatch.js";
import {
  ...
  const addressed = deps.db
    ...
    .where(
      and(
        ...
        sql`json_valid(${threads.pendingStartContext}) AND json_extract(${threads.pendingStartContext}, '$.kind') = 'addressed-provision' AND json_extract(${threads.pendingStartContext}, '$.state') IN ('provisioning', 'routing')`,
      ),
    )
    .all();
  for (const thread of addressed) {
    try {
      await finishAddressedProvision(deps, thread.id);
    } catch (error) {
      deps.logger.warn(
        { threadId: thread.id, err: error },
        "Addressed dispatch recovery failed",
      );
    }
  }
}
