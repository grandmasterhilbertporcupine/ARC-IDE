...
import { createPreparedThreadApi } from "./services/threads/prepared-threads.js";
import { createPreparedTurnApi } from "./services/threads/prepared-turns.js";
...
export function createApp(
  ...
  const pluginService = createPluginService({
    preparedThreads: (pluginId, signal) =>
      createPreparedThreadApi(deps, { pluginId, signal }),
    preparedTurns: (pluginId, signal) =>
      createPreparedTurnApi(deps, { pluginId, signal }),
    db: deps.db,
    hub: deps.hub,
    logger: deps.logger,
  ...
  registerPreviewRoutes(publicApi, deps);
  ...
  if (!options?.staticDir) {
    app.get("/", (context) => context.text("ARC server"));
  }
