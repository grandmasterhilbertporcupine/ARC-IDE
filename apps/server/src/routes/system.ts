...
export function registerSystemRoutes(
  ...
  async function resolveSelectedTheme(
    ...
    surfaceStyle: AppTheme["surfaceStyle"],
  ): Promise<AppTheme> {
    ...
    return resolveAppTheme(themeRoot, themeId, faviconColor, surfaceStyle);
  }
  ...
  put(routes.appearance, async (context, payload) => {
    ...
    const surfaceStyle = payload.surfaceStyle ?? getStoredSurfaceStyle(deps.db);
    setStoredAppearance(deps.db, { themeId, faviconColor, surfaceStyle });
    deps.hub.notifySystem(["config-changed"]);
    return context.json(
      await resolveSelectedTheme(themeId, faviconColor, surfaceStyle),
    );
  });
