...
import { retryAddressedMessage } from "../../services/threads/addressed-dispatch.js";
...
async function compactThreadContext(
  ...
  assertPreparedTurnHistoryMutable(deps.db, thread.id);
...
export function registerThreadActionRoutes(app: Hono, deps: AppDeps): void {
  ...
  post(routes.experimental_retryAddressed, async (context, payload) => {
    const thread = requirePublicThread(deps.db, context.req.param("id"));
    const result = await retryAddressedMessage(
      deps,
      thread,
      payload.operationId,
      payload.senderThreadId,
    );
    return context.json({
      ok: true as const,
      delivery: "sent" as const,
      experimental_addressed: result,
    });
  });
