...
data_dir=${BB_DATA_DIR:-"$HOME/.arc-machines/$server_host"}
mkdir -p "$data_dir"
...
port_registry_dir="$HOME/.arc-machines/host-daemon-ports"
...
register_existing_default_ports() {
  for existing_data_dir in "$HOME/.arc-machines"/*; do
...
if [ "$package_status" = 304 ] && [ -n "$installed_artifact_digest" ]; then
  ...
  rm -rf "$package_dir"
  fail_step "Could not download the server-matched ARC host package from $package_url (HTTP $package_status). ARC cannot use an upstream bb-app package."
  exit 1
fi
rm -rf "$package_dir"
...
if [ "$platform" = darwin ]; then
  ...
  service_label="dev.arc.host-daemon.$service_slug"
...
EOF
  ...
  service_name="arc-host-daemon-$service_slug"
