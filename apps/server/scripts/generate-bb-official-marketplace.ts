...
function marketplaceIcon(
  ...
  const declared = branding.icon ?? branding.logo?.light;
  if (declared === undefined) {
    throw new Error(`bundled plugin ${pluginName} has no branding asset`);
  }
...
export async function generateBbOfficialMarketplace(args: {
  ...
  const entries = await Promise.all(
    args.plugins.map(async (plugin) => {
      ...
      const author = marketplaceEntryV2Schema.shape.author.safeParse(
      ...
      return {
        ...
        icon: marketplaceIcon(plugin.name, manifest.bb.branding),
        tags: [],
        author:
          author.success && author.data.name !== "BB"
            ? author.data
            : { name: "Community" },
  ...
  const document = {
    ...
    displayName: BUILTIN_PUBLISHER_LABEL,
    description: "Plugins included with ARC.",
