...
describe("thread runtime display", () => {
  it("projects requested models with their actual provider and null for missing retained requests", () => {
    const { db, hostId, hub } = setup();
    try {
      const threads = ["codex", "custom-provider", "claude"].map(
        (providerId) =>
      ...
      insertEvents(
        ...
        threads.slice(0, 2).map((thread) => ({
          ...
          data: JSON.stringify(
            turnRequestData({
              input: [],
              requestId: formatClientTurnRequestIdSuffix({
                suffix: "23456789ab",
              }),
            }),
          ),
      ...
      const rows = toThreadListEntryResponses(
        { db, hub, providerRegistry },
        {
          threads: threads.map((thread) =>
            createThreadListEntry({ thread, environmentHostId: hostId }),
          ),
        },
      );
      expect(rows.map((row) => row.lastRequestedModel)).toEqual([
        { providerId: "codex", model: "gpt-5" },
        { providerId: "custom-provider", model: "gpt-5" },
        null,
      ]);
    } finally {
      db.$client.close();
