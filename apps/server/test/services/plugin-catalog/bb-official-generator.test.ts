...
describe("bb-official marketplace generator", () => {
  it("generates one valid v2 entry for every bundled plugin", async () => {
    ...
    expect(catalog.displayName).toBe("Included with ARC");
    expect(catalog.categories).toEqual(PLUGIN_CATALOG_CATEGORIES);
    expect(catalog.plugins).toHaveLength(BUNDLED_PLUGINS.length);
    expect(catalog.plugins.find((entry) => entry.id === "arc")?.icon).toEqual({
      url: "./arc/assets/logo.png",
    });
    ...
    for (const plugin of BUNDLED_PLUGINS) {
      ...
      expect(entry?.author).toEqual({ name: "Community" });
    }
  });

  it("preserves declared authors and uses Community for unknown bundled authors", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "arc-catalog-authors-"));
    cleanup.push(root);
    const authors = [
      { name: "Jane Doe", github: "jane", url: "https://jane.example" },
      "Alex Example",
      "BB",
      undefined,
    ];
    const plugins = authors.map((_, index) => ({
      name: `sample-${index}`,
      pluginId: `sample-${index}`,
    }));
    ...
    const catalogFieldsPath = path.join(root, "catalog.json");
    ...
    await writeFile(
      ...
      JSON.stringify(
        Object.fromEntries(
          plugins.map((plugin) => [
            plugin.name,
            { category: "utilities", screenshots: [] },
          ]),
        ),
      ),
    );
    ...
    expect(catalog.plugins.map((plugin) => plugin.author)).toEqual([
      authors[0],
      { name: "Alex Example" },
      { name: "Community" },
      { name: "Community" },
