...
describe("plugin catalog service", () => {
  ...
  it("lists bundled plugins and the seeded official catalog", async () => {
    ...
    expect(docs).toMatchObject({
      ...
      author: { name: "Community", url: null },
      installed: false,
      compatible: true,
    });
  ...
  it("presents a stored upstream catalog as Community without changing its authors or source", async () => {
    const storedManifest = JSON.stringify({
      schemaVersion: 1,
      name: "bb-community",
      displayName: "BB Community",
      description: "Plugins reviewed by the BB team.",
      plugins: BUNDLED_CURATED_MARKETPLACE.plugins,
    });
    ...
    const catalog = service();
    expect(
      catalog.listMarketplaces().find((entry) => entry.name === "bb-community"),
    ).toMatchObject({
      displayName: "Community",
      description: "Plugins from the upstream community marketplace.",
      source: MANIFEST_URL,
    });
    const [entry] = await catalog.search("thread-hover-cards");
    ...
    expect(
      await catalog.installPlan({ entryId: "thread-hover-cards" }),
    ).toMatchObject({
      marketplace: "bb-community",
      marketplaceDisplayName: "Community",
      author: { name: "Bersabel Tadesse", github: "brsbl" },
    ...
    expect(getPluginMarketplace(db, "bb-community")?.manifestJson).toBe(
      storedManifest,
    );
  });
  ...
  it("serves ARC's logo-only branding in the catalog without tinting its artwork", async () => {
    const arc = listBundledPluginRegistrations().find(
      (plugin) => plugin.name === "arc",
    );
    if (arc === undefined) throw new Error("ARC registration missing");
    const catalog = service({ bundledPlugins: [arc] });
    const results = await catalog.search("");
    const entry = results.find((candidate) => candidate.entryId === arc.name);
    const icon = await catalog.icon("bb-official", arc.name);

    expect(entry).toMatchObject({ icon: null, iconTinted: false });
    expect(entry?.iconUrl).toBe(
      `/api/v1/plugin-catalog/icons/bb-official/arc?h=${icon?.hash}`,
    );
    expect(icon?.contentType).toBe("image/png");
    expect(icon?.bytes).toEqual(
      await readFile(join(arc.rootDir, "assets/logo.png")),
    );
  });

  describe("refresh", () => {
    it.each([1, 2])(
      "keeps Community branding after fresh and unchanged v%i catalog refreshes",
      async (schemaVersion) => {
        const entries = [remoteEntry({ icon: "Zap" })];
        const upstreamManifest =
          schemaVersion === 1 ? manifest(entries) : manifestV2(entries);
        let refreshed = false;
        const catalog = service({
          fetch: async (url) => {
            if (url !== MANIFEST_URL) {
            ...
            if (refreshed) return new Response(null, { status: 304 });
            refreshed = true;
            return jsonResponse(upstreamManifest, { etag: '"upstream"' });
          },
        });

        for (const attemptedAt of [1_000, 2_000]) {
          await catalog.refresh(attemptedAt);
          ...
          expect(getPluginMarketplace(db, "bb-community")?.manifestJson).toBe(
            JSON.stringify(upstreamManifest),
          );
        }
      },
    );
