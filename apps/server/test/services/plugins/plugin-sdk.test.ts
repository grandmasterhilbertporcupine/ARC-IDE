...
describe("plugin bb.sdk bind gate", () => {
  ...
  it("keeps internal RPC private and supplies the actual caller identity", async () => {
    ...
    const target = requireApi(service, "target");
    const caller = requireApi(service, "caller");
    const contract = defineRpcContract({
      inspect: {
        input: z.object({ value: z.number() }).strict(),
        output: z.object({ caller: z.string(), value: z.number() }).strict(),
      },
    });
    target.rpc.experimental_registerInternal(contract, {
      inspect: (input, context) => ({
        caller: context.callerPluginId,
        value: input.value,
      }),
    });
    const client = caller.rpc.experimental_client({
      pluginId: "target",
      contract,
    });
    expect(await client.call("inspect", { value: 7 })).toEqual({
      caller: "caller",
      value: 7,
    });
    expect(service.getRpcHandler("target", "inspect")).toEqual({
      outcome: "not-found",
    });
    await expect(
      client.call("inspect", { value: Number.NaN }),
    ).rejects.toThrow();
  });

  it("fences target replacement during async validation and stale caller handles", async () => {
    for (const name of ["caller", "target"])
      await service.installPath(
        await writePlugin(workDir, {
          name: `bb-plugin-${name}`,
    ...
    const contract = defineRpcContract({
      work: {
        input: z.string().superRefine(async () => {
          entered();
          await gate;
        }),
        output: z.string(),
    ...
    requireApi(service, "target").rpc.experimental_registerInternal(contract, {
      work: handler,
    });
    const staleCaller = requireApi(service, "caller").rpc.experimental_client({
      pluginId: "target",
      contract,
    });
    const pending = staleCaller.call("work", "request");
    ...
    requireApi(service, "target").onDispose(disposed);
    const reload = service.reload("target");
    await disposal;
    release();
    await reload;
    await rejected;
    expect(handler).not.toHaveBeenCalled();
    await service.reload("caller");
    await expect(staleCaller.call("work", "request")).rejects.toThrow();
  });

  it("aborts an internal handler before target disposal finishes", async () => {
    for (const name of ["caller", "target"])
      await service.installPath(
        await writePlugin(workDir, {
          name: `bb-plugin-${name}`,
    ...
    const contract = defineRpcContract({
      work: { input: z.string(), output: z.string() },
    });
    requireApi(service, "target").rpc.experimental_registerInternal(contract, {
    ...
    const client = requireApi(service, "caller").rpc.experimental_client({
      pluginId: "target",
      contract,
    });
    const pending = client.call("work", "request");
    const rejected = expect(pending).rejects.toThrow();
    await enteredPromise;
    await service.reload("target");
    await rejected;
  ...
  it("throws a descriptive error before bindSdk and resolves after", async () => {
    ...
    expect(() =>
      api.sdk.threads.spawn({
        ...
        prompt: "Bound agent",
        experimental_executionContextId: "context-1",
