...
describe("builtin plugin reconciliation", () => {
  ...
  it("gives every builtin plugin a deliberate settings icon", async () => {
    ...
    expect(BUILTIN_PLUGINS).toHaveLength(expectedIcons.size + 1);
    for (const builtin of BUILTIN_PLUGINS) {
      ...
      if (builtin.name === "arc") {
        ...
        expect(manifest.branding.logo?.lightPath).toBe(
          join(resolveBuiltinPluginRootPath("arc"), "assets", "logo.png"),
        );
      } else {
  ...
  it("hot-reloads a source-layout builtin server instead of a compatible dist artifact", async () => {
    ...
    globals.__hotBuiltinServerVersion = "still-running";
    const sourcePath = join(mutableRoot, "server.ts");
    const sourceStats = await stat(sourcePath);
    await utimes(sourcePath, new Date("2020-01-01"), sourceStats.mtime);
    await readFile(sourcePath);
    await new Promise((resolvePromise) => setTimeout(resolvePromise, 800));
    expect(globals.__hotBuiltinServerVersion).toBe("still-running");
    ...
    expect(service.list()).toMatchObject([
      {
        ...
        enabled: true,
        status: "incompatible",
        statusDetail: `server artifact for plugin "automations" was built for SDK major ${incompatibleMajor}, running SDK major is ${PLUGIN_SDK_MAJOR}; rebuild the server artifact with this ARC version`,
      },
    ]);
