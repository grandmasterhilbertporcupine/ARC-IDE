...
function localGitCachePath(repository: string): string {
  const source = parsePluginSource(`git:${repository}`);
...
describe("plugin install sources", () => {
  ...
  it("preserves local Windows git paths with stable filesystem-safe cache keys", () => {
    for (const repository of [
      "C:\\Users\\Collin Chen\\plugins\\notes.git",
      "D:/plugin work/repo.git",
      "\\\\server\\shared plugins\\notes.git",
    ]) {
      const source = parsePluginSource(`git:${repository}@main`);
      expect(source).toEqual({
        kind: "git",
        url: repository,
        spec: "main",
        selector: { kind: "ref", ref: "main" },
        cachePath: expect.stringMatching(/^local\/windows\/[a-f0-9]{32}$/u),
      });
      expect(parsePluginSource(`git:${repository}@main`)).toEqual(source);
    }
    expect(parsePluginSource("git:/tmp/plugin-repo.git@main")).toMatchObject({
      url: "/tmp/plugin-repo.git",
      cachePath: "local/tmp/plugin-repo",
    });
    expect(localGitCachePath("C:/plugin work/notes.git")).toBe(
      localGitCachePath("C:\\plugin work\\notes.git"),
    );
    expect(localGitCachePath("C:/plugin work/notes.git")).not.toBe(
      localGitCachePath("D:/plugin work/notes.git"),
    );
  });

  it("rejects local git traversal with either separator, including encoded paths", () => {
    for (const repository of [
      "C:\\plugins\\..\\outside",
      "C:/plugins/../outside",
      "\\\\server\\share\\plugins\\..\\outside",
      "C:%5Cplugins%5C..%5Coutside",
      "/tmp/plugins\\..\\outside",
    ]) {
      expect(() => parsePluginSource(`git:${repository}@main`)).toThrow(
        /invalid git repository path/u,
      );
    }
  });
...
describe("plugin install flows", () => {
  ...
  describe.skipIf(!hasGit)("git sources", { timeout: 30_000 }, () => {
    ...
    it("refuses a catalog entry whose plugin requires another plugin SDK", async () => {
      ...
      await expect(
        ...
        new RegExp(
          `install refused.*requires ARC plugin SDK >=99\\.0\\.0, running SDK is ${PLUGIN_SDK_VERSION.replaceAll(".", "\\.")}`,
          "u",
        ),
      );
    ...
    it("hard-fails install on an engines.bb mismatch and cleans up the clone", async () => {
      ...
      await expect(
        service.install(source, { kind: "root" }),
      ).rejects.toThrowError(/install refused.*requires ARC >=99\.0\.0/);
    ...
    it("refuses a cached artifact whose engine range no longer matches", async () => {
      ...
      await expect(
        service.install(source, { kind: "root" }),
      ).rejects.toThrowError(/install refused.*requires ARC >=0\.9\.0/u);
  ...
  it("keeps path installs developer-friendly while surfacing SDK incompatibility", async () => {
    ...
    expect(entry.statusDetail).toContain(
      `requires ARC plugin SDK >=99.0.0, running SDK is ${PLUGIN_SDK_VERSION}`,
    );
  });
