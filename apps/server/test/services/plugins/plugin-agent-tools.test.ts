...
describe("bb.agents.registerTool", () => {
  ...
  it("binds only the admitted turn owner without replacing the conversation origin", async () => {
    const owner = await writePlugin(workDir, {
      name: "bb-plugin-turn-bound",
      serverSource: `export default function plugin(bb) { bb.agents.configure(ctx=>({tools:[],skills:[],instructions:JSON.stringify({turn:ctx.experimental_turnContext,origin:ctx.origin,permanent:ctx.thread.experimental_executionContextId})})); }`,
    });
    const observer = await writePlugin(workDir, {
      name: "bb-plugin-turn-observer",
      serverSource: `export default function plugin(bb) { bb.agents.configure(ctx=>({tools:[],skills:[],instructions:JSON.stringify({turn:ctx.experimental_turnContext,origin:ctx.origin})})); }`,
    });
    await service.installPath(owner);
    await service.installPath(observer);
    const identity = {
      ownerPluginId: "turn-bound",
      operationId: "run:main",
      executionContextId: "sealed-turn",
      clientTurnRequestId: encodeClientTurnRequestIdNumber({ value: 43 }),
    };
    const context = makePluginAgentConfigurationContext({
      experimental_turnContext: identity,
      origin: { kind: null, pluginId: null },
    });
    const result = await service.resolveAgentConfiguration({
      context,
      skillIdsByPlugin: new Map(),
    });
    expect(result.dynamicInstructions).toEqual([
      {
        pluginId: "turn-bound",
        text: JSON.stringify({
          turn: identity,
    ...
    await service.setEnabled("turn-bound", false);
    await expect(
      service.resolveAgentConfiguration({
        context,
        skillIdsByPlugin: new Map(),
      }),
    ).rejects.toThrow("owner configuration is unavailable");
  });

  it("keeps bound instructions intact, hides context from other plugins, and fails when the owner is disabled", async () => {
    const owner = await writePlugin(workDir, {
      name: "bb-plugin-bound",
      serverSource: `export default function plugin(bb: any) {
        bb.agents.configure((ctx: any) => {
          if (ctx.thread.experimental_executionContextId !== "snapshot-1") throw new Error("Unknown snapshot");
          return { tools: [], skills: [], instructions: "x".repeat(65536) };
        });
      }`,
    });
    const observer = await writePlugin(workDir, {
      ...
      serverSource: `export default function plugin(bb: any) {
        bb.agents.configure((ctx: any) => ({ tools: [], skills: [], instructions:
          ctx.thread.experimental_executionContextId === null ? "y".repeat(5000) : "LEAKED" }));
      }`,
    });
    expect((await service.installPath(owner)).status).toBe("running");
    expect((await service.installPath(observer)).status).toBe("running");
    const context = makePluginAgentConfigurationContext({
      origin: { pluginId: "bound" },
      thread: { experimental_executionContextId: "snapshot-1" },
    });
    const result = await service.resolveAgentConfiguration({
      context,
      skillIdsByPlugin: new Map(),
    ...
    expect(result.dynamicInstructions).toEqual([
      { pluginId: "bound", text: "x".repeat(65_536) },
      { pluginId: "observer", text: "y".repeat(4_096) },
    ]);
    await service.setEnabled("bound", false);
    await expect(
      service.resolveAgentConfiguration({
        context,
        skillIdsByPlugin: new Map(),
  ...
  it("fails bound dispatch for absent hooks, invalid snapshots, and oversized instructions", async () => {
    const context = makePluginAgentConfigurationContext({
      origin: { pluginId: "bound" },
      thread: { experimental_executionContextId: "snapshot-1" },
    });
    ...
    await expect(resolve()).rejects.toThrow("configuration is unavailable");
    const owner = await writePlugin(workDir, {
      name: "bb-plugin-bound",
    ...
    await expect(resolve()).rejects.toThrow("configuration is unavailable");
    await writeFile(
      join(owner, "server.ts"),
      `export default function plugin(bb: any) {
      bb.agents.configure(() => { throw new Error("Unknown snapshot"); });
    }`,
    );
    await service.reload("bound");
    await expect(resolve()).rejects.toThrow("Unknown snapshot");
    await writeFile(
      join(owner, "server.ts"),
      `export default function plugin(bb: any) {
      bb.agents.configure(() => ({ tools: [], skills: [], instructions: "x".repeat(65537) }));
    }`,
    );
    await service.reload("bound");
    await expect(resolve()).rejects.toThrow("65536-character limit");
  });
...
describe("plugin tools reach thread runtime config", () => {
  ...
  it("binds immutable context before the first creation dispatch and retains full owner instructions", async () => {
    const { host } = seedHostSession(harness.deps, {
      id: "host-bound-creation",
    });
    ...
    const environment = seedEnvironment(harness.deps, {
      ...
      path: join(harness.config.dataDir, "bound-workspace"),
    });
    const rootDir = await writePlugin(pluginsDir, {
      name: "bb-plugin-bound",
      serverSource: `export default function plugin(bb: any) {
        bb.agents.configure((ctx: any) => {
          if (ctx.thread.experimental_executionContextId !== "snapshot-first" || ctx.project.id !== ${JSON.stringify(project.id)}) throw new Error("Wrong snapshot binding");
          if (!ctx.thread.id) throw new Error("Missing thread identity");
          return { tools: [], skills: [], instructions: "FIRST_IMMUTABLE_DEFINITION:" + "x".repeat(5000) };
        });
      }`,
    });
    ...
    const thread = await createThreadFromRequest(harness.deps, {
      ...
      origin: "plugin",
      originPluginId: "bound",
      experimental_executionContextId: "snapshot-first",
      input: textInput("Build the requested app"),
      projectId: project.id,
      providerId: "codex",
      startedOnBehalfOf: null,
    });
    expect(thread.experimental_executionContextId).toBe("snapshot-first");
    const first = await waitForQueuedCommand(
      harness,
      (queued) =>
        queued.command.type === "thread.start" &&
        queued.command.threadId === thread.id,
    );
    if (first.command.type !== "thread.start")
      throw new Error("Expected first thread.start");
    expect(first.command.instructions).toContain(
      "FIRST_IMMUTABLE_DEFINITION:" + "x".repeat(5000),
    );
    const execution = await resolveExecutionOptions(harness.deps, {
      threadId: thread.id,
      requestedExecution: { model: "gpt-5.6", source: "client/turn/requested" },
    });
    ...
    await harness.pluginService.setEnabled("bound", false);
    await expect(
  ...
  it("thread.start dynamicTools include plugin tools with per-tool instructions", async () => {
    ...
    expect(command.instructions).toContain(
      'The following instructions come from the ARC plugin "tooldemo" for its tool "demo_lookup":',
    );
    expect(command.instructions).toContain(
  ...
  it("resolves different conditional tools, skills, instructions, and context without rebuilding static registrations", async () => {
    ...
    expect(sideCommand.instructions).toContain(
      'The following dynamic instructions come from the ARC plugin "conditional":',
    );
    expect(
    ...
    await withTestHarness(async (harness) => {
      ...
      try {
        ...
        seedEvent(harness.deps, {
          ...
          scope: turnScope("turn-plugin-tool"),
          data: {
            clientRequestId: encodeClientTurnRequestIdNumber({ value: 42 }),
          },
        });
