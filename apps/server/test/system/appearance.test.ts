...
describe("appearance settings", () => {
  it("persists Liquid Glass and preserves it across legacy palette and favicon updates", async () => {
    await withTestHarness(async (harness) => {
      const update = (selection: Record<string, string>) =>
        harness.app.request("/api/v1/settings/appearance", {
          method: "PUT",
      ...
      expect(appThemeSchema.parse(await readJson(enabled))).toEqual(
        appearanceForPalette("nord", {
          faviconColor: "teal",
          surfaceStyle: "liquid-glass",
        }),
      );
      expect(getStoredSurfaceStyle(harness.db)).toBe("liquid-glass");
      const legacy = await update({ themeId: "dracula", faviconColor: "pink" });
      expect(legacy.status).toBe(200);
      const expected = appearanceForPalette("dracula", {
        faviconColor: "pink",
        surfaceStyle: "liquid-glass",
      });
      expect(appThemeSchema.parse(await readJson(legacy))).toEqual(expected);
      const config = systemConfigResponseSchema.parse(
        await readJson(await harness.app.request("/api/v1/system/config")),
      );
      expect(config.appearance).toEqual(expected);
      const catalog = themeCatalogResponseSchema.parse(
        await readJson(await harness.app.request("/api/v1/settings/themes")),
      );
      expect(catalog.active).toEqual(expected);
      ...
      expect(appThemeSchema.parse(await readJson(reset))).toEqual(
        appearanceForPalette("dracula", { faviconColor: "pink" }),
      );
    });
  });

  it.each(["frosted", null, true])(
    "rejects invalid surface style %s without writing",
    async (surfaceStyle) => {
      await withTestHarness(async (harness) => {
        ...
        expect(getStoredSurfaceStyle(harness.db)).toBe("default");
