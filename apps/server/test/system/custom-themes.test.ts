...
describe("custom themes service", () => {
  ...
  it("resolves a built-in id without reading disk", () => {
    expect(resolveAppTheme(themeRoot, "nord", "blue", "default")).toEqual({
      ...defaultAppTheme,
      themeId: "nord",
      faviconColor: "blue",
  ...
  it("resolves a custom theme's CSS from disk", async () => {
    ...
    expect(resolveAppTheme(themeRoot, "ocean", "teal", "liquid-glass")).toEqual(
      {
        ...defaultAppTheme,
        themeId: "ocean",
  ...
  it("falls back to default palette but keeps the favicon tint for a missing or unsafe selection", () => {
    expect(
      resolveAppTheme(themeRoot, "missing", "pink", "liquid-glass"),
    ).toEqual({
      ...defaultAppTheme,
      faviconColor: "pink",
      surfaceStyle: "liquid-glass",
    ...
    expect(
      resolveAppTheme(themeRoot, "../escape", "pink", "liquid-glass"),
    ).toEqual({
      ...defaultAppTheme,
      faviconColor: "pink",
      surfaceStyle: "liquid-glass",
  ...
  it("rejects oversized stylesheets so the broadcast payload stays bounded", async () => {
    ...
    expect(resolveAppTheme(themeRoot, "huge", "default", "default")).toEqual(
      defaultAppTheme,
    );
  });
  ...
  it("loads convention Pierre JSON files next to theme.css", async () => {
    ...
    expect(resolveAppTheme(themeRoot, "ocean", "default", "default")).toEqual({
      ...defaultAppTheme,
      themeId: "ocean",
      customCss: ":root {}",
      resolvedCodeTheme: {
