...
describe("injected skill source discovery", () => {
  it("propagates snapshot admission failure instead of omitting a required skill", async () => {
    const temp = await makeTempDir();
    const skillRoot = path.join(temp, "builtin");
    await writeSkill({ rootPath: skillRoot, name: "required" });
    ...
    expect(() =>
      resolveInjectedSkillSourcesWithRegistry(logger, {
        builtinSkillsRootPath: skillRoot,
        dataDir: temp,
        skillTreeRegistry,
      }),
    ).toThrow("Skill tree registry capacity exhausted");
    expect(() =>
      discoverPluginSkillIds(logger, {
        pluginSkillRoots: [{ pluginId: "test", rootPath: skillRoot }],
        skillTreeRegistry,
      }),
    ).toThrow("Skill tree registry capacity exhausted");
  ...
  it("hashes trees deterministically and includes content, paths, and modes", async () => {
    ...
    await chmod(secondReference, 0o444);
    expect(readSkillTreeManifest(secondRoot).treeHash).not.toBe(baseline);
  });
