...
describe("project-default thread environment", () => {
  it.each([
    "/tmp/project-default-source",
    String.raw`B:\Collin's projects\Native project Δ with spaces`,
  ])(
    "resolves project-default exactly like the explicit managed-worktree default for %s",
    async (sourcePath) => {
  ...
  it.each([
    ...
    async ({ checkout }) => {
      await withTestHarness(async (harness) => {
        ...
        seedPrimaryHost(harness.deps, host.id);
        const sourcePath = String.raw`B:\Collin's projects\Native project Δ with spaces`;
        const { project } = seedProjectWithSource(harness.deps, {
          hostId: host.id,
          path: sourcePath,
        });
