...
describe("public provider installation routes", () => {
  ...
  it.each([
    { actionKind: "install", success: true },
    { actionKind: "update", success: true },
    { actionKind: "update", success: false },
  ] as const)(
    "refreshes cached models after $actionKind only when success is $success",
    async ({ actionKind, success }) => {
      await withTestHarness(async (harness) => {
        const { host, session } = seedHostSession(harness.deps, {
          id: "provider-installation-model-refresh-host",
        });
        let modelIds = ["gpt-5.5"];
        const responder = registerHostRpcResponder(harness, {
          hostId: host.id,
          sessionId: session.id,
          handle: (request) => {
        ...
        const listModelIds = async () => {
          const response = await harness.app.request(
            `${API}/system/execution-options?hostId=${host.id}&providerId=codex`,
          );
          expect(response.status).toBe(200);
          const options = systemExecutionOptionsResponseSchema.parse(
            await readJson(response),
          );
          expect(options.modelLoadError).toBeNull();
          return options.models.map((model) => model.model);
        };
        const modelRequests = () =>
        ...
        expect(await listModelIds()).toEqual(["gpt-5.5"]);
        expect(await listModelIds()).toEqual(["gpt-5.5"]);
        ...
        const response = await harness.app.request(
          `${API}/hosts/${host.id}/provider-clis/install`,
          {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({ provider: "codex", actionKind }),
          },
        );
        expect(response.status).toBe(200);
        expect(await response.text()).toContain(`"success":${success}`);
        expect(await listModelIds()).toEqual(modelIds);
        expect(await listModelIds()).toEqual(modelIds);
        expect(modelRequests()).toHaveLength(success ? 2 : 1);
      });
    },
  );
