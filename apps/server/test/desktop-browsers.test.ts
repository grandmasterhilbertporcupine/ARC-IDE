...
describe("desktop browser public API", () => {
  ...
  it("retains HTML source through native navigation and reconnect without resurrecting an explicitly removed source", async () => {
    await withBrowserTest(async (test, harness) => {
      const htmlSource = {
        hostId: test.scope.hostId,
        rootPath: "C:\\Preview site",
        filePath: "pages/index.html",
      };
      const native = {
        ...test.tab(),
        url: "http://localhost/api/v1/file-previews/renewed/pages/index.html",
        title: "Live HTML",
      };
      replaceStoredThreadTabs(harness.db, {
        threadId: test.scope.threadId,
        expectedRevision: 0,
        tabsJson: JSON.stringify([
          {
            ...
            environmentId: null,
            title: "Saved HTML",
            url: "http://localhost/api/v1/file-previews/expired/pages/index.html",
            htmlSource,
          },
        ]),
      });
      ...
      test.change({ generation: "renewed-window" }, [native]);
      expect(test.stored()).toEqual([
        expect.objectContaining({
          htmlSource,
          desktopTarget: expect.objectContaining({
            generation: "renewed-window",
          }),
        }),
      ]);
      const response = await harness.app.request(
        `/api/v1/threads/${test.scope.threadId}/tabs`,
        {
          method: "PUT",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({
            expectedRevision: getStoredThreadTabs(
              harness.db,
              test.scope.threadId,
            )!.revision,
            tabs: [
      ...
      expect(response.status).toBe(200);
      test.change({ generation: "renewed-window" }, [
        { ...native, url: "https://example.com/", title: "External page" },
      ]);
      expect(test.stored()).toEqual([
        expect.objectContaining({ url: "https://example.com/" }),
      ]);
      expect(test.stored()[0]).not.toHaveProperty("htmlSource");
    });
  });

  it("keeps renderer-owned HTML source when native create completes after its tab was persisted", async () => {
    await withBrowserTest(async (test, harness) => {
      const htmlSource = {
        hostId: test.scope.hostId,
        rootPath: "C:\\Preview site",
      ...
      test.intercept((request) => {
        if (request.command.type !== "desktop.browser.create_tab") return null;
        const tab = {
          ...test.tab(),
          tabId: request.command.tabId,
          url: request.command.url,
        };
        replaceStoredThreadTabs(harness.db, {
          threadId: test.scope.threadId,
          expectedRevision: 0,
      ...
      const response = await test.post("create", {
        ...test.scope,
        url: "http://localhost/api/v1/file-previews/live/index.html",
      });
      expect(response.status).toBe(200);
      expect(test.stored()).toEqual([
        expect.objectContaining({
