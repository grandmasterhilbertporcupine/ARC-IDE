...
describe("machine install script", () => {
  ...
  it("uses the server artifact and passes the launcher join flags verbatim", () => {
    const fixture = createFixture();
  ...
  it.each([404, 503])(
    "refuses upstream and PATH fallback when the ARC artifact returns %s",
    (status) => {
      const fixture = createFixture();
      writeServerInstallTools(fixture, 200);
      writeCurlArtifactMock(fixture, status);
      const invocationPath = join(fixture.dataDir, "foreign-invocation");
      writeExecutable(
        join(fixture.binDir, "bb-app"),
        '#!/bin/sh\nprintf called >"' + invocationPath + '"\n',
      );
      const result = runScript(JOIN_ARGS, fixture, {
        BB_INSTALL_SKIP_SERVICE: "1",
      });
      ...
      expect(result.stderr).toContain(
        "ARC cannot use an upstream bb-app package",
      );
      expect(existsSync(join(fixture.dataDir, "npm.log"))).toBe(false);
  ...
  it("defaults the data dir to a per-server directory under ~/.arc-machines", () => {
    const fixture = createFixture();
    writeServerInstallTools(fixture, 200);
    const result = runScript(JOIN_ARGS, fixture, {
      BB_DATA_DIR: "",
    ...
    const defaultDataDir = join(
      fixture.homeDir,
      ".arc-machines/machine.getbb.app",
    );
    expect(
  ...
  it("atomically reserves different ports for concurrent custom data directories", async () => {
    ...
    const registryDir = join(
      fixture.homeDir,
      ".arc-machines/host-daemon-ports",
    );
    expect(
      new Set([
...
`,
    ...
    expect(secondResult.stdout).toContain(
      "service " +
        join(
          fixture.homeDir,
          "Library/LaunchAgents/dev.arc.host-daemon.machine-getbb-app.plist",
        ),
    );
    ...
    expect(plist).toContain(
      "<string>dev.arc.host-daemon.machine-getbb-app</string>",
    );
...
`,
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain(
      "Could not register the bb host-daemon launch agent dev.arc.host-daemon.machine-getbb-app.",
    );
...
if [ "$*" = "--user restart arc-host-daemon-machine-getbb-app.service" ]; then
  port=$(sed -n '1p' "${join(fixture.dataDir, "host-daemon-port")}")
  BB_DATA_DIR="${fixture.dataDir}" "${join(fixture.dataDir, "npm/bin/bb-app")}" host-daemon --host-daemon-port "$port" --server-url https://machine.getbb.app >/dev/null 2>&1 &
  echo $! >"${join(fixture.dataDir, "service-daemon.pid")}"
fi
`,
    ...
    const unit = readFileSync(
      join(
        fixture.homeDir,
        ".config/systemd/user/arc-host-daemon-machine-getbb-app.service",
      ),
      "utf8",
    );
    const selectedPort = readFileSync(
    ...
    expect(readFileSync(join(fixture.dataDir, "systemctl.log"), "utf8")).toBe(
      "--user daemon-reload\n--user enable arc-host-daemon-machine-getbb-app.service\n--user restart arc-host-daemon-machine-getbb-app.service\n",
    );
  });
});
