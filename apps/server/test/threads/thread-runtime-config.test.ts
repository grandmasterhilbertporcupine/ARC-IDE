  ...
  it("resolves the workspace, storage path, and environment directory dynamic tool", async () => {
    await withTestHarness(async (harness) => {
      ...
      expect(runtimeConfig.instructions).toContain(
        "You are working inside ARC, an agentic IDE",
      );
  ...
  describe("plugin contributeInstructions assembly", () => {
    ...
    it("appends attributed plugin instructions after tool snippets and before data-dir instructions", async () => {
      await withTestHarness(async (harness) => {
        ...
        const pluginHeader =
          'The following instructions come from the ARC plugin "connect":';
        const dataDirHeader =
          "The following user instructions come from <dataDir>/AGENTS.md:";
    ...
    it("skips null/empty providers, truncates long text, and isolates throws", async () => {
      await withTestHarness(async (harness) => {
        ...
        expect(instructions).not.toContain(
          'The following instructions come from the ARC plugin "nuller":',
        );
        expect(instructions).not.toContain(
          'The following instructions come from the ARC plugin "blank":',
        );
        expect(instructions).not.toContain(
          'The following instructions come from the ARC plugin "boom":',
        );
        expect(instructions).toContain(
          'The following instructions come from the ARC plugin "verbose":',
        );
        expect(instructions).toContain(
          'The following instructions come from the ARC plugin "ok":',
        );
