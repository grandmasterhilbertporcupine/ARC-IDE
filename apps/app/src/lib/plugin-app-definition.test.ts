...
describe("thread companion and batched annotations registrations", () => {
  it("uses the same canonical registrations in the app and plugin harness", async () => {
    ...
    expect(collected.threadViews).toEqual([
      { id: "team", label: "Team", component: Component },
    ]);
    expect(collected.threadListAnnotations).toEqual([
      { id: "bindings", component: Component },
    ]);
    expect(await loadPluginApp(definition)).toMatchObject({
      threadViews: collected.threadViews,
      threadListAnnotations: collected.threadListAnnotations,
    });
  });
  it("rejects duplicate view ids, empty labels and malformed annotation components", () => {
    expect(() =>
      collectPluginAppRegistrations(
        definePluginApp((app) => {
          app.slots.experimental_threadView({
