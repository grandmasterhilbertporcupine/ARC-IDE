...
import { liquidGlassCss } from "./liquid-glass";
...
export function resolveAppThemeCss(appearance: AppTheme): string {
  const paletteCss = isBuiltInThemeId(appearance.themeId)
    ? builtInThemeCss[appearance.themeId]
    : (appearance.customCss ?? "");
  return appearance.surfaceStyle === "liquid-glass"
    ? `${paletteCss}\n${liquidGlassCss}`
    : paletteCss;
}
