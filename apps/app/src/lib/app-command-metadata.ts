  ...
  },
  {
    ...
    commands: [
      paletteHiddenCommand(
        "palette.open",
        "Open quick palette",
        "Search and run ARC commands from the keyboard.",
      ),
      command("window.new", "New window", "Open another ARC desktop window."),
      command(
        "app.back",
        "Back to app",
        "Return from Settings or Extensions to the app.",
      ),
      command("settings.open", "Open settings", "Open ARC settings."),
      command(
        "settings.openServers",
        "Open server settings",
        "Open settings to add or manage ARC servers.",
      ),
      command(
        "sidebar.toggle",
        "Toggle sidebar",
      ...
      command(
        ...
        "Open the desktop log viewer for the ARC server and host daemon.",
      ),
      command(
        "notifications.open",
  ...
  },
  {
    ...
    commands: QUESTION_SELECT_APP_COMMAND_IDS.map((id, index) =>
      paletteHiddenCommand(
        ...
        `Choose visible answer ${index + 1} when ARC asks a question.`,
      ),
    ),
  },
];
