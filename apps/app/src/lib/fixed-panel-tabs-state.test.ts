...
describe("fixed-panel-tabs-state", () => {
  it("persists scoped HTML source identities and rejects traversal before opening a browser", () => {
    const htmlSource = {
      hostId: "host",
      rootPath: "C:\\Site",
      filePath: "pages/index.htm",
    };
    const tab = createBrowserFixedPanelTab({
      environmentId: null,
      url: "",
      htmlSource,
    });
    ...
    expect(
      areFixedPanelTabsEquivalent(tab, {
        ...tab,
        htmlSource: { ...htmlSource, hostId: "other" },
      }),
    ).toBe(false);
    for (const filePath of [
      ...
      expect(
        threadTabsSchema.safeParse([
          { ...tab, htmlSource: { ...htmlSource, filePath } },
        ]).success,
      ).toBe(false);
    }
  });
