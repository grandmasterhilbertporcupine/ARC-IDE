...
const GITHUB_PLUGIN = makePluginListItem({
  ...
  sourceDisplay: "Included with ARC · GitHub",
});
...
describe("PluginDetail official catalog lifecycle", () => {
  it("offers Install from an unowned Included with ARC plugin detail page", () => {
    ...
    expect(screen.getByText("Included with ARC")).toBeTruthy();
  ...
  it("explains why an incompatible official plugin cannot be installed", () => {
    ...
    const compatibilityStatus = screen
      .getByText("Update ARC to install this plugin")
    ...
    expect(compatibilityStatus.textContent).toContain(
      "Update ARC to install this plugin",
    );
    expect(compatibilityStatus.textContent).toContain(
      "Requires ARC 0.20 or newer.",
    );
    expect(
      screen
  ...
  it("keeps catalog provenance and release management in the unified detail taxonomy", async () => {
    ...
    expect(screen.getAllByText("Included with ARC").length).toBeGreaterThan(0);
    expect(screen.getByText("Developer tools")).toBeTruthy();
    expect(
    ...
    expect(screen.getByText("Updates with ARC")).toBeTruthy();
  ...
  it("never describes a managed plugin with an unknown install date as bundled", () => {
    ...
    expect(screen.queryByText("Updates with ARC")).toBeNull();
  });
  ...
  it.each([
    ...
    },
    {
      ...
      updateState: {
        ...
        blockedReasons: ["Requires ARC 0.20 or newer."],
      },
  ...
  it("shows a disabled Uninstall action for a built-in plugin", async () => {
    ...
    expect(
      await screen.findAllByText(
        "Included with ARC; disable this plugin instead.",
      ),
    ).not.toHaveLength(0);
  });
});

describe("Included with ARC plugin detail routing", () => {
...
describe("plugin removal confirmation", () => {
  it("warns that removing a local plugin deletes its settings, secrets, and schedules and names the move path", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("menuitem", { name: "Remove from ARC" }),
    );

    expect(
      await screen.findByRole("heading", { name: "Remove plugin from ARC?" }),
    ).toBeTruthy();
...
describe("PluginDetail banner precedence", () => {
  ...
  const collision: PluginListItem = {
    ...
    updateState: {
      ...
      availableVersion: "1.5.0",
      blockedVersion: "2.0.0",
      blockedReasons: ["Requires a newer ARC version."],
      lastFailure: {
        version: "1.4.5",
        at: null,
...
describe("PluginDetail runtime health", () => {
  ...
  it.each([
    [
      "incompatible",
      "This plugin version isn't compatible with your version of ARC.",
      "Update ARC to load a compatible bundled plugin.",
    ],
    [
      "missing",
      "The plugin's files are missing.",
      "Restart ARC. If the files are still missing, reinstall ARC.",
    ],
  ] as const)(
  ...
  it("keeps needs-configuration actionless because saving Settings retries it", () => {
    ...
    expect(alert.textContent).toContain(
      "Complete the Configuration section; ARC reloads the plugin after you save.",
    );
    expect(screen.queryByRole("button", { name: "Reload" })).toBeNull();
  });
