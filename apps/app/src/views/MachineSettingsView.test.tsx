...
const hostDaemon = vi.hoisted(() => ({
  ...
  platform: null as HostPlatform | null,
}));
...
describe("MachineSettingsView", () => {
  it.each([
    ...
    { platform: "wsl" as const, label: "WSL" },
  ])(
    "identifies the local $label host in its machine details",
    async ({ platform, label }) => {
      hostDaemon.localDaemonHostId = HOST_ID;
      hostDaemon.platform = platform;
      ...
      expect(screen.getByText(/^Online ·/u).textContent).toContain(label);
    },
  );
  ...
  it("refuses to remove the primary machine", async () => {
    ...
    expect(
      screen.getByText("ARC's primary machine can't be removed."),
    ).toBeDefined();
  });
