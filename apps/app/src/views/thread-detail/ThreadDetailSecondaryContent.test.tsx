...
function createProps(
  ...
  return {
    mainElementId: "main-chat-fixture",
...
describe("ThreadDetailSecondaryContent", () => {
  it("keeps the same main chat, unsent composer state and scroll across companion switches", () => {
    const mounted = vi.fn();
        ...
        <DefaultPaneContextProvider>
          <CompactViewportOverrideProvider isCompactViewport={false}>
            <ThreadDetailSecondaryContent
              ...
              footer={<Composer />}
              companion={show ? <div>Team content</div> : null}
            />
          </CompactViewportOverrideProvider>
        </DefaultPaneContextProvider>
      </MemoryRouter>
    ...
    const composer = screen.getByRole("textbox", { name: "Unsent message" });
    const timeline = screen.getByTestId("thread-timeline-pane");
    const main = document.getElementById(props.mainElementId);
    expect(main?.contains(timeline)).toBe(true);
    expect(main?.contains(composer)).toBe(true);
    timeline.scrollTop = 137;
    fireEvent.change(composer, { target: { value: "Keep this unsent" } });
    for (let index = 0; index < 6; index++) {
      view.rerender(tree(index % 2 === 0));
      expect(document.getElementById(props.mainElementId)).toBe(main);
      expect(screen.getByTestId("thread-timeline-pane")).toBe(timeline);
      expect(timeline.scrollTop).toBe(137);
      expect(screen.getByRole("textbox", { name: "Unsent message" })).toBe(
        composer,
      );
      expect(composer).toHaveProperty("value", "Keep this unsent");
    }
