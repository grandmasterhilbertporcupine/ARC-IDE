...
import { useThreadView } from "@/components/plugin/PluginThreadView";
...
function ThreadDetailViewInternal(props: ThreadRoutePathArgs) {
  ...
  const threadView = useThreadView({
    ...
    isRootThread: thread?.parentThreadId === null,
  });
  ...
  usePreviewFeedbackDraft({
    threadId: thread?.id,
    projectId,
    append: handleSelectionAddToChat,
  });
  ...
  const timelineHeader = (
    <ThreadDetailHeader
      ...
      viewSwitch={threadView.controls}
  ...
  const renderSecondaryTabContent = (
    ...
    switch (tab.kind) {
      ...
      case "workspace-file-preview": {
        ...
        return (
          <LazyWorkspaceFilePreviewTabContent
            onOpenHtmlPreview={(source) => openBrowserTabAndReveal("", source)}
            activePath={tab.path}
            copyPath={copyPath}
      ...
      case "thread-storage-file-preview": {
        ...
        return (
          <LazyThreadStorageFilePreviewTabContent
            htmlSource={
              environment?.hostId && threadStorageRootPath
                ? {
      ...
      >
        <AppNavigationHostProvider capabilities={appNavigationCapabilities}>
          <ThreadDetailSecondaryContent
            mainElementId={threadView.mainElementId}
            footer={composerFooter}
            companion={threadView.companion}
