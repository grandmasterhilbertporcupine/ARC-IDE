...
import { useAddressedRecipients } from "@/hooks/useAddressedRecipients";
...
export function ThreadDetailPromptArea({
  ...
  const addressed = useAddressedRecipients({
    projectId,
    threadId: thread.id,
    mentions: promptDraft.mentions,
    removeMention: (recipient) =>
      promptDraft.setTextAndMentions(
        promptDraft.text,
        promptDraft.mentions.filter(
          ({ resource }) =>
            !(
              resource.kind === "plugin" &&
              resource.pluginId === recipient.pluginId &&
              resource.experimental_recipient?.entityId ===
                recipient.entityId &&
              resource.experimental_recipient.kind === recipient.kind
            ),
        ),
      ),
  });
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      const addressing = await addressed.forSend(submittedInput);
      if (!addressing && !isQueuingMessage && isDefaultExecutionOptionsLoading)
        return;
      ...
      if (addressing) {
        ...
        addressed.acknowledge(addressing.operationId);
        if (result.delivery === "sent" && result.experimental_addressed)
          appToast.success(result.experimental_addressed.summary);
  ...
  const submitScheduled = useCallback(
    async ({ sendAt }: { sendAt: number }) => {
      if (addressed.recipients.length > 0)
        throw new Error(
          "Addressed work must be sent now. Remove recipients to schedule an ordinary message.",
        );
