...
interface AppearanceSettingsSectionProps {
  ...
  onSurfaceStyleChange: (surfaceStyle: AppSurfaceStyle) => void;
...
const CREATE_CUSTOM_PALETTE_PROMPT =
  "Create a custom ARC palette. First run `bb theme dir` to find the custom theme directory. Ask me for the palette name and visual direction, then create `<theme-dir>/<name>/theme.css` with light and dark theme variables compatible with ARC's theme tokens.";
const PALETTE_SETTING_DESCRIPTION =
  "Palettes change ARC's colors, including syntax colors in diffs and file previews. Choose a built-in palette or create one from a prompt.";
...
export function LocalOpenTargetSettingsSection({
  ...
  if (!hasDaemon) {
    ...
    const descriptionText = accessDenied
      ? "Your browser blocked access to ARC on this device. Allow local network access for this site in browser settings, then reload ARC."
      : accessAvailable
        ? "ARC couldn’t connect to its local editor helper. Make sure the ARC desktop app or bb CLI is running on this device, then retry. If it is already running, a remote browser origin may need to be configured."
        : "Connect this browser to ARC on this device so it can discover installed editors. ARC only contacts the local helper after you choose Enable; your browser may ask for local network access.";
...
function ManagedBranchPrefixSetting({
  ...
  return (
    <SettingsWithControl
      ...
      description={
        valid ? (
          `ARC puts this in front of every branch it creates for a worktree, such as ${draft}${MANAGED_BRANCH_PREFIX_EXAMPLE_SLUG}. Leave it empty for no prefix.`
        ) : (
  ...
  return (
    <SettingsSection title="Appearance">
      <div className="space-y-5">
        ...
        <SettingsWithControl
          label="Liquid Glass"
          description="Translucent panels, soft blur, and subtle highlights."
        >
          <Switch
            aria-label="Liquid Glass"
            checked={appearance.surfaceStyle === "liquid-glass"}
            disabled={appearanceDisabled}
            onCheckedChange={(enabled) =>
              onSurfaceStyleChange(enabled ? "liquid-glass" : "default")
            }
          />
        </SettingsWithControl>
      ...
      <div className="space-y-5">
        ...
        {desktopBrowserAvailable ? (
          <SettingsWithControl
            ...
            description="Open web links inside ARC."
          >
            <Switch
  ...
  return (
    <SettingsSection title="Debug">
      <SettingsWithControl
        ...
        description="Show raw provider events ARC does not recognize. Development builds always show these events."
      >
        <Switch
          checked={enabled}
          disabled={disabled}
    ...
    >
      <div className="space-y-5">
        ...
        <SettingsWithControl
          ...
          description="Pair a companion mobile app through bb connect: shows Add mobile device under Remote access and enables bb connect machine-code."
        >
          <Switch
  ...
  if (activePluginId !== null) {
    ...
    content = (
      <AppearanceSettingsSection
        ...
        onSurfaceStyleChange={(surfaceStyle) =>
          updateAppearanceMutation.mutate({
