...
describe("SkillsOverview", () => {
  it("defaults to BB skills and places Included with ARC skills first", () => {
    const markup = render({
      skills: [
    ...
    expect(markup).toContain('aria-label="Filters: Provider: ARC"');
    expect(markup).not.toContain("Provider: 1 selected");
    expect(markup).toContain("Sort");
    expect(markup).not.toContain('role="tab"');
    expect(markup).toContain("Included with ARC");
    expect(markup).toContain("New ARC skill");
  ...
  it("labels the Type filter and preserves independent source toggles", async () => {
    ...
    for (const name of ["Included with ARC", "Included in plugin", "User"]) {
      expect(
        screen
    ...
    expect(
      screen.getByLabelText(
        "automations is included with Automations (ARC plugin)",
      ).textContent,
    ).toBe("Included");
  ...
  it("puts every non-builtin, non-plugin scope in the User bucket", async () => {
    ...
    fireEvent.click(screen.getByRole("menuitemcheckbox", { name: "ARC" }));
    fireEvent.click(screen.getByRole("menuitemcheckbox", { name: "User" }));
  ...
  it("toggles Included with ARC independently from Included in plugin", async () => {
    renderDom(
      <SkillsOverview
        providerRoster={NO_PROVIDER_ROSTER}
        skills={[
    ...
    fireEvent.click(
      screen.getByRole("menuitemcheckbox", { name: "Included with ARC" }),
    );
    expect(await screen.findByText("official-skill")).toBeTruthy();
    expect(screen.getByText("automations")).toBeTruthy();
  ...
  it("lists a provider filter only for providers present in the skills", async () => {
    ...
    expect(
      screen
        .getByRole("menuitemcheckbox", { name: "ARC" })
        .getAttribute("aria-disabled"),
    ).toBeNull();
  });
  ...
  it("labels the Provider filter and prefixes its logo tooltip", async () => {
    ...
    expect(
      screen.getByRole("menuitemcheckbox", { name: "ARC" }).querySelector("img"),
    ).not.toBeNull();
  });
  ...
  it("keeps the default BB filter selected when only provider skills exist", async () => {
    ...
    fireEvent.pointerDown(screen.getByRole("button", { name: /^Filters/ }));
    const bbFilter = screen.getByRole("menuitemcheckbox", { name: "ARC" });
...
describe("SkillsLibrary library detail routing", () => {
  it("keeps a detail loading state while the skill library resolves", () => {
    ...
    expect(screen.queryByText("New ARC skill")).toBeNull();
  });
...
describe("SkillsLibrary registry detail lifecycle", () => {
  ...
  it("opens on Browse before Library and can start a skill from the registry", async () => {
    ...
    let forkButton = await screen.findByRole("button", {
      name: "Fork Useful skill into a new ARC skill",
    });
  ...
  it("renders the authoritative page order, exposes social proof, and loads more on scroll", () => {
    ...
    expect(
      screen.getByRole("button", {
        name: "Fork Alpha into a new ARC skill",
      }).textContent,
    ).toBe("");
    const zuluCreate = screen.getByRole("button", {
      name: "Fork Zulu into a new ARC skill",
    });
...
describe("SkillDetailDialogView", () => {
  it("presents a built-in skill as Included with ARC without an actions menu", async () => {
    ...
    const official = screen.getByLabelText("bb-cli is Included with ARC");
    expect(official.textContent).toBe("Included with ARC");
  ...
  it.each([
    ...
    },
    {
      ...
      accessibleLabel:
        "plugin-notes is included with Skill catalog fixture (ARC plugin)",
  ...
  it("uses a hoverable copy target and delegates editing to the thread flow", () => {
    ...
    expect(screen.queryByText("Included with ARC", { exact: true })).toBeNull();
