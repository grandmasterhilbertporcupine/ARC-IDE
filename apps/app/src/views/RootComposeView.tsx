...
import { usePreviewFeedbackDraft } from "@/hooks/usePreviewFeedbackDraft";
import { transferAddressedRecipients } from "@/hooks/useAddressedRecipients";
...
const ROOT_COMPOSE_CENTERED_CONTENT_CLASS = "min-h-full justify-center py-14";
...
export function RootComposeView() {
  ...
  const handleSubmit = useCallback(
    async (request: NewThreadComposerSubmission) => {
      ...
      if (request.experimental_addressing)
        transferAddressedRecipients(
          thread.projectId,
          thread.id,
          request.experimental_addressing.recipients,
        );
      setForkSeed(null);
...
function RootComposeSurface({
  ...
  usePreviewFeedbackDraft({
    ...
    projectId,
    append: handleRootPanelSelectionAddToChat,
  });
  ...
  const openBrowserTab = useCallback(
    (url?: string, htmlSource?: BrowserHtmlPreviewSource) => {
      const browserUrl = url ?? "";
      const tab = openTab({ kind: "browser", url: browserUrl, htmlSource });
      if (!htmlSource && browserUrl.length === 0 && tab?.kind === "browser") {
  ...
  const isComposing = startedComposing || !isPromptDraftEmpty(promptDraft);
  const showEmptyWelcome =
    !isForkDraft &&
    !isComposing &&
    projects !== undefined &&
    projects.length === 0;
  const showMobileHomeRecents =
    !showEmptyWelcome && !isComposing && !isForkDraft;
            ...
            >
              {showEmptyWelcome ? (
                ...
                <div className="flex w-full flex-col items-center gap-6">
                  {!isCompactViewport || !showMobileHomeRecents ? (
                    <div role="img" aria-label="ARC" className="select-none">
                      <BbLogo className="h-20 w-24 md:h-24 md:w-28" />
                    </div>
                  ) : null}
