...
describe("system cache effects", () => {
  it.each(["initial", "reconnect"] as const)(
    "refreshes plugin state missed before %s connection without touching newer data",
    async (connection) => {
      const queryClient = createCacheEffectQueryClient();
      const boundary = Date.now();
      const pluginKey = pluginListQueryKey(true);
      const pluginKeys = [
        pluginKey,
        pluginContributionsQueryKey(),
        pluginSettingsViewQueryKey("arc"),
        pluginSdkSettingsQueryKey("arc"),
        pluginSourceQueryKey("arc"),
        pluginCatalogSearchQueryKey("agents"),
      ];
      for (const queryKey of pluginKeys) {
        queryClient.setQueryData(queryKey, "not loaded", {
          updatedAt: boundary - 500,
        });
      }
      const freshKey = pluginSettingsViewQueryKey("codex");
      queryClient.setQueryData(freshKey, "current", {
        updatedAt: boundary + 500,
      });
      const observer = new QueryObserver(queryClient, {
        queryKey: pluginKey,
      ...
      try {
        if (connection === "initial") {
          invalidateRealtimeQueriesFetchedBeforeInitialConnect({
            connectedAt: boundary,
            queryClient,
          });
        } else {
          invalidateRealtimeQueriesAfterServerReconnect({
            disconnectedAt: boundary,
            queryClient,
          });
        }
        await vi.waitFor(() => {
          expect(observer.getCurrentResult().data).toBe("running");
        });
        for (const queryKey of pluginKeys.slice(1)) {
  ...
  it("recovers a plugin list request that failed before the first connection", async () => {
    const queryClient = createCacheEffectQueryClient();
    const queryFn = vi
      .fn<() => Promise<string>>()
