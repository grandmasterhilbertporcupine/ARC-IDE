...
describe("useThreadSearch", () => {
  it("keeps project-scoped and global results separate across a project switch", async () => {
    let settleOwn: ((value: ThreadSearchResponse) => void) | undefined;
    const ownRequest = new Promise<ThreadSearchResponse>((resolve) => {
      settleOwn = resolve;
    });
    const ownThread = makeThreadListEntry({
      id: "own",
      projectId: "project-a",
    });
    ...
    const response = (thread: ThreadListEntry): ThreadSearchResponse => ({
      active: { total: 1, results: [{ thread, matches: [] }] },
      archived: { total: 0, results: [] },
    });
    vi.mocked(sdk.threads.search).mockImplementation(async (input) =>
      input.projectId === "project-a" ? ownRequest : response(otherThread),
    );
    const { wrapper, queryClient } = createQueryClientTestHarness();
    const hook = renderHook<
      ReturnType<typeof useThreadSearch>,
    ...
    await waitFor(() =>
      expect(sdk.threads.search).toHaveBeenCalledWith(
        expect.objectContaining({ projectId: "project-a", limitPerGroup: "3" }),
      ),
    );
    hook.rerender({ projectId: "project-b" });
    await waitFor(() =>
      expect(hook.result.current.data?.active.results[0]?.thread.id).toBe(
        "other",
      ),
    );
    await act(async () => {
      settleOwn?.(response(ownThread));
      await ownRequest;
    });
    expect(hook.result.current.data?.active.results[0]?.thread.id).toBe(
      "other",
    );
    hook.rerender({ projectId: undefined });
    await waitFor(() =>
      expect(sdk.threads.search).toHaveBeenCalledWith(
        expect.objectContaining({ projectId: undefined, query: "needle" }),
      ),
    );
    expect(
    ...
    ).toEqual(response(otherThread));
    expect(
      threadSearchQueryKey({ query: "needle", limitPerGroup: 3 }),
    ).not.toEqual(
      threadSearchQueryKey({
        query: "needle",
        limitPerGroup: 3,
