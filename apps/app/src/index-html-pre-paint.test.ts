...
describe("index.html pre-paint script", () => {
  ...
  it("restores glass and its selected palette before paint without assuming native transparency", () => {
    const css = resolveAppThemeCss({
      ...defaultAppTheme,
      themeId: "nord",
    ...
    expect(document.getElementById("bb-app-theme")?.textContent).toBe(css);
    expect(css).toContain("--arc-surface-style: liquid-glass");
    expect(document.documentElement.hasAttribute("data-native-glass")).toBe(
      false,
    );
  });

  it("keeps the selected palette when glass is disabled and does not restore glass on reload", () => {
    const theme = { ...defaultAppTheme, themeId: "nord" };
    applyAppThemeCss(
      resolveAppThemeCss({ ...theme, surfaceStyle: "liquid-glass" }),
    );
    const paletteCss = resolveAppThemeCss(theme);
    ...
    expect(paletteCss).not.toContain("--arc-surface-style");
    expect(paletteCss.length).toBeGreaterThan(0);
  });
