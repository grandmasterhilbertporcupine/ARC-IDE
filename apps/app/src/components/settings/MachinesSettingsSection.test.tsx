...
const hostDaemon = vi.hoisted(() => ({
  ...
  platform: "darwin" as HostPlatform | null,
}));
...
describe("MachinesSettingsSection", () => {
  it("uses the primary host's Windows identity when no local helper is reachable", async () => {
  ...
  it("disables removal of the primary machine", async () => {
    ...
    expect(
      await screen.findByRole("tooltip", {
        name: "ARC's primary machine can't be removed.",
      }),
    ).toBeDefined();
