                  ...
                  <Tooltip delayDuration={300} disableHoverableContent>
                    <TooltipTrigger asChild>
                      <Button
                        ...
                        aria-label={`Dismiss ARC ${entry.version} changelog preview`}
                        onClick={() => {
            ...
            <div className="p-4 text-center sm:p-5">
              <div className="mx-auto max-w-sm">
                ...
                <p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">
                  We'll show the next ARC release here.
                </p>
              </div>
            </div>
          </div>
...
export function BbAppUpdateRows({
  ...
  if (isDesktop && desktopInfo === null) {
    return row(
      <RowName name="ARC app" current={null} latest={null} />,
      <RowStateControl live state="in-progress" />,
    );
  }
  ...
  if (desktopInfo !== null) {
    ...
    const name = (
      <RowName name="ARC app" current={desktopInfo.version} latest={latest} />
    );

    if (desktopInfo.updatesConfigured === false) {
      return row(
        name,
    ...
    if (desktopInfo.updateDownloaded) {
      return row(
        ...
        <RowStateControl
          ...
          actionLabel="Relaunch ARC to finish updating"
    ...
    if (desktopInfo.checkState === "checking") {
      return row(
        name,
        <RowStateControl live state="in-progress" />,
        <RowStateCaption state="in-progress">
          Checking for updates
        </RowStateCaption>,
      );
    }
    if (desktopInfo.checkState === "failed") {
      return row(
        name,
        <RowStateControl
          state="failed"
          buttonLabel="Retry"
          actionLabel="Retry checking for updates"
          onClick={() => onRetryDesktop?.()}
        />,
    ...
    if (desktopInfo.checkState === "idle") {
      return row(
        name,
        <span className="text-2xs text-muted-foreground">Not checked yet</span>,
      );
    }
  ...
  if (systemVersion.source === "disabled") {
    return row(
      name,
      <span className="text-2xs text-muted-foreground">
        Updates not configured
...
export function MachineUpdatesFleetSection({
  ...
  return (
    <SettingsSection
      ...
      description="Manage ARC and provider CLI updates across all machines."
