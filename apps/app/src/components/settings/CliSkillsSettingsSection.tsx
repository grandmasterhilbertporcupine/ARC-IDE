...
const CLI_SKILLS_SETTING_LABEL = "ARC CLI skills";
...
function installDescription(hasConnectedMachine: boolean): string {
  return hasConnectedMachine
    ? "Install them into ~/.agents/skills and ~/.claude/skills so agents outside ARC can use its bb CLI command."
...
function reportInstallResults(result: SystemInstallCliSkillsResponse): void {
  ...
  if (installed.length > 0) {
    appToast.success(
      `Installed the ARC CLI skills on ${installed
