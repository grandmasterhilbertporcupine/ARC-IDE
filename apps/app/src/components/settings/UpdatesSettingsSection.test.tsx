...
import { LATEST_CHANGELOG_ENTRY, RELEASE_META } from "./changelog-preview";
...
describe("UpdatesSettingsSection", () => {
  ...
  it("places fleet-wide Update all above every machine section", () => {
    ...
    expect(
      screen.getByText(
        "Manage ARC and provider CLI updates across all machines.",
      ),
    ).toBeDefined();
  });
...
`),
    ...
    expect(
      screen.queryByRole("button", {
        name: /^Open the full ARC .* changelog$/,
      }),
    ).toBeNull();
    const latestEntry = LATEST_CHANGELOG_ENTRY;
    if (latestEntry === null)
      throw new Error("Expected bundled ARC release notes");
    const latestVersion = latestEntry.version;
    const changelog = document.querySelector(
      '[data-updates-domain="changelog"]',
    );
    expect(changelog?.textContent).toContain(latestVersion);
    expect(changelog?.textContent).not.toContain("9.9.9");
    expect(fetch).not.toHaveBeenCalled();
    expect(
    ...
    expect(
      within(changelog as HTMLElement).getByRole("heading", {
        level: 3,
        name: RELEASE_META[latestVersion].headline,
      }),
    ).toBeDefined();
    expect(changelog?.textContent).not.toContain(
      "The canonical release summary.",
    );
    expect(
      changelog?.querySelector(`[data-changelog-version="${latestVersion}"]`),
    ).not.toBeNull();
    ...
    expect(changelog?.querySelector("[data-changelog-footer]")).toBeNull();
    expect(changelog?.textContent).not.toContain("Full changelog");
    for (const section of latestEntry.sections) {
      expect(
        within(changelog as HTMLElement).getByRole("heading", {
          level: 4,
          name: section.title,
    ...
    const dismissChangelog = screen.getByRole("button", {
      name: `Dismiss ARC ${latestVersion} changelog preview`,
    });
  ...
  it("does not call an offline fleet all in sync", async () => {
    ...
    const daemonRow = screen
      .getByText("ARC daemon")
      .closest("[data-resource-row]");
    expect(daemonRow).not.toBeNull();
    expect(screen.getByText("ARC app")).toBeDefined();
    expect(
  ...
  it("treats a recent daemon protocol mismatch as an automatic update", () => {
    ...
    expect(screen.getByText("ARC daemon")).toBeDefined();
    expect(screen.getAllByText("In progress").length).toBeGreaterThan(0);
    expect(
      document.querySelector('[data-updates-machine="host_1"]'),
    ).not.toBeNull();
    expect(screen.queryByText("1 machine is updating ARC")).toBeNull();
  ...
  it("explains and retries a daemon update that has stalled", () => {
    ...
    expect(
      screen.getByText("ARC daemon").closest("[data-resource-row]")?.className,
  ...
  it("names a machine running a newer ARC version than the server", () => {
    useDesktopUpdateInfoMock.mockReturnValue({
      desktopApi: null,
      desktopInfo: null,
      isDesktop: false,
  ...
  it("sweeps every machine stalled on the same ARC update", () => {
    useDesktopUpdateInfoMock.mockReturnValue({
      desktopApi: null,
      desktopInfo: null,
      isDesktop: false,
  ...
  it("shows unconfigured ARC updates while keeping provider actions available", () => {
    useDesktopUpdateInfoMock.mockReturnValue({
      desktopApi: null,
      desktopInfo: null,
      isDesktop: false,
    ...
    renderSection();
    expect(screen.getByText("Updates not configured")).toBeDefined();
    expect(screen.queryByText("npx bb-app@latest")).toBeNull();
    expect(
      screen.getByRole("button", { name: /Update Codex on workstation/ }),
    ).toBeDefined();
  });
  ...
  it("identifies an unconfigured desktop feed without presenting it as up to date", () => {
    ...
    renderSection();
    expect(screen.getByText("Updates not configured")).toBeDefined();
    expect(screen.queryByRole("button", { name: /Relaunch ARC/ })).toBeNull();
  });

  it("shows up to date only after a successful desktop check and clears retry errors", () => {
    const retry = vi.fn();
    const row = (checkState: BbDesktopInfo["checkState"]) => (
      <TooltipProvider>
        <BbAppUpdateRows
          ...
          onRetryDesktop={retry}
        />
      </TooltipProvider>
    );
    const view = render(row("idle"));
    expect(screen.getByText("Not checked yet")).toBeDefined();
    expect(screen.queryByText("Up to date")).toBeNull();
    view.rerender(row("checking"));
    expect(screen.getByText("Checking for updates")).toBeDefined();
    expect(screen.queryByText("Up to date")).toBeNull();
    view.rerender(row("failed"));
    expect(screen.getByText("Couldn't check for updates")).toBeDefined();
    expect(screen.queryByText("Up to date")).toBeNull();
    fireEvent.click(
      screen.getByRole("button", {
        name: "Failed · Retry checking for updates",
      }),
    );
    expect(retry).toHaveBeenCalledOnce();
    view.rerender(row("checking"));
    expect(screen.queryByText("Couldn't check for updates")).toBeNull();
    view.rerender(row("succeeded"));
    expect(screen.getByText("Up to date")).toBeDefined();
    expect(screen.queryByRole("button", { name: /Retry checking/ })).toBeNull();
  });

  it("retries a failed desktop check through the bridge without hiding the prior version", async () => {
    const desktopInfo: BbDesktopInfo = {
      checkState: "failed",
      updatesConfigured: true,
      downloadState: "idle",
      lastCheckedAt: null,
      latestVersion: "0.42.10",
      pendingVersion: null,
      platform: "windows",
      updateAvailable: true,
      updateDownloaded: false,
    ...
    expect(screen.getByText("Couldn't check for updates")).toBeDefined();
    expect(screen.getByText("0.42.10")).toBeDefined();
    fireEvent.click(
      screen.getByRole("button", {
        name: "Failed · Retry checking for updates",
      }),
  ...
  it("checks for desktop updates through the desktop bridge", async () => {
    ...
    const relaunch = screen.getByRole("button", {
      name: /Relaunch ARC to finish updating/,
    });
