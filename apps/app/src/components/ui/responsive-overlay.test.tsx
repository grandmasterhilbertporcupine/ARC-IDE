...
describe("ResponsiveDrawerShell", () => {
  it("settles immediately when reduced motion changes without replacing drawer inputs", () => {
    vi.useFakeTimers();
    let reduced = false;
    ...
    const view = render(
      <PersistentResponsiveDrawerShell
        open
        onOpenChange={() => {}}
        onContentAnimationEnd={settled}
        srLabel="Models"
      >
        <input aria-label="Model draft" defaultValue="Unsaved model" />
      </PersistentResponsiveDrawerShell>,
    );
    const input = screen.getByRole("textbox", { name: "Model draft" });
    ...
    expect(panel?.style.transition).toContain("0ms");
    ...
    expect(
      screen.getByRole("textbox", { name: "Model draft", hidden: true }),
    ).toBe(input);
    expect(input).toHaveProperty("value", "Unsaved model");
  });
