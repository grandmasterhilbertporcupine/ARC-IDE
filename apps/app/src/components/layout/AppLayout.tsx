...
import { ThreadBrowserLayout } from "@/components/sidebar/ThreadBrowser";
import { useMediaQuery } from "@bb/shared-ui/hooks/use-media-query";

const SIDEBAR_WIDTH_KEY = "arc.navigation.width";
const SIDEBAR_OPEN_KEY = "arc.navigation.open";
...
function SidebarStateBridge({ children }: SidebarStateBridgeProps) {
  ...
  const narrowViewport = useMediaQuery("(max-width: 1199px)");
  const autoCollapse =
    narrowViewport && (route.isRootView || route.isThreadView);
  const [narrowOpen, setNarrowOpen] = useState(false);
  const visibleOpen = autoCollapse ? narrowOpen : open;
...
export function AppLayout({ children }: AppLayoutProps) {
  ...
  useRouteEntryMotion(routeContentRef, location.pathname);
  const {
    projectId,
    threadId,
    isThreadView,
  ...
  const documentTitle = (() => {
    ...
    if (documentTitleBreadcrumbs) {
      const sectionLabel = documentTitleBreadcrumbs[0]?.label ?? "ARC";
    ...
    return routeTitle && routeTitle.length > 0 ? routeTitle : "ARC";
  })();
          ...
          <SidebarStateBridge>
            ...
            <SidebarInset>
              ...
              >
                ...
                <ThreadBrowserLayout active={isRootView || Boolean(threadId)}>
                  <main
                    ref={routeContentRef}
                    data-arc-route-motion
                    className={cn(
                      "flex min-h-0 min-w-0 flex-1 flex-col",
                      threadId ? "p-0" : "p-4 md:p-5",
                    )}
                  >
                    {children}
                  </main>
