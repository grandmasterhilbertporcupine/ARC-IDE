...
export function pluginRemovalLabel(plugin: PluginListItem): string {
  return pluginIsLocalSource(plugin) ? "Remove from ARC" : "Uninstall";
}

export function pluginRemovalDescription(plugin: PluginListItem): string {
  return pluginIsLocalSource(plugin)
    ? `Remove "${plugin.id}" from ARC and delete its settings, secrets, and schedules? Its source files stay on disk. To move it to another directory, install the new path instead; that keeps its settings.`
...
export function CatalogPluginDetailBanner({
  ...
  return (
    <PluginBannerBar
      tone="warning"
      icon="AlertTriangle"
      title="Update ARC to install this plugin"
