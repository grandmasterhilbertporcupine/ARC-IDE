...
export function SkillDetailStates() {
  return (
    ...
    >
      ...
      <State
        name="Provider-owned"
        note="A skill discovered under Claude Code or Codex carries that provider's logo where a ARC skill carries the ARC mark."
      >
      ...
      <State
        name="Included with ARC"
        note="A skill that is included with ARC uses the same publisher badge as a Included with ARC plugin. Its read-only behavior remains a separate permission fact."
      >
        <Skill
          titleBadge={{
            label: "Included with ARC",
            tooltip: "Included with ARC",
          }}
        />
      </State>
      ...
      <State
        name="Imported"
        note="Ownership is passive: a skill ARC cannot write shows its origin as a status, with no edit or acquisition control."
      >
        <Skill
      ...
      >
        <Skill
          headerActions={
            <ResourceInstallControl
              accessibleLabel="Fork writing-voice into a new ARC skill"
              label="Fork"
              icon="Fork"
              onAction={noop}
            />
...
const PLUGIN: PluginListItem = makePluginListItem({
  ...
  version: "1.4.0",
  description: "Browse GitHub issues and pull requests without leaving ARC.",
  name: "GitHub",
  icon: "Github",
...
const AWKWARD_PLUGIN: PluginListItem = {
  ...
  description:
    "Keeps issues, pull requests, review comments, and release checklists synchronized between ARC threads and your issue tracker, including bidirectional status mapping, attachment mirroring, and per-project field translation.",
  cliCommand: {
...
const BUNDLED_PLUGIN: PluginListItem = {
  ...
  provenance: "builtin",
  sourceDisplay: "Included with ARC",
...
const UNINSTALLED_CATALOG_PLUGIN = {
  ...
  source: "builtin:github",
  repositoryUrl: null,
  marketplaceDisplayName: "Included with ARC",
  publisherKey: "bb-official",
  publisherLabel: "Included with ARC",
  official: true,
  author: null,
  installed: false,
  installs: null,
...
const COMPATIBILITY_BLOCKED_PLUGIN: PluginListItem = {
  ...
  updateState: {
    ...
    blockedReasons: [
      "Requires ARC 0.20 or newer, and this ARC version is 0.18.",
    ],
  },
};
  ...
  return (
    <PluginStoryQueryBoundary>
      <Story
        ...
        description="An uninstalled Included with ARC plugin shows the catalog facts ARC can verify and offers Install. Once installed, the page adds runtime capabilities, settings, services, and schedules when they apply."
      >
        <State
          name="Before ownership"
          note="An uninstalled Included with ARC plugin opens as a real detail page. Install is the primary header action; the full-trust confirmation is the commit step."
        >
          <CatalogPlugin />
        </State>
        ...
        >
          <CatalogPlugin
            entry={{
              ...
              compatible: false,
              incompatibleReason: "Requires ARC 0.20 or newer.",
            }}
          />
        </State>
        ...
        <State
          name="Included with ARC · catalog"
          note="Installed from the included catalog. It shares the Included with ARC badge with built-in plugins, while its install date and ownership menu preserve the lifecycle difference."
        >
          <Plugin plugin={CATALOG_PLUGIN} />
        </State>

        <State
          name="Included with ARC · built-in"
          note="Included with ARC. The badge matches catalog-installed official plugins; the missing install date and ownership menu show that it cannot be uninstalled separately."
        >
        ...
        <State
          ...
          note="A newer release requires a newer ARC version. A dedicated Update row explains the requirement and preserved version; there is no unavailable action or modal."
        >
    ...
    <PluginStoryQueryBoundary>
      ...
      >
        ...
        <State
          ...
          note="The installed plugin cannot run with this version of ARC. The banner directs the user to install a compatible version without repeating the server's raw compatibility string."
        >
          <Plugin
            plugin={{
              ...PLUGIN,
              status: "incompatible",
              statusDetail: "requires ARC 0.20 or newer",
            }}
          />
        </State>
    ...
    <PluginStoryQueryBoundary>
      ...
      >
        ...
        <State
          ...
          note="Not a banner and not a failed attempt. The Update row names the ARC version requirement and preserved installed version; there is no unavailable action or dialog to dismiss."
        >
          <Plugin plugin={COMPATIBILITY_BLOCKED_PLUGIN} />
        </State>
      </Story>
      ...
      <main className="mx-auto w-full max-w-[72rem] space-y-8 px-5 py-6">
        ...
        >
          <ControlRow
            ...
            meaning="Canonical Included with ARC plugin acquisition action on both Browse and the pre-ownership detail page."
          />
          <ControlRow
          ...
          <ControlRow
            ...
            control={
              <ResourceInstallControl
                accessibleLabel="Fork example skill into a new ARC skill"
                label="Fork"
                icon="Fork"
                onAction={noop}
              />
            }
            meaning="Creates a new ARC skill from a registry source on the skill detail page."
          />
          <ControlRow
            state="Skill · forking"
            control={
        ...
        >
          <ControlRow
            state="Plugin · Included with ARC catalog"
            control={<PluginProvenancePill plugin={CATALOG_PLUGIN} />}
            meaning="Included with ARC and installed from the catalog."
          />
          <ControlRow
            state="Plugin · Included with ARC built-in"
            control={<PluginProvenancePill plugin={BUNDLED_PLUGIN} />}
            meaning="Included with ARC. The same badge communicates publisher; lifecycle differences remain in metadata and actions."
          />
          <ControlRow
            state="Skill · Included with ARC"
            control={
              <ProvenancePill
                label="Included with ARC"
                tooltip="Included with ARC"
            ...
            meaning="A skill that is included with ARC."
          />
          <ControlRow
        ...
        >
          ...
          <ControlRow
            ...
            meaning="Local sources can be edited, opened, submitted to the marketplace, or removed from ARC without deleting the source directory."
          />
          <ControlRow
            state="Included with ARC built-in actions"
        ...
        >
          ...
          <ControlRow
            ...
            meaning="A ARC skill can be edited, opened, or deleted."
          />
          <ControlRow
            state="Read-only actions"
            control={<NoControl>No ownership menu</NoControl>}
            meaning="Included with ARC, Included, and Imported skills expose provenance without pretending they are mutable."
          />
        </ControlTable>
