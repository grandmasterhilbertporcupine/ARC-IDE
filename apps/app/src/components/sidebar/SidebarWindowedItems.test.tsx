...
describe("SidebarWindowedItems", () => {
  it("keeps a thousand rich rows out of the DOM while retaining every navigation target", () => {
    const keys = Array.from({ length: 1000 }, (_, index) => `thread-${index}`);
    const { container } = render(
      <ThreadBrowserPresentation.Provider value={true}>
        <SidebarWindowedItems
          itemKeys={keys}
          focusItemKey="thread-999"
          estimateRows={() => 1}
          getNavigationEntries={(index) => [
            { threadId: keys[index], projectId: "project" },
          ]}
          renderItem={(index) => (
            <a href="#thread" data-sidebar-thread-id={keys[index]}>
              Thread {index}
            </a>
          )}
        />
    ...
    expect(document.activeElement).toBe(
      screen.getByRole("link", { name: "Thread 999" }),
    );
    expect(
      container.querySelectorAll("[data-sidebar-windowed-nav]"),
    ).toHaveLength(999);
    expect(
      container.querySelector<HTMLElement>("[data-sidebar-windowed-item]")
        ?.style.height,
    ).toBe("84px");
  });
