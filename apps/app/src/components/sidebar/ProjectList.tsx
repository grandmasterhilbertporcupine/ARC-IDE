...
interface ProjectListProps {
  ...
  browser?: { projectId: string | null; threads: ThreadListEntry[] };
}
...
function ProjectListComponent({
  ...
  const threads = useMemo(() => {
    if (browser) return browser.threads;
  ...
  const handleCreateProjectlessThread = useCallback(() => {
    openRootComposeForProject(browser?.projectId ?? PERSONAL_PROJECT_ID);
  }, [browser?.projectId, openRootComposeForProject]);
  const handleCreateThreadInSection = useCallback(
    (sectionId: string) => {
      openRootComposeForProject(
        browser?.projectId ?? PERSONAL_PROJECT_ID,
        sectionId,
      );
    },
    [browser?.projectId, openRootComposeForProject],
  ...
  const renderSectionDisplayOptions = (sectionId: SidebarSectionId) => {
    ...
    return (
      <SidebarDisplayOptionsMenu
        ...
        organization={browser ? "sections" : "all"}
      />
    );
  };
  const isSectionDisplayOptionsOpen = (sectionId: SidebarSectionId) =>
    openSidebarMenu === `displayOptions:${sectionId}`;
  const savedOrganizationMode = useAtomValue(sidebarOrganizationModeAtom);
  const organizationMode = browser ? "chronological" : savedOrganizationMode;
  const [chronologicalSort, setChronologicalSort] = useAtom(
    sidebarChronologicalSortAtom,
  );
