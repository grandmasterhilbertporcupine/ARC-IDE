...
import { useThreadBrowserPresentation } from "./threadBrowserState";
import {
  ThreadBrowserRowContent,
...
function ThreadRowComponent({
  ...
  const browserPresentation = useThreadBrowserPresentation();
  ...
  const rowClassName = cn(
    ...
    browserPresentation && "arc-thread-row",
  );
  ...
  const rowContent = (
    <>
      <NavLink
        ...
        state={
          searchMessageSeq === undefined
            ? undefined
            : { searchMessageSeq, searchThreadId: thread.id }
        }
      ...
      >
        {browserPresentation ? (
          <ThreadBrowserRowContent
            thread={thread}
            status={trailingIndicatorResolution.accessibleLabel}
            needsInput={trailingHasPendingInteraction}
            hasError={trailingHasUnreadError || thread.status === "error"}
            onSelect={onProjectSelect}
            working={splitIndicatorIsWorking}
          ...
          >
            {isEditing ? (
              <span className="pointer-events-auto relative z-10 block min-w-0">
                {editor}
              </span>
            ) : (
              <span
                className="bb-sidebar-thread-title line-clamp-2"
                title={labelTitle}
        ...
        {!browserPresentation && crossProjectLabel !== null ? (
          <Tooltip>
        ...
        {!browserPresentation && parentOptions && hasChildren ? (
          <SidebarChildToggleChevron
            isCollapsed={isParentCollapsed}
            expandLabel={`Expand ${labelTitle} threads`}
            collapseLabel={`Collapse ${labelTitle} threads`}
