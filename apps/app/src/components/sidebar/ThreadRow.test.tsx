...
import { ThreadBrowserPresentation } from "./threadBrowserState";
...
describe("thread browser row presentation", () => {
  it("shows pending child attention on a collapsed parent", () => {
    render(
  ...
  it("shows model and branch while retaining the existing thread link and title editing", () => {
    const thread = createThread({
      lastRequestedModel: { providerId: "codex", model: "gpt-5.6-sol" },
      environmentBranchName: "feature/threads",
    });
    render(
      <ThreadBrowserPresentation.Provider value={true}>
    ...
    expect(screen.getByText("feature/threads")).toBeTruthy();
    const link = screen.getByRole("link", { name: "Open Thread" });
    expect(link.getAttribute("href")).toContain(thread.id);
    fireEvent.doubleClick(link);
    expect(screen.getByRole("textbox", { name: "Thread name" })).toBeTruthy();
  });

  it("prioritizes a pending question over a working label", () => {
    const thread = createThread({
      hasPendingInteraction: true,
      activity: { ...createThread().activity, activeWorkflowCount: 1 },
    });
    render(
      <ThreadBrowserPresentation.Provider value={true}>
        <ThreadRowTestHarness thread={thread} />
