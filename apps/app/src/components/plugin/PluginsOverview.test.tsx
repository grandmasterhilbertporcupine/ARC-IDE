...
describe("PluginsOverview", () => {
  ...
  it("sorts enabled plugins before inactive plugins and published plugins first within enabled", async () => {
    ...
    const officialPills = screen.getAllByText("Included with ARC");
  ...
  it("gives each publisher its own Type facet, separate from User", async () => {
    ...
    fireEvent.click(
      screen.getByRole("menuitemcheckbox", { name: "Community" }),
    );
    await waitFor(() => {
      expect(rowIds()).toEqual([
        "plugin-row-builtin-one",
  ...
  it("badges a built-in plugin Included with ARC and a catalog install by its marketplace", async () => {
    installFetch([
      AUTOMATIONS_PLUGIN,
      {
        ...AUTOMATIONS_PLUGIN,
    ...
    const official = await screen.findAllByText("Included with ARC");
    expect(official).toHaveLength(1);
    const community = screen.getAllByText("Community");
