...
describe("AddPluginDialog", () => {
  ...
  it("describes each catalog source kind truthfully", () => {
    ...
    expect(
      screen.getByText("Install this plugin, bundled with ARC."),
    ).not.toBeNull();
    unmount();
    ...
    expect(
      screen.getByText(
        "Install this Community plugin from its listed source repository.",
      ),
    ).not.toBeNull();
    ...
    expect(
      screen.getByText(
        "Install this Community plugin from its listed npm package.",
      ),
    ).not.toBeNull();
  });
  ...
  it("names and links a catalog plugin when installation fails", async () => {
    ...
    stubFetch(
      { ok: false, error: "requires ARC >= 0.15 — you have 0.14.1" },
      422,
    );
    renderDialog({
      entryId: "linear",
    ...
    expect(pluginLink.parentElement?.textContent).toBe(
      "Linear — requires ARC >= 0.15 — you have 0.14.1",
    );
  });
