...
import { HtmlPreviewOpenAction } from "./HtmlPreviewOpenAction";
import {
  useHtmlPreviewUrl,
  hostHtmlPreviewTarget,
} from "@/hooks/queries/html-preview-query";
...
interface WorkspaceFilePreviewTabContentProps {
  onOpenHtmlPreview?: (source: BrowserHtmlPreviewSource) => void;
  activePath: string;
  isPanelOpen: boolean;
...
interface ProjectFilePreviewTabContentProps {
  htmlSource?: BrowserHtmlPreviewSource;
  onOpenHtmlPreview?: (source: BrowserHtmlPreviewSource) => void;
  activePath: string;
  isPanelOpen: boolean;
  copyPath?: string | null;
...
export function WorkspaceFilePreviewTabContent({
  ...
  const { data: previewEnvironment } = useEnvironment(environmentId, {
    enabled: isPanelOpen && isHtmlFilePreviewPath(activePath),
  });
  const { data: liveHtmlUrl } = useHtmlPreviewUrl({
    hostId: previewEnvironment?.hostId,
    rootPath: previewEnvironment?.path,
    filePath: activePath,
    enabled: isPanelOpen && source?.kind === "working-tree",
  });
  const {
  ...
  return (
    <HtmlPreviewOpenAction
      source={
        previewEnvironment?.hostId &&
        previewEnvironment.path &&
...
export function ProjectFilePreviewTabContent({
  ...
  const { data: liveHtmlUrl } = useHtmlPreviewUrl({
    hostId: htmlSource?.hostId,
    rootPath: htmlSource?.rootPath,
    filePath: htmlSource?.filePath ?? activePath,
    enabled: isPanelOpen,
  });
  const {
...
export function HostFilePreviewTabContent({
  ...
  const { data: liveHtmlUrl } = useHtmlPreviewUrl({
    hostId: previewEnvironment?.hostId,
    ...hostHtmlPreviewTarget(activePath),
    enabled: isPanelOpen,
  });
  const {
  ...
  return (
    ...
    >
      <SecondaryPanelFilePreview
        ...
        htmlPreviewUrl={
          liveHtmlUrl ?? buildRawFilesystemHtmlContentUrl(threadId, activePath)
        }
        isLoading={isHostFilePreviewLoading}
...
export function HostScopedFilePreviewTabContent({
  ...
  return (
    <HtmlPreviewOpenAction
      source={{ hostId, ...hostHtmlPreviewTarget(activePath) }}
      onOpen={onOpenHtmlPreview}
    >
      <SecondaryPanelFilePreview
        activePath={activePath}
