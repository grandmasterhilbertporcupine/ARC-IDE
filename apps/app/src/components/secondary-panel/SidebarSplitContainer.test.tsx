...
describe("SidebarSplitContainer", () => {
  ...
  it("aligns a restored single pane with the host selection so external open actions work after reload", async () => {
    persistState(
      createSidebarSplitState(
        TABS.map((tab) => tab.id),
        "tab-b",
    ...
    renderContainer({
      activeTabId: "tab-a",
      renderPane: ({ group }) => (
        <div data-testid="restored-active-tab">{group.activeTabId}</div>
      ),
    });
    await waitFor(() =>
      expect(screen.getByTestId("restored-active-tab").textContent).toBe(
        "tab-a",
      ),
    );
  });
