...
describe("useThreadFileTabs recently closed tabs", () => {
  it("keeps the HTML source after opening and explicitly clears it on navigation away", () => {
    const { result } = renderThreadHook(() =>
      useThreadFileTabs({
        panelStateId: "html-preview",
        syncThreadId: null,
        environmentId: "env_1",
    ...
    expect(result.current.browserTabs[0]).toEqual(
      expect.objectContaining({ htmlSource }),
    );
    act(() => {
    ...
    expect(result.current.browserTabs[0]?.htmlSource).toBeUndefined();
  });
