...
export interface UpdateBrowserTabArgs {
  htmlSource?: BrowserHtmlPreviewSource | null;
  tabId: string;
  url: string;
  title: string | null;
}
...
export type OpenSecondaryPanelTabRequest =
  ...
  | { kind: "browser"; url: string; htmlSource?: BrowserHtmlPreviewSource }
  | { kind: "new-tab" };
...
export function useThreadFileTabs({
  ...
  const updateBrowserTab = useCallback(
    ({ tabId, url, title, htmlSource }: UpdateBrowserTabArgs) => {
      updateFixedPanelTabsState((state) => {
