...
function IframeFilePreview({ sandbox, title, url }: IframeFilePreviewTarget) {
  ...
  useEffect(() => {
    const match = /\/api\/v1\/file-previews\/([^/]+)\//u.exec(url);
    if (!match) return;
    let cancelled = false;
    ...
    const poll = async () => {
      try {
        if (
          !document.hidden &&
          iframeRef.current &&
          iframeRef.current.getBoundingClientRect().width > 0
        ) {
          ...
          setLiveError(
            result.truncated
              ? "Live reload reached its 2,048-file limit. Additional assets require manual refresh."
              : null,
          );
          if (result.changed) setReloadVersion((version) => version + 1);
        }
      } catch (error) {
        if (!cancelled)
          setLiveError(
            error instanceof Error
              ? `Live preview paused: ${error.message}. Refresh to reconnect.`
              : "Live preview paused. Refresh to reconnect.",
          );
      }
  ...
  return (
    <div className="relative min-h-0 flex-1 overflow-hidden">
      ...
      {liveError && (
        <p
          role="status"
          className="absolute inset-x-0 top-0 z-10 bg-background px-3 py-2 text-xs text-muted-foreground"
        >
          {liveError}
        </p>
      )}
