...
import { ContextEmbeddingProxy } from "./context/proxy.js";
...
const contextEmbeddings = new ContextEmbeddingProxy(
...
async function handleCall(
  ...
  try {
    ...
    const result = await handler(input, {
      ...
      experimental_contextEmbeddings: contextEmbeddings.capability(
        message.callId,
      ),
...
try {
  ...
  process.on("message", (raw: unknown) => {
    if (
      isRecord(raw) &&
      (raw.type === "context.result" || raw.type === "context.cancelled")
    ) {
      contextEmbeddings.receive(raw);
      return;
    }
