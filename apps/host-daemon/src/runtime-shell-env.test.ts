...
describe("resolveUserShellPath", () => {
  ...
  it("probes the PowerShell profile PATH on Windows", async () => {
    const fakeSpawn = createFakeShellEnvSpawn({
      results: [
        createShellEnvSpawnResult({
    ...
    expect(fakeSpawn.calls[0]?.args).toContain("-Command");
    expect(fakeSpawn.calls[0]?.args.at(-1)).toContain("$env:PATH");
  });
});
