...
afterEach(async () => {
  for (const linkedPath of linkedPaths.splice(0)) await unlink(linkedPath);
  await Promise.all(
...
describe("injected skill staging", () => {
  it("separates required and inherited catalogs even when the skill bytes match", async () => {
    const dataDir = await makeTempDir();
    const payload = createTreePayload("required-reader");
    const source = createTreeSource("required-reader", payload.treeHash);
    if (source.kind !== "tree") throw new Error("Expected tree source");
    const inherited = await stageInjectedSkillSources({
      dataDir,
      injectedSkillSources: [source],
      fetchSkillTree: async () => payload,
    });
    const required = await stageInjectedSkillSources({
      dataDir,
      injectedSkillSources: [{ ...source, required: true }],
      fetchSkillTree: async () => payload,
    });
    expect(required.catalogHash).not.toBe(inherited.catalogHash);
    expect(required.skillRoots[0]?.skills).toEqual(
      inherited.skillRoots[0]?.skills,
    );
  });
  it("pulls a missing tree, stages identical bytes and modes, and reuses the store", async () => {
    ...
    if (process.platform !== "win32") {
      ...
      expect(
        payload.entries.find((entry) => entry.path === "scripts/run.sh")?.mode,
      ).toBe(0o755);
      ...
      expect(catalog).toMatchObject({
        skills: [
          expect.objectContaining({
            name: "synced-skill",
            sourceRootPath: `skill-tree:${payload.treeHash}`,
          }),
        ],
      });
    }
  ...
  it("rejects linked files or junction directories that escape the skill root", async () => {
    const dataDir = await makeTempDir();
    ...
    if (process.platform === "win32")
      await symlink(
        outsideDir,
        path.join(skillRootPath, "references", "escaped-directory"),
        "junction",
      );
    else
      await symlink(
    ...
    linkedPaths.push(
      path.join(
        skillRootPath,
        "references",
        process.platform === "win32" ? "escaped-directory" : "escape.md",
      ),
    );
    const warnings: CapturedWarning[] = [];
