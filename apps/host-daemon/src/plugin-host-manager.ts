...
import { ContextEmbeddingBridge } from "./context/bridge.js";
import { contextBridgeCancelSchema } from "./context/bridge-contract.js";
...
interface PluginHostManagerOptions {
  ...
  contextEmbeddings?: ContextEmbeddingBridge;
}

const START_TIMEOUT_MS = 10_000;
const CANCEL_GRACE_MS = 5_000;
const HOST_WORKER_PROTOCOL_VERSION = 4;
...
export class PluginHostManager {
  ...
  private readonly contextEmbeddings: ContextEmbeddingBridge;

  constructor(private readonly options: PluginHostManagerOptions) {
    this.contextEmbeddings =
      options.contextEmbeddings ?? new ContextEmbeddingBridge();
  }
  ...
  async shutdown(): Promise<void> {
    ...
    await this.contextEmbeddings.dispose();
  }
  ...
  private async ensureWorkerNow(
    ...
    try {
      const workerEntryPath = defaultWorkerEntryPath();
      child = fork(
        workerEntryPath,
        [artifactPath, command.pluginId, command.generation, dataDir, tempDir],
        {
          ...(workerEntryPath.endsWith(".ts")
            ? {
                execArgv: [
                  ...process.execArgv,
                  "--import",
    ...
    const failWorker = (
      ...
      void this.contextEmbeddings.retire(worker).catch((error: unknown) => {
        this.options.logger.warn(
          { pluginId: worker.pluginId, err: error },
          "Context helper cleanup remains unresolved",
        );
      });
    ...
    child.on("message", (message: unknown) => {
      ...
      if (record.type === "context.request") {
        if (worker.disposing || this.workers.get(worker.pluginId) !== worker)
          return;
        this.cancelWorkerIdleTimer(worker);
        this.contextEmbeddings.handle(
          worker,
          message,
          (result) => sendToWorker(worker.child, result),
          () => this.scheduleWorkerIdle(worker),
        );
        this.scheduleWorkerIdle(worker);
        return;
      }
      if (record.type === "context.cancel") {
        const parsed = contextBridgeCancelSchema.safeParse(message);
        if (parsed.success)
          void this.contextEmbeddings
            .cancel(worker, parsed.data.requestId)
            .then(
              ({ state }) => {
                sendToWorker(worker.child, {
              ...
              (error: unknown) => {
                this.options.logger.warn(
                  { pluginId: worker.pluginId, err: error },
                  "Context cancellation remains unresolved",
                );
                sendToWorker(worker.child, {
                  type: "context.cancelled",
                  requestId: parsed.data.requestId,
                  ...
                  error: {
                    ...
                    message: "Context helper exit has not been confirmed",
                  },
                });
              },
            );
  ...
  private async stopWorker(worker: WorkerState, reason: string): Promise<void> {
    ...
    try {
      await this.contextEmbeddings.retire(worker);
    } catch (error) {
      contextFailure =
        error instanceof Error
          ? error
          : new Error("Context helper cleanup remains unresolved");
    }
  ...
  private scheduleWorkerIdle(worker: WorkerState): void {
    if (
      ...
      this.contextEmbeddings.has(worker) ||
      worker.idleTimer !== null
    ) {
      return;
    }
  ...
  private cancelPendingCall(
    ...
    if (pending.forceTimer === null) {
      const schedule = () => {
        if (!worker.pending.has(callId) || pending.forceTimer !== null) return;
        pending.forceTimer = setTimeout(() => {
          if (worker.pending.has(callId)) worker.child.kill("SIGKILL");
        }, CANCEL_GRACE_MS);
        pending.forceTimer.unref?.();
      };
      if (this.contextEmbeddings.has(worker, callId)) {
        void this.contextEmbeddings
          .cancelCall(worker, callId)
          .then(schedule, (error: unknown) => {
            this.options.logger.warn(
              { pluginId: worker.pluginId, err: error },
              "Context cancellation remains unresolved",
            );
