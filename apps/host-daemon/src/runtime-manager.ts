...
interface RuntimeSkillConfig {
  ...
  requiredSkillConfiguration?: boolean;
}
...
export class RuntimeManager {
  ...
  private async resolveRuntimeSkillConfig(
    ...
    return {
      ...staged,
      requiredSkillConfiguration: args.injectedSkillSources.some(
        (source) => source.kind === "tree" && source.required === true,
      ),
    };
  }
  ...
  private async ensureCompatibleEntry(
    ...
    if (
      ...
      if (args.skillConfig.requiredSkillConfiguration)
        throw new Error(
          "assigned_skill_catalog_busy: This environment is running with different skills. Wait for its current work to finish before starting this assigned role.",
        );
      if (
    ...
    runtime = this.createRuntime({
      ...
      ...(args.skillConfig
        ? {
            ...
            requiredSkillConfiguration:
              args.skillConfig.requiredSkillConfiguration ?? false,
          }
        : {}),
      ...(providerProcessEnv ? { env: providerProcessEnv } : {}),
      shellEnv,
