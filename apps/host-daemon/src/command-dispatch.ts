...
const commandHandlers: CommandHandlerMap = {
  ...
  "turn.stop-if-current": async (command, options) => {
    const entry = await options.runtimeManager.getOrAwait(
      command.environmentId,
    );
    const result = entry
      ? await entry.runtime.stopTurnIfCurrent(command)
      : {
...
const onlineRpcHandlers: OnlineRpcHandlerMap = {
  ...
  "terminal.probePreview": async (command, options) =>
    options.terminalManager?.probePreview
      ? options.terminalManager.probePreview(command.terminalId, command.url)
      : {
          state: "unavailable",
          statusCode: null,
          reason: "Terminal readiness inspection is unavailable",
        },
  "terminal.stopPreview": async (command, options) =>
    options.terminalManager?.stopPreview
      ? options.terminalManager.stopPreview(command.terminalId)
      : {
          state: "unavailable",
          exitCode: null,
          reason: "Confirmed preview cleanup is unavailable on this host",
        },
