import { probeTerminalPreview } from "./preview-probe.js";
import type {
...
function terminalSpawnArgsForStart(
  ...
  if (/^(?:pwsh|powershell)(?:\.exe)?$/iu.test(path.win32.basename(shell))) {
    const encoding =
    ...
    return message.start.mode === "shell"
      ? ["-NoLogo", "-NoExit", "-Command", encoding]
      : [
          "-NoLogo",
          "-NoProfile",
          "-NonInteractive",
          "-Command",
          `${encoding}; ${message.start.command}`,
        ];
  }
...
export class TerminalManager {
  ...
  private readonly confirmedCloseOnly = new Set<string>();
  private readonly previewClosures = new Map<
    string,
    {
      promise: Promise<TerminalPreviewStopResult>;
      resolve: (result: TerminalPreviewStopResult) => void;
    }
  >();
  private readonly previewProbes = new Map<
    string,
  ...
  async probePreview(
    ...
    if (!session?.pty.pid)
      return {
        state: "unavailable",
        statusCode: null,
        reason: "The owned preview terminal is no longer available",
      };
    const key = JSON.stringify([terminalId, url]);
    const existing = this.previewProbes.get(key);
    if (existing) return existing;
    if (this.previewProbes.size >= 2)
      return {
        state: "unavailable",
        statusCode: null,
        reason: "Preview readiness inspection is busy; retry shortly",
      };
    const pending = (async (): Promise<TerminalPreviewProbeResult> => {
      const result = await probeTerminalPreview(
        session.pty.pid!,
        url,
        this.platform,
      );
      if (this.sessions.get(terminalId) !== session)
        return {
          state: "unavailable",
          statusCode: null,
          reason: "The preview terminal ended during readiness inspection",
        };
      return result;
    })();
    this.previewProbes.set(key, pending);
    try {
      return await pending;
    } finally {
      this.previewProbes.delete(key);
    }
  }

  async stopPreview(terminalId: string): Promise<TerminalPreviewStopResult> {
    const pending = this.previewClosures.get(terminalId);
    ...
    if (!this.sessions.has(terminalId))
      return {
        ...
        exitCode: null,
        reason:
          "The owned terminal is unavailable; its process exit cannot be confirmed",
      };
    let resolve!: (result: TerminalPreviewStopResult) => void;
    const promise = new Promise<TerminalPreviewStopResult>((done) => {
      resolve = done;
    });
    this.previewClosures.set(terminalId, { promise, resolve });
    this.confirmedCloseOnly.add(terminalId);
    try {
      await this.runTerminalOperation({
        terminalId,
        operation: () => {
          if (!this.sessions.has(terminalId)) {
            this.confirmedCloseOnly.delete(terminalId);
            this.resolvePreviewClose(terminalId, {
              state: "unavailable",
              exitCode: null,
              reason:
                "The owned terminal disappeared before cleanup could be confirmed",
            });
            return;
          }
          this.closeTerminal({ terminalId, reason: "user" });
        },
      });
      ...
      this.resolvePreviewClose(terminalId, {
        ...
        reason:
          error instanceof Error
            ? error.message
            : "Preview cleanup could not be confirmed",
      });
      return promise;
    }
  }

  private resolvePreviewClose(
    terminalId: string,
    result: TerminalPreviewStopResult,
  ): void {
    const pending = this.previewClosures.get(terminalId);
    this.previewClosures.delete(terminalId);
  ...
  private closeTerminal(args: CloseTerminalArgs): void {
    ...
    try {
      ...
      if (this.confirmedCloseOnly.has(session.terminalId)) return;
      this.finishTerminalSession({
        closeReason: args.reason,
        exitCode: null,
        session,
  ...
  private forceCloseTerminal(session: TerminalSession): void {
    ...
    if (this.confirmedCloseOnly.has(session.terminalId)) {
      session.closeTimeout = setTimeout(() => {
        session.closeTimeout = null;
        if (this.sessions.get(session.terminalId) !== session) return;
        session.closeReason = null;
        this.resolvePreviewClose(session.terminalId, {
          state: "unavailable",
          exitCode: null,
          reason:
            "The preview process has not confirmed exit. Its terminal is retained; retry Stop before starting a replacement.",
        });
      }, this.closeGracePeriodMs);
      return;
    }
  ...
  private finishTerminalSession(args: FinishTerminalSessionArgs): void {
    ...
    if (
      args.exitCode === null &&
      this.confirmedCloseOnly.has(args.session.terminalId)
    ) {
      this.resolvePreviewClose(args.session.terminalId, {
        state: "unavailable",
        exitCode: null,
        reason:
          "Preview cleanup ended without a native process exit acknowledgement. Reconnect its host and retry Stop.",
      });
      return;
    }
    ...
    this.confirmedCloseOnly.delete(args.session.terminalId);
  }
