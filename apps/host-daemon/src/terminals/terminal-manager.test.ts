...
function createFakeRuntime(): AgentRuntime {
  ...
  return {
    ...
    stopTurnIfCurrent: vi.fn(async () => {
      throw new Error("Unexpected conditional stop in fake runtime");
    }),
...
describe("TerminalManager", () => {
  ...
  it.each(["kill failure", "missing exit"])(
    "retains an owned preview after %s until a native exit arrives",
    async (failure) => {
      vi.useFakeTimers();
      const harness = createHarnessWithOptions({
        closeGracePeriodMs: 10,
      ...
      if (failure === "kill failure")
        kill.mockImplementation(() => {
          throw new Error("Kill failed");
        });

      const pending = harness.manager.stopPreview("term-1");
      ...
      kill.mockRestore();
      const retry = harness.manager.stopPreview("term-1");
  ...
  it("shares pending preview cleanup and acknowledges only the actual exit code", async () => {
    vi.useFakeTimers();
    const harness = createHarness();
    const pty = await openTerminal(harness);
    const first = harness.manager.stopPreview("term-1");
    const second = harness.manager.stopPreview("term-1");
    await vi.advanceTimersByTimeAsync(0);
    expect(pty.killCalls).toEqual([null]);
    expect(pty.disposeCount).toBe(0);
    pty.emitExit(17);
  ...
  it("retains preview ownership through shutdown without inventing a null exit", async () => {
    vi.useFakeTimers();
    const harness = createHarnessWithOptions({
      closeGracePeriodMs: 10,
      onSendMessage: () => undefined,
  ...
  it("cannot confirm preview cleanup for a missing terminal", async () => {
    const harness = createHarness();
    await expect(harness.manager.stopPreview("missing")).resolves.toMatchObject(
      { state: "unavailable", exitCode: null },
    );
    expect(harness.messages).toEqual([]);
  });
  ...
  it("opens native Windows PowerShell terminals", async () => {
    const harness = createHarness();
    const manager = new TerminalManager({
      logger: {
        debug: vi.fn(),
