...
Windows, macOS and Linux Electron shell for ARC. The desktop app loads the ARC
web UI and uses the compatible `bb-app` launcher for server and host-daemon
lifecycle. Windows is the first release target.

Windows packaging uses `pnpm exec turbo run package:windows --filter=@bb/desktop` for an
unpacked application and `pnpm exec turbo run dist:windows --filter=@bb/desktop` for an NSIS
installer. It does not bundle forecasting or a Python model runtime. See
[Windows installation and provider setup](../../docs/arc-windows.md),
[packaged verification](scripts/smoke-arc-windows.md) and
[installer verification](scripts/smoke-arc-installer.md).

The assisted Windows installer uses a dark ARC rail and a silent animated ARC
mark beside the native installation controls. `dist:windows` generates the
installer artwork through its `generate:installer-art` dependency. The generated
AVI/BMP files are ignored and excluded from the installed application. Windows
client-area animation settings and high contrast select the still artwork;
silent installation creates no theme controls. The location chooser, real file
progress, shortcuts and data-preserving uninstaller retain the NSIS lifecycle.

ARC uses app ID `dev.arc.desktop`, a separate Electron profile, runtime data in
`~/.arc` and packaged service ports 38986/38987. Stable Windows builds use public GitHub Releases at grandmasterhilbertporcupine/ARC-IDE by default. ARC checks on launch, periodically and through Settings > Updates, downloads new stable releases in the background, and installs on restart or quit. No GitHub token is bundled or needed. At build time, an explicitly empty `ARC_UPDATE_BASE_URL` disables updating; a nonempty HTTPS override preserves the custom `/desktop-latest/` or `/desktop-nightly/` feed path. Default nightly, macOS and Linux builds have no published feed. Builds through 0.42.8 need a one-time manual install to enable GitHub updates.

See [GitHub release publishing](../../docs/arc-releases.md).
Public release acceptance requires valid Authenticode signatures, clean Windows
11 standard-user verification, provider workflows, update migration and rollback.
...
`~/.arc-dev/<checkout-instance>` data directory and deterministic high ports as
