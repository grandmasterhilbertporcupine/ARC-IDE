...
function menuArgs(
  ...
  return {
    ...
    servers: [{ checked: true, id: "builtin", name: "This computer" }],
    setServerUrl: () => {},
    ...overrides,
  };
}
...
describe("application menu", () => {
  ...
  it("shows reload shortcuts without globally stealing browser commands", () => {
    ...
    expect(submenu.some((item) => item.role === "togglefullscreen")).toBe(
      false,
    );
  ...
  it("builds a Window ▸ Server radio submenu with a Set Server URL item", () => {
    ...
    const template = buildApplicationMenuTemplate(
      menuArgs(() => {}, {
        selectServer,
        servers: [
          { checked: false, id: "builtin", name: "This computer" },
  ...
  it.each(["Windows", "Linux"])(
    "builds native %s window shortcuts",
    (platform) => {
      vi.mocked(Menu.sendActionToFirstResponder).mockClear();
      const template = buildApplicationMenuTemplate(
        menuArgs(() => {}, {
      ...
      const fullscreen = viewSubmenu.find(
      ...
      if (platform === "Windows") {
        ...
        expect(fullscreen).toBeUndefined();
      }
