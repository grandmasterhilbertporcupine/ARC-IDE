...
describe("bb app process", () => {
  it.each(["executable", "working directory"])(
    "rejects a missing %s with the spawn error",
    async (missing) => {
      const script = await createTempScript({ contents: "process.exit(0);\n" });
      const missingPath = join(script.root, "missing");

      await expect(
        startBbAppProcess({
          bridgePath: script.path,
          cwd: missing === "working directory" ? missingPath : script.root,
          env: process.env,
          logLineLimit: 20,
          runtime: {
            executablePath:
              missing === "executable" ? missingPath : process.execPath,
            kind: "direct",
            mode: "node",
          },
        }),
  ...
  it("observes a successful bridge that exits immediately", async () => {
    const script = await createTempScript({ contents: "process.exit(0);\n" });
    const processEntry = await startBbAppProcess({
      bridgePath: script.path,
      cwd: script.root,
      env: process.env,
      logLineLimit: 20,
    ...
    expect(processEntry.pid).toBeGreaterThan(0);
    await expect(processEntry.exit).resolves.toEqual({ code: 0, signal: null });
  });
