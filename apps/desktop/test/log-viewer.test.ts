...
describe("log viewer", () => {
  ...
  it("kills tail child processes when stopped", async () => {
    ...
    expect(processIds).toHaveLength(process.platform === "win32" ? 0 : 2);
  ...
  it.skipIf(process.platform !== "win32")(
    "preserves split UTF-8 and stops native reads",
    async () => {
      const tempDir = await createTempDir();
      const file = join(tempDir.path, "server.1.log");
      const lines: LogViewerLine[] = [];
      ...
      tailers.push(tailer);
      await tailer.start();
      const message = Buffer.from("Δ orchestration\n");
      await appendFile(file, message.subarray(0, 1));
      await new Promise((done) => setTimeout(done, 50));
      await appendFile(file, message.subarray(1));
      await waitFor({
        predicate: () =>
          lines.some((line) => line.text === "[server] Δ orchestration"),
      });
      await writeFile(file, "reset\n");
      await waitFor({
        predicate: () => lines.some((line) => line.text === "[server] reset"),
      });
      tailer.stop();
  ...
  it.skipIf(process.platform !== "win32")(
    "bounds an unterminated native log line",
    async () => {
      const tempDir = await createTempDir();
      const file = join(tempDir.path, "server.1.log");
      const lines: LogViewerLine[] = [];
      ...
      tailers.push(tailer);
      await tailer.start();
      await appendFile(file, "x".repeat(2 * 1024 * 1024));
      await new Promise((done) => setTimeout(done, 100));
      await appendFile(file, "END\n");
      await waitFor({
        predicate: () => lines.some((line) => line.text.endsWith("END")),
      });
      const line = lines.find((entry) => entry.text.endsWith("END"))!;
      expect(line.text).toContain("[earlier log text truncated]");
      expect(line.text.length).toBeLessThan(128 * 1024 + 64);
    },
  );
