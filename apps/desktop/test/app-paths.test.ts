...
describe("desktop app paths", () => {
  it("resolves the packaged bb-app bridge beside the active asar", () => {
    ...
    expect(resolveDesktopBridgePath({ paths })).toBe(
      normalize("/Applications/bb.app/Contents/Resources/app.asar.unpacked/dist/bb-app-bridge.mjs"),
    );
  });
  ...
  it("resolves the universal packaged bb-app bridge beside the selected arch asar", () => {
    ...
    expect(resolveDesktopBridgePath({ paths })).toBe(
      normalize("/Applications/bb.app/Contents/Resources/app-arm64.asar.unpacked/dist/bb-app-bridge.mjs"),
    );
  });
  ...
  it("uses the release-specific icon inside packaged apps", () => {
    ...
    expect(
      ...
      normalize("/Applications/bb Nightly.app/Contents/Resources/app.asar/assets/icon-nightly.png"),
    );
  });
  ...
  it("keeps the development icon independent of the release channel", () => {
    ...
    ).toBe(normalize("/checkout/apps/desktop/assets/arc-icon.png"));
  });
});
