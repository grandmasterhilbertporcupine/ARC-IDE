...
import type { BbDesktopInfo, BbDesktopVersionFeed } from "@bb/desktop-contract";
import {
...
describe("desktop update service", () => {
  it.each(["network", "http", "malformed"])(
    "reports %s failures and succeeds only after a valid retry",
    async (failure) => {
      let attempt = 0;
      ...
      const service = createDesktopUpdateService({
        ...
        fetchImpl: async () => {
          ...
          return createFeedResponse("0.0.1");
        },
      });
      ...
      expect(states).toEqual(["checking", "failed", "checking", "succeeded"]);
    },
  );
