...
describe("desktop window factory", () => {
  ...
  it("gives Windows native caption controls, standard resizing and Alt menu access", async () => {
    const tempDir = await createTempDir();
    const createdWindows: FakeDesktopWindow[] = [];
    const factory = createDesktopWindowFactory({
      browserWindowCreator: {
        create(options) {
          const window = new FakeDesktopWindow({ options });
          createdWindows.push(window);
          return window;
        },
      },
      createWindowStateKey: () => "windows-overlay",
    ...
    await factory.createWindow({
      initialUrl: "http://localhost:38886",
      stateKey: null,
    });
    const options = createdWindows[0]!.options;
    expect(options.titleBarStyle).toBe("hidden");
    expect(options.titleBarOverlay).toEqual({ height: 36 });
    ...
    expect(createdWindows[0]!.loadedUrls).toEqual(["http://localhost:38886"]);
  });
