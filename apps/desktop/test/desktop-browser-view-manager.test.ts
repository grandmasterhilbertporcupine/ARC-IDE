...
import { z } from "zod";
import { protectPrivateFile } from "@bb/process-utils";
import {
...
const electronMock = vi.hoisted(() => {
  ...
  class FakeWebContents {
    ...
    emitConsoleMessage(level: number, message: string): void {
      for (const listener of this.listeners["console-message"])
        listener(fakeWebContentsEvent, level, message, 1, "page.js");
    }
...
describe("DesktopBrowserViewManager", () => {
  it("captures only the bound conversation with bounded redacted console feedback and resets on navigation", async () => {
    const { manager, hostWindow, view } = createRendererRecoveryFixture(111);
    for (let index = 0; index < 65; index++)
      view.webContents.emitConsoleMessage(2, `row ${index} token=secret`);
    await expect(
      manager.captureFeedback({
        hostWindow,
        request: {
          tabId: "browser:a",
    ...
    const pending = manager.captureFeedback({
      hostWindow,
      request: { tabId: "browser:a", threadId: "thread-1", mode: "capture" },
    });
    await settlePendingCaptures(view);
    const feedback = await pending;
    expect(feedback?.console).toHaveLength(50);
    expect(feedback?.console[0]?.level).toBe("2");
    expect(JSON.stringify(feedback?.console)).not.toContain("secret");
    expect(feedback?.screenshot).toBe(
      Buffer.from("jpeg-bytes").toString("base64"),
    );
    view.webContents.emitDidNavigate("https://example.com/next?token=secret");
    const next = manager.captureFeedback({
      hostWindow,
      request: { tabId: "browser:a", threadId: "thread-1", mode: "capture" },
    });
    await settlePendingCaptures(view);
  ...
  it.each(["document", "same-document"])(
    "rejects feedback if the preview has a %s navigation during screenshot capture",
    async (kind) => {
      const { manager, hostWindow, view } = createRendererRecoveryFixture(112);
      const pending = manager.captureFeedback({
        hostWindow,
        request: { tabId: "browser:a", threadId: "thread-1", mode: "capture" },
      });
      const rejected = expect(pending).rejects.toThrow("navigated");
      if (kind === "document")
        view.webContents.emitDidNavigate("https://example.com/changed");
      else
        view.webContents.emitDidNavigateInPage("https://example.com/changed");
      await settlePendingCaptures(view);
      await rejected;
      manager.destroyAll();
    },
  ...
  it.each([false, true])(
    "releases destroyed native windows without reading their webContents (browser tabs: %s)",
    async (hasBrowserTab) => {
      const { manager, hostWindow: firstWindow } =
        createRendererRecoveryFixture(91);
      const closingWindow = new FakeHostWindow({
        contentBounds: { width: 700, height: 450 },
        webContentsId: 92,
      });
      ...
      for (const window of [firstWindow, closingWindow]) {
      ...
      const firstTarget = broker.getTarget(91);
      const closingTarget = broker.getTarget(92);
      if (!firstTarget || !closingTarget)
        throw new Error("Expected both application windows to be registered");
      if (hasBrowserTab) {
        attachBrowserTab({
          manager,
          hostWindow: closingWindow,
          tabId: "browser:closing",
          url: "https://example.com/",
        });
        await broker.execute({
          type: "desktop.browser.acquire_control",
          instanceId: closingTarget.instanceId,
          generation: closingTarget.generation,
          threadId: "thread-1",
          leaseId: "closing-window-lease",
          tabIds: ["browser:closing"],
          controllerLabel: "Test",
          expiresAt: Date.now() + 60_000,
        });
      }
      const closingContents = closingWindow.webContents;
      const sentBeforeClose = closingContents.sentPayloads.length;
      closingWindow.destroyed = true;
      Object.defineProperty(closingWindow, "webContents", {
        configurable: true,
        get() {
          throw new Error(
            "WebContents unavailable after native window destruction",
          );
        },
      });
      try {
        manager.releaseWindow(92);
        broker.releaseWindow(92);
        expect(broker.getTarget(92)).toBeNull();
        expect(broker.getTarget(91)).toEqual(firstTarget);
        expect(broker.listInstances()).toHaveLength(1);
        expect(
          manager.listTabs({ hostWebContentsId: 91, threadId: "thread-1" }),
        ).toHaveLength(1);
        expect(
          manager.listTabs({ hostWebContentsId: 92, threadId: "thread-1" }),
        ).toEqual([]);
        expect(closingContents.sentPayloads).toHaveLength(sentBeforeClose);
        await expect(
          broker.execute({
        ...
        ).rejects.toThrow("Desktop instance is unavailable");
      } finally {
        Object.defineProperty(closingWindow, "webContents", {
          value: closingContents,
        });
        broker.dispose();
        manager.destroyAll();
      }
  ...
  it("opens popup dispositions in a hardened native window", () => {
    ...
    expect(popupWindow.titleCalls).toEqual(["ARC browser popup"]);
