...
import {
  BB_DESKTOP_BROWSER_FEEDBACK_CHANNEL,
...
const electronMock = vi.hoisted(() => {
  ...
  const handlers = new Map<
    string,
    (event: FakeIpcEvent, payload: unknown) => Promise<unknown>
  >();
  ...
  return {
    ...
    ipcMain: {
      handle(
        channel: string,
        handler: (event: FakeIpcEvent, payload: unknown) => Promise<unknown>,
      ) {
...
class RecordingDesktopBrowserViewManager implements DesktopBrowserViewManager {
  async captureFeedback(): Promise<null> {
    return null;
  }
  createTab(): never {
...
describe("registerDesktopBrowserIpc", () => {
  it("allows feedback only from a registered application main frame and validates its scope", async () => {
    const manager = new RecordingDesktopBrowserViewManager();
    const capture = vi.spyOn(manager, "captureFeedback");
    const mainFrame = {};
    const sender = { id: 50, mainFrame };
    electronMock.windowsBySender.set(sender, { label: "trusted" });
    registerDesktopBrowserIpc(manager, (id) => id === 50);
    ...
    await expect(handler({ sender, senderFrame: {} }, payload)).rejects.toThrow(
      "Untrusted",
    );
    await expect(
      handler(
        { sender: { id: 51, mainFrame }, senderFrame: mainFrame },
        payload,
      ),
    ...
    await expect(
      handler(
        ...
        { ...payload, mode: "arbitrary" },
      ),
    ).rejects.toThrow();
    expect(capture).not.toHaveBeenCalled();
    await expect(
      handler({ sender, senderFrame: mainFrame }, payload),
    ).resolves.toBeNull();
    expect(capture).toHaveBeenCalledExactlyOnceWith({
      hostWindow: { label: "trusted" },
      request: payload,
    });
  });
