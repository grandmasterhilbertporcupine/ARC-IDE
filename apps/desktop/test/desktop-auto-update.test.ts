...
describe("desktop auto-update service", () => {
  it("preserves the last release after a failed check and recovers on retry", async () => {
    ...
    const states: Array<BbDesktopInfo["checkState"]> = [];
    service.subscribe((info) => states.push(info.checkState));
    expect(service.getInfo().checkState).toBe("idle");
    await service.checkForUpdates();
    updater.updateCheckError = new Error("network offline");
    const failure = service.checkForUpdates();
    expect(service.getInfo().checkState).toBe("checking");
  ...
  it("does not report an empty native update result as a successful check", async () => {
    const updater = new DesktopAutoUpdaterAdapterStub();
    const service = createDesktopAutoUpdateService({
      currentVersion: "0.0.1",
      enabled: true,
  ...
  it("does not initialize electron-updater in dev mode without the override", async () => {
    ...
    expect(service.getInfo().updatesConfigured).toBe(false);
