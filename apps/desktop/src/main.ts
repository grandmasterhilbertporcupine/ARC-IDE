...
import { registerDesktopSurfaceStyleIpc } from "./desktop-surface-style.js";
import { registerDesktopTitleBarIpc } from "./desktop-title-bar.js";
import {
...
let localViewTitleBar: { logoDataUrl: string } | null = null;
...
let quitting = false;
let ownedRuntimeStopPromise: Promise<void> | null = null;
...
function registerApplicationWindow(browserWindow: DesktopBrowserWindow): void {
  ...
  registerDesktopWindowStateEvents(browserWindow, (state) => {
    sendToApplicationRenderer(
  ...
  if (!localRuntimeReady || currentRuntime === null) {
    return (
      cachedFailure ?? {
        code: "network",
        detail:
          "the local ARC server is unavailable, and this app has no stored bb Connect credential",
        ok: false,
      }
    );
  }
...
function ensureDesktopMachineEnrolled(): void {
  ...
  if (!cache.canPersist()) {
    createDesktopLogger().info(
      "[desktop] no OS keychain available — keeping the local ARC server for bb Connect sessions",
    );
    return;
  }
  ...
  if (target.kind === "builtin") {
    ...
    if (!attached) {
      await loadStartupError({
        details:
          "Could not connect to the local ARC server on this computer. Check that the port is free or that a compatible ARC server is running.",
        logs: "",
...
async function loadLoadingView(): Promise<void> {
  ...
  await loadWindowUrl({
    url: createLocalViewUrl({
      titleBar: localViewTitleBar,
      viewModel: {
        kind: "loading",
        message: "Starting local services and opening the ARC workspace.",
        title: "Opening ARC",
      },
    }),
  });
...
function stopOwnedRuntime(): Promise<void> {
  if (ownedRuntimeStopPromise !== null) {
    return ownedRuntimeStopPromise;
  }
  ...
  ownedRuntimeStopPromise = stopOwnedRuntimeAndRelease({
    release() {
    ...
    async stop() {
      if (runtime.bbProcess === null) {
        throw new Error("The owned runtime has no process to confirm stopped");
      }
    ...
    ownedRuntimeStopPromise = null;
  });
  return ownedRuntimeStopPromise;
}

function reportDesktopShutdownError(error: unknown): void {
  createDesktopLogger().error(
    `Desktop shutdown did not finish cleanly: ${error instanceof Error ? error.message : String(error)}`,
  );
}
...
function handleBeforeQuit(event: Event): void {
  ...
  void handleDesktopQuit({
    finishQuit,
    onError: reportDesktopShutdownError,
    quitApplication() {
      app.quit();
    },
  });
...
function registerDesktopUpdateIpc(): void {
  registerDesktopTitleBarIpc(applicationWindowWebContentsIds);
  registerDesktopSurfaceStyleIpc(applicationWindowWebContentsIds);
...
async function startOwnedRuntime(
  ...
  const bbProcess = await startBbAppProcess({
  ...
  void bbProcess.exit
    .then(async (exit) => {
      ...
      await stopOwnedRuntime().catch(reportDesktopShutdownError);
      if (quitting || (currentRuntime !== null && currentRuntime !== runtime)) {
        return;
      }
      await loadStartupError({
    ...
    .catch(reportDesktopShutdownError);
  ...
  await loadStartupError({
    details:
      raceResult.result.kind === "incompatible"
        ? `Port ${args.serverUrl} is responding, but it does not look like ARC: ${raceResult.result.reason}.`
        : `Timed out waiting for ARC at ${args.serverUrl}: ${raceResult.result.reason}.`,
    logs: bbProcess.logs.text(),
    title: "Could not start ARC",
  });
  await stopOwnedRuntime();
...
async function decideOnExistingServer(
  ...
  if (stopResult.kind === "unverified") {
    await loadStartupError({
      details:
        `The ARC instance at ${probe.serverUrl} records process ${String(stopResult.pid)}, but that ` +
        "process no longer matches the record. ARC did not stop it. Stop it yourself, then open ARC again.",
      logs: "",
      title: "Could not stop the running ARC instance",
    });
    return "quit";
  }
  if (stopResult.kind === "still-running") {
    await loadStartupError({
      details: `ARC could not stop process ${String(stopResult.pid)}, even after forced termination.`,
      logs: "",
      title: "Could not stop the running ARC instance",
    });
    return "quit";
  ...
  if (stopResult.kind === "replaced") {
    await loadStartupError({
      details:
        `Another ARC instance started at ${probe.serverUrl} while the question was open, so ARC stopped nothing. ` +
        "Open ARC again to see the copy that runs now.",
      logs: "",
      title: "Could not stop the running ARC instance",
    });
    return "quit";
  ...
  if (!(await waitForServerToStop(probe.serverUrl))) {
    await loadStartupError({
      details: `The ARC instance at ${probe.serverUrl} stopped, but the address is still in use.`,
      logs: "",
      title: "Could not stop the running ARC instance",
    });
    return "quit";
...
async function initializeRuntime(args: InitializeRuntimeArgs): Promise<void> {
  ...
  if (existingProbe.kind === "incompatible") {
    await loadStartupError({
      details: `Port ${args.serverUrl} is already in use, but it is not a compatible ARC server: ${existingProbe.reason}.`,
      logs: "",
      title: "Port conflict",
    });
    return;
...
async function runDesktopApp(): Promise<void> {
  ...
  if (process.platform === "win32") {
    app.setAppUserModelId(
      DESKTOP_RELEASE_CHANNEL === "nightly"
        ? "dev.arc.desktop.nightly"
        : "dev.arc.desktop",
    );
  }
  const explicitUserDataPath = app.commandLine.getSwitchValue("user-data-dir");
  if (explicitUserDataPath && !isAbsolute(explicitUserDataPath)) {
    throw new Error("--user-data-dir must be an absolute path.");
  }
  app.setPath(
    "userData",
    explicitUserDataPath || join(app.getPath("appData"), applicationName),
  );
  ...
  registerDesktopShutdownSignalHandlers({
    ...
    onError(error) {
      quitting = false;
      reportDesktopShutdownError(error);
    },
  ...
  registerDesktopBrowserIpc(desktopBrowserViewManager, (id) =>
  ...
  if (desktopUpdateSupport.autoUpdate) {
    ...
    logger.info(
      desktopUpdateSupport.versionCheck
        ? "Desktop auto-install is unavailable for this installation. Version checks remain enabled."
        : "Desktop updates are disabled for this build or platform.",
    );
  }
  ...
  localViewTitleBar =
    process.platform === "win32"
      ? {
          logoDataUrl: windowIcon.resize({ width: 16, height: 16 }).toDataURL(),
        }
      : null;
