import { bbDesktopBrowserFeedbackRequestSchema } from "@bb/desktop-contract";
...
export function registerDesktopBrowserIpc(
  ...
  isApplicationSender: (id: number) => boolean = () => false,
): void {
  ipcMain.handle(
    BB_DESKTOP_BROWSER_FEEDBACK_CHANNEL,
    async (event, payload: unknown) => {
      if (
        !isApplicationSender(event.sender.id) ||
        event.senderFrame !== event.sender.mainFrame
      )
        throw new Error("Untrusted preview request");
      const hostWindow = BrowserWindow.fromWebContents(event.sender);
      if (!hostWindow) throw new Error("Preview window is unavailable");
      return manager.captureFeedback({
        hostWindow,
        request: bbDesktopBrowserFeedbackRequestSchema.parse(payload),
      });
    },
  );
