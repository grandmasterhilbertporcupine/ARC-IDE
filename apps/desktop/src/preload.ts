...
import { BB_DESKTOP_BROWSER_FEEDBACK_CHANNEL } from "./desktop-browser-ipc.js";
...
import {
  ...
  type BbDesktopTitleBarAppearance,
  bbDesktopSurfaceStyleSchema,
  bbDesktopSurfaceStyleResultSchema,
  type BbDesktopApi,
...
function syncLocalViewWindowState(): void {
  if (
  ...
  if (root?.hasAttribute("data-bb-local-view")) {
    root.setAttribute(
      "data-bb-desktop-fullscreen",
      String(currentWindowState.isFullScreen),
    );
  }
}
...
async function invokeDesktopWindowState(): Promise<BbDesktopWindowState> {
  const requestRevision = windowStateRevision;
  try {
    const payload: unknown = await ipcRenderer.invoke(
      BB_DESKTOP_GET_WINDOW_STATE_CHANNEL,
    );
    if (requestRevision !== windowStateRevision) {
      return currentWindowState;
    }
...
const bbBrowserApi: BbDesktopBrowserApi = {
  async captureFeedback(request) {
    return bbDesktopBrowserFeedbackSchema
      .nullable()
      .parse(
        await ipcRenderer.invoke(BB_DESKTOP_BROWSER_FEEDBACK_CHANNEL, request),
      );
  },
  async getTarget() {
...
const bbDesktopApi: BbDesktopApi = {
  ...
  get updatesConfigured() {
    return currentInfo.updatesConfigured;
  },
  ...
  ...(process.platform === "win32"
    ? {
        ...
        async setTitleBarAppearance(
          appearance: BbDesktopTitleBarAppearance,
        ): Promise<void> {
          await ipcRenderer.invoke(
            BB_DESKTOP_SET_TITLE_BAR_APPEARANCE_CHANNEL,
            bbDesktopTitleBarAppearanceSchema.parse(appearance),
          );
        },
      }
    : {}),
  ...
  async setSurfaceStyle(style) {
    return bbDesktopSurfaceStyleResultSchema.parse(
      await ipcRenderer.invoke(
        BB_DESKTOP_SET_SURFACE_STYLE_CHANNEL,
        bbDesktopSurfaceStyleSchema.parse(style),
      ),
    );
  },
};
...
if (typeof document !== "undefined") {
  document.addEventListener("DOMContentLoaded", syncLocalViewWindowState, {
    once: true,
  });
}
