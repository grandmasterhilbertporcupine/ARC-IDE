...
const LOG_VIEWER_MAX_LINE_CHARACTERS = 128 * 1024;
const LOG_VIEWER_TRUNCATED_LINE_PREFIX = "[earlier log text truncated] ";
...
<head>
  ...
  <title>ARC - Server & Daemon Logs</title>
  <style>
    :root {
...
export function createLogTailer(args: CreateLogTailerArgs): LogTailer {
  ...
  function handleTailChunk(handleArgs: HandleTailChunkArgs): void {
    ...
    const boundLine = (line: string): string =>
      line.length > LOG_VIEWER_MAX_LINE_CHARACTERS
        ? LOG_VIEWER_TRUNCATED_LINE_PREFIX +
          line.slice(-LOG_VIEWER_MAX_LINE_CHARACTERS)
        : line;
  ...
  function stopTailProcess(stopArgs: StopTailProcessArgs): void {
    ...
    stopArgs.state.offset = 0;
    stopArgs.state.decoder = new StringDecoder("utf8");
    if (tailProcess === null) {
      return;
    }
    if (tailProcess.childProcess.pid !== undefined) {
  ...
  async function refreshTailProcesses(): Promise<void> {
    ...
    for (const state of componentStates) {
      ...
      if (process.platform === "win32") await readWindowsLog(state);
    }
  }

  async function readWindowsLog(state: ComponentTailState): Promise<void> {
    if (state.currentFilePath === null || stopped) return;
    const handle = await open(state.currentFilePath, "r");
    try {
      const { size } = await handle.stat();
      if (stopped) return;
      const initial = state.offset < 0;
      if (initial || size < state.offset) {
        state.offset = Math.max(0, size - 1024 * 1024);
        state.pendingText = "";
        state.decoder = new StringDecoder("utf8");
      }
      let initialText = "";
      while (state.offset < size && !stopped) {
        const bytes = Buffer.alloc(Math.min(64 * 1024, size - state.offset));
        ...
        if (bytesRead === 0 || stopped) break;
        state.offset += bytesRead;
        const chunk = state.decoder.write(bytes.subarray(0, bytesRead));
        if (initial) initialText += chunk;
        else handleTailChunk({ chunk, state });
      }
      if (initial && !stopped) {
        const lines = initialText.split("\n");
        const count =
          LOG_VIEWER_INITIAL_TAIL_LINES + (initialText.endsWith("\n") ? 1 : 0);
        handleTailChunk({ chunk: lines.slice(-count).join("\n"), state });
      }
    } finally {
      await handle.close();
    }
