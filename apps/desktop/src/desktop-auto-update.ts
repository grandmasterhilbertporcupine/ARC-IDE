...
export function createDesktopAutoUpdateService(
  ...
  let currentInfo: BbDesktopInfo = {
    ...createBaseInfo(args.currentVersion, args.platform),
    updatesConfigured: args.enabled,
  };
  ...
  function updateInfo(nextInfo: BbDesktopInfo): void {
    ...
    currentInfo = { ...nextInfo, updatesConfigured: args.enabled };
  ...
  async function checkForUpdates(): Promise<BbDesktopInfo> {
    ...
    const requestPromise = (async () => {
      ...
      updateInfo({ ...currentInfo, checkState: "checking" });
      ...
      if (result === null) {
        ...
        logger.warn("Desktop auto-update check returned no release result.");
        return currentInfo;
      }
  ...
  if (args.enabled) {
    ...
    args.updater.onError((errorArgs) => {
      ...
      } else if (currentInfo.checkState === "checking") {
        updateInfo({ ...currentInfo, checkState: "failed" });
      }
    });
  }
