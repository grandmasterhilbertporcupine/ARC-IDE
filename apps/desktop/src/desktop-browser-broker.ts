...
export function createDesktopBrowserBroker(args: {
  ...
  function instanceForWindow(webContentsId: number): InstanceEntry | undefined {
    return [...instances.values()].find(
      (entry) => entry.hostWebContentsId === webContentsId,
    );
  }
  ...
  function tabsFor(
    ...
    return args.manager.listTabs({
      hostWebContentsId: instance.hostWebContentsId,
      threadId,
    });
  }
  ...
  async function openConnection(lease: ControlLease) {
    ...
    const scope = {
      hostWebContentsId: lease.instance.hostWebContentsId,
  ...
  return {
    registerWindow(window: BrokerWindow) {
      ...
      if (instanceForWindow(hostWebContentsId)) return;
      const descriptor = {
        instanceId: randomUUID(),
        generation: randomUUID(),
        label: `ARC window ${hostWebContentsId}`,
      };
