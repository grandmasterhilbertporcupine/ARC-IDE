...
async function fetchJson<TValue>(
  ...
  try {
    ...
    return {
      kind: error instanceof SyntaxError ? "schema-error" : "network-error",
      message,
    };
  } finally {
...
export async function probeBbServer(
  ...
  if (configResult.kind === "network-error") {
    return {
      kind: "unavailable",
      reason: `/api/v1/system/config returned ${configResult.message}`,
      serverUrl: args.serverUrl,
    };
  }
...
export async function waitForCompatibleServer(
  ...
  return {
    kind: "unavailable",
    reason: `Timed out after ${args.timeoutMs}ms waiting for ARC server. Last probe: ${lastResult.reason}`,
    serverUrl: args.serverUrl,
  };
}
