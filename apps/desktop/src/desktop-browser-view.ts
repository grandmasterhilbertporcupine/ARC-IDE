import {
  cancelPreviewElementSelection,
  selectPreviewElement,
  redactPreviewDiagnostic,
  previewDiagnosticUrl,
} from "./desktop-browser-feedback.js";
import type {
  BbDesktopBrowserFeedback,
  BbDesktopBrowserFeedbackRequest,
...
interface BrowserViewEntry {
  diagnostics: Pick<BbDesktopBrowserFeedback, "console" | "network">;
...
export interface DesktopBrowserViewManager {
  captureFeedback(
    args: HostScopedRequestArgs<BbDesktopBrowserFeedbackRequest>,
  ): Promise<BbDesktopBrowserFeedback | null>;
  createTab(args: {
    hostWindow: DesktopBrowserHostWindow;
    tabId: string;
    threadId: string;
...
export function createDesktopBrowserViewManager(
  ...
  function ensureHardenedSession(tabPartition: string): Session {
    ...
    const recordNetwork = (details: {
      ...
      if (!entry) return;
      entry.diagnostics.network.push({
        url: previewDiagnosticUrl(details.url),
        method: details.method.slice(0, 32),
        status: details.statusCode ?? 0,
        error: details.error ? redactPreviewDiagnostic(details.error) : null,
      });
      if (entry.diagnostics.network.length > 100)
        entry.diagnostics.network.shift();
    };
    browserSession.webRequest?.onCompleted(recordNetwork);
    browserSession.webRequest?.onErrorOccurred(recordNetwork);
  ...
  function wireWebContents(
    ...
    webContents.on("console-message", (_event, level, message) => {
      entry.diagnostics.console.push({
        level: String(level).slice(0, 30),
        message: redactPreviewDiagnostic(message),
      });
      if (entry.diagnostics.console.length > 50)
        entry.diagnostics.console.shift();
    });
    const resetFeedback = () => {
      entry.feedbackRevision += 1;
      entry.diagnostics.console = [];
      entry.diagnostics.network = [];
    };
    webContents.on("did-navigate", resetFeedback);
    webContents.on("did-navigate-in-page", (_event, _url, isMainFrame) => {
      if (isMainFrame) resetFeedback();
    });
  ...
  return {
    async captureFeedback({ hostWindow, request }) {
      ...
      if (
        ...
        throw new Error("Preview tab is unavailable");
      const contents = entry.view.webContents;
      const revision = entry.feedbackRevision;
      if (request.mode === "cancel") {
        await cancelPreviewElementSelection(contents);
        return null;
      }
      const element =
        request.mode === "select" ? await selectPreviewElement(contents) : null;
      if (request.mode === "select" && element === null) return null;
      if (
        contents.isDestroyed() ||
        entry.feedbackRevision !== revision ||
        entries.get(browserViewKey(hostWindow, request.tabId)) !== entry
      )
        throw new Error("Preview tab changed while capturing");
      const captured = await captureDesktopBrowserPage(contents);
      if (entry.feedbackRevision !== revision)
        throw new Error("Preview navigated while capturing; try again");
      if (captured.isEmpty()) throw new Error("Preview screenshot is empty");
      const dimensions = captured.getSize();
      const ratio = Math.min(
        1,
        1600 / dimensions.width,
        1200 / dimensions.height,
      );
      const image =
        ratio < 1
          ? captured.resize({
          ...
          : captured;
      return {
        url: previewDiagnosticUrl(contents.getURL()),
        title: contents.getTitle().slice(0, 1024),
        screenshot: image.toJPEG(70).toString("base64"),
        element,
