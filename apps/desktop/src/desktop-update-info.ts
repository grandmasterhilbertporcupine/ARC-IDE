...
export function mergeDesktopUpdateInfo(
  ...
  const checkState =
    args.autoInfo?.updatesConfigured === true
      ? args.autoInfo.checkState
      : (args.feedInfo ?? args.autoInfo)?.checkState;

  return {
    ...baseInfo,
    ...(checkState === undefined ? {} : { checkState }),
    ...(args.feedInfo?.updatesConfigured === undefined &&
    args.autoInfo?.updatesConfigured === undefined
      ? {}
      : {
          updatesConfigured:
            args.feedInfo?.updatesConfigured === true ||
            args.autoInfo?.updatesConfigured === true,
        }),
