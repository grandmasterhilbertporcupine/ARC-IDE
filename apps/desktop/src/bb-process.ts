...
export async function startBbAppProcess(
  args: StartBbAppProcessArgs,
): Promise<BbAppProcess> {
  ...
  return {
    ...
    async stop(stopArgs) {
      ...
      if (process.platform === "win32") {
        killProcessGroup({ child: childProcess, signal: stopArgs.killSignal });
        const result = await waitForProcessExitWithTimeout({
          childProcess,
          timeoutMs: stopArgs.killTimeoutMs,
        });
        if (result !== "exited")
          throw new Error(
            "The owned Windows runtime did not exit after tree termination",
          );
        return;
      }
