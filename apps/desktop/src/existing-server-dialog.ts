...
export function formatSurface(surface: string): string {
  return surface === "desktop" ? "the ARC desktop app" : "a terminal";
}
...
export function renderExistingServerDialogHtml(
  ...
  const replaceButtonHtml = canReplace
    ? `<button type="button" data-choice="replace">Quit other ARC</button>`
    : "";
  ...
  const introText = canReplace
    ...
    : "This app can use the copy that is already running. ARC cannot identify that copy, so it cannot stop it for you.";
...
<head>
  ...
  <title>ARC is already running</title>
  <style>
    :root {
...
<body>
  <h1>ARC is already running on this computer</h1>
  <p>${introText}</p>
  <div class="details">
      ${detailHtml}
  </div>
  ${replaceWarningHtml}
  <div class="actions">
    <button type="button" data-choice="quit">Quit this ARC</button>
    ${replaceButtonHtml}
