...
import { readPrivateFileUtf8 } from "@bb/process-utils";
import {
...
async function readBrokerDescriptor(dataDir: string) {
  const descriptorPath = join(dataDir, DESKTOP_BROWSER_BROKER_DESCRIPTOR_FILE);
  return desktopBrowserBrokerDescriptorSchema.parse(
    JSON.parse(await readPrivateFileUtf8(descriptorPath)),
  );
}
