...
describe("promoteRuntimeEntries", () => {
  ...
  it("preserves live exports and stale files when a staged replacement is missing", async () => {
    rootDir = await mkdtemp(
      path.join(tmpdir(), "bb-plugin-sdk-promote-missing-"),
    );
    const distDir = path.join(rootDir, "dist");
    const stagingDir = path.join(rootDir, "staging");
    await mkdir(distDir);
    await mkdir(stagingDir);
    await writeFile(path.join(distDir, "provider-bridge.js"), "intact bridge");
    await writeFile(path.join(distDir, "stale.js"), "still needed");
    await expect(
      promoteRuntimeEntries({
        distDir,
        stagingDir,
        relativeOutputs: ["provider-bridge.js"],
      }),
    ).rejects.toMatchObject({ code: "ENOENT" });
    expect(
      await readFile(path.join(distDir, "provider-bridge.js"), "utf8"),
    ).toBe("intact bridge");
    expect(await readFile(path.join(distDir, "stale.js"), "utf8")).toBe(
      "still needed",
    );
  });
});
