 ...
 * side effects. The ARC app imports these to keep its real implementation in
...
export interface PluginThreadListProps {
  ...
  onNavigate: () => void;
  /**
   * Compatibility value for the former sidebar search field. ARC now searches
  ...
  searchQuery: string;
  /**
   * ARC's thread list, bound to this sidebar instance. Render it to delegate
...
/** An optional companion beside a root thread's existing chat and composer. */
export interface ExperimentalThreadViewProps {
  threadId: string;
  projectId: string;
  /** Stable unique DOM id of the preserved main chat. Resolve with getElementById for geometry; do not persist or mutate it. */
  mainElementId: string;
  /** This view's opaque URL state, up to 512 characters, or null. Validate any referenced records against the thread. */
  viewState: string | null;
  /** Push this view's URL state through host navigation; null removes it. Other views and query parameters are preserved. */
  onViewStateChange(value: string | null): void;
}

/** Display-only metadata for one exact thread supplied to an annotation publisher. */
export interface ExperimentalThreadListAnnotation {
  threadId: string;
  identities: readonly {
    kind: "agent" | "group";
    id: string;
  ...
  counters: readonly { id: string; label: string; value: number }[];
  /** Local id of this plugin's registered companion, or null. */
  viewId: string | null;
}

/** Mounted once per represented project, never once per row. Render null. */
export interface ExperimentalThreadListAnnotationsProps {
  projectId: string;
  /** Up to 500 currently visible or expanded thread ids, all in this project. */
  threadIds: readonly string[];
  /** Replace this publisher's annotations. Publish [] while unavailable. */
  onChange(annotations: readonly ExperimentalThreadListAnnotation[]): void;
}
...
export interface PluginFileOpenerProps {
  ...
  /**
   * ARC's file preview, bound to this file. Render it to delegate conditionally
...
/**
 * Props of the host-owned `experimental_SourceCode` component — ARC's source
 * viewer. The host owns syntax highlighting, gutters, wrapping, line-selection
 * presentation, and the live ARC code theme; the caller owns loading the text
...
/**
 * Props of the host-owned `experimental_Diff` component — ARC's diff viewer.
 ...
 * and the live ARC code theme. Content that cannot be parsed as a patch
...
export interface DiffProps {
   ...
   * patch, ARC enables expand-context controls between hunks. The caller owns
...
export interface PluginSourceCodeRendererProps {
  ...
  highlightedLines: SourceCodeLineRange | null;
  /**
   * ARC's source renderer, bound to this request. Render it to delegate
   * conditionally without re-entering plugin replacement resolution.
   *
   * @experimental Audit before relying on this as a stable contract.
   */
...
export interface PluginDiffRendererProps {
   ...
   * treating the contents as complete. ARC's original renderer performs that
   * verification when it mounts.
   */
  experimental_fullFileContents: ExperimentalDiffFullFileContents | null;
  /**
   * ARC's diff renderer, bound to this request. Render it to delegate
   * conditionally without re-entering plugin replacement resolution.
   *
   * @experimental Audit before relying on this as a stable contract.
   */
...
/**
 * Render app-wide plugin UI outside ARC's layout regions.
 *
 ...
 * or creates a React portal. ARC supplies no chrome, positioning, visibility,
...
export type PluginFixedTabRegistration<Target extends JsonValue = never> =
  ExperimentalPluginFixedTabReference<Target> & {
    title: string;
    /** Icon hint (ARC icon name); unknown names fall back to a generic icon. */
    icon: string;
...
export interface PluginNavPanelRegistration {
   ...
   * ARC owns selection and persistence and always includes its native Browser
...
export interface PluginThreadPanelActionRegistration {
  ...
  title: string;
  /**
   * Icon hint (ARC icon name) used when the plugin ships no logo; the
...
export interface PluginNewThreadPanelActionRegistration {
  ...
  /** Label of the action row in the panel's new-tab launcher. */
  title: string;
  /** Icon hint (ARC icon name) used when the plugin ships no logo. */
  icon?: string;
  /** Rendered inside every panel tab this action opens. */
...
export interface ExperimentalSidebarFooterItemBase {
  ...
  label: string;
  /** ARC icon-name hint; unknown names fall back to a generic icon. */
  icon: string;
}
 ...
 * needs. `attention` is ARC's rolled-up "does this need you" signal, so a row
...
/**
 * The active code theme as a VS Code theme file: the same document ARC's own
...
export interface PluginCodeThemeData {
  /** Registered theme name — a bundled Shiki name or an ARC-registered id. */
  name: string;
  type: "light" | "dark";
...
/**
 * The code theme ARC is currently rendering with (see
...
export interface PluginSidebarThreadActions {
  /**
   * Navigate to a thread. `split: true` applies ARC's split placement rules —
  ...
  /**
   * Opens ARC's delete confirmation, which counts child threads first. Deletion
...
/** Core owns the Chat/view switch and URL; this component mounts only when selected. */
export interface ExperimentalThreadViewRegistration {
  id: string;
  label: string;
  component: ComponentType<ExperimentalThreadViewProps>;
}

/** Batched, plugin-owned metadata; cannot change core thread identity or navigation. */
export interface ExperimentalThreadListAnnotationsRegistration {
  id: string;
  component: ComponentType<ExperimentalThreadListAnnotationsProps>;
}
 ...
 * ARC's list or a specific provider under Settings → Appearance. A plugin can
 * also use its own setting and render `Original` conditionally.
 * An absent or crashing replacement falls back to ARC's list rather than
 ...
 * order. The user can pin ARC's preview or a specific opener per extension
...
/**
 * Replace ARC's source-code renderer everywhere it renders supplied source
 ...
 * order wins. A missing, disabled, or crashing replacement falls back to ARC's
...
/**
 * Replace ARC's diff renderer everywhere it renders supplied diff content — the
...
/**
 * A row in ARC's quick palette (Mod+Shift+P), listed under the plugin's name
 * beside ARC's own commands. Host-rendered: the plugin supplies a title and
...
export interface PluginAppSlots {
  ...
  experimental_threadView(
    registration: ExperimentalThreadViewRegistration,
  ): void;
  experimental_threadListAnnotations(
    registration: ExperimentalThreadListAnnotationsRegistration,
  ): void;
  fileOpener(registration: PluginFileOpenerRegistration): void;
  /**
   * Replace ARC's source-code renderer (see
  ...
  /**
   * Replace ARC's diff renderer (see
...
export interface ComposerPlusMenuItem {
  id: string;
  label: string;
  /** ARC icon name; unknown names fall back to the generic plugin icon. */
  icon?: string;
...
export interface PluginComposerThreadRowStatus {
  /** ARC icon-name hint; unknown names fall back to the generic plugin icon. */
  icon: string;
...
export interface PluginComposerApi {
   ...
   * Failures of the underlying request are reported by ARC's own submit error
 ...
 * engine), rendered by the ARC app inside a plugin slot. This is the
 ...
 * empty catalogs leave `value` unchanged. Omit `routing` to use ARC's
...
/** Props of ARC's controlled, host-resolved permission-mode picker. */
...
export interface NewThreadRequest {
  experimental_addressing?: import("@bb/domain").ExperimentalAddressing;
  /**
   * The selected project id. Choosing "Don't work in a project" submits ARC's
...
/**
 * Props of the host-owned `experimental_NewThreadComposer` component — ARC's
 ...
 * rendered by the ARC app inside a plugin slot.
 *
...
/**
 * Props of the host-owned `Markdown` component — ARC's chat message renderer
...
/**
 * Props for ARC's semantic URL link. The host owns ordinary activation while
...
/** Options shared by ARC's preview and preferred-external file intents. */
...
/**
 * Props for ARC's host-rendered semantic file link. Valid targets receive a
...
export interface BbNavigate {
  ...
  /**
   * Open an HTTP(S) URL using this client's ARC browser preference. Returns
   * false for schemes the host does not own. Experimental: see
   * docs/api_to_audit.md.
   */
  openUrl(url: string): boolean;
  /** Open a live file in this surface's shared ARC preview panel. */
...
// ARC registry (`npx shadcn add @bb/<name>`) and own it. `bb plugin build`
...
/**
 * Everything `@get-bb/plugin-sdk/app` resolves to at runtime. The ARC app builds
...
export interface PluginSdkApp {
   ...
   * engine of its own and needs ARC's palette to reach it. Experimental: see
   * docs/api_to_audit.md.
   */
  ...
  /**
   * A real anchor whose ordinary HTTP(S) activation uses ARC's URL preference.
   * Experimental: see docs/api_to_audit.md.
   */
  ...
  /**
   * ARC's controlled provider/model/reasoning picker. Provider changes emit
  ...
  /**
   * ARC's controlled permission-mode picker. The host resolves provider
   ...
   * supplied source text with ARC's syntax highlighting, gutters, and live code
   ...
   * content with ARC's normalization, optional full-file context expansion,
