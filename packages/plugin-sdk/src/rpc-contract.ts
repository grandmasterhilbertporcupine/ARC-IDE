 ...
 * the same without becoming part of ARC's public protocol.
 */
...
export interface ExperimentalInternalRpcContext {
  readonly callerPluginId: string;
  readonly signal: AbortSignal;
}

export type ExperimentalInternalRpcHandlers<
  Contract extends PluginRpcContract,
> = {
  [Method in keyof Contract]: (
    input: StandardSchemaV1InferOutput<Contract[Method]["input"]>,
    context: ExperimentalInternalRpcContext,
  ) =>
    | StandardSchemaV1InferInput<Contract[Method]["output"]>
    | Promise<StandardSchemaV1InferInput<Contract[Method]["output"]>>;
};

export interface ExperimentalInternalRpcClient<
  Contract extends PluginRpcContract,
> {
