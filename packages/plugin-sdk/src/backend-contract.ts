...
import type { ExperimentalPreparedThreads } from "./prepared-threads-contract.js";
import type {
  ExperimentalPreparedTurns,
  ExperimentalOwnedTurnIdentity,
  ExperimentalToolInvocation,
} from "./prepared-turns-contract.js";
import type {
 ...
 *
 * Types only: the implementation lives in the ARC server
 ...
 * "@get-bb/plugin-sdk"`); the import is erased when ARC loads the file.
 *
...
export interface PluginHttp {
   ...
   * - "local": Origin/Host must be a local ARC app origin; non-GET requires
...
export interface PluginRpc {
  ...
  experimental_registerInternal<Contract extends PluginRpcContract>(
    contract: Contract,
    handlers: ExperimentalInternalRpcHandlers<Contract>,
  ): void;
  experimental_client<Contract extends PluginRpcContract>(args: {
    pluginId: string;
    contract: Contract;
  }): ExperimentalInternalRpcClient<Contract>;
}
...
export interface PluginAgentToolContext {
  threadId: string;
  projectId: string;
  /** Native invocation identity. Null is reserved for explicitly non-native
   * test/host calls. Native tool requests fail retryably until ownership can
   * be resolved; ownedTurn null means the native turn is proven unowned. */
  experimental_invocation?: ExperimentalToolInvocation | null;
...
export interface PluginAgentToolRegistrationBase {
   ...
   * ARC shows the standard tool name (`Running <name>` / `Ran <name>`) and the
   * plugin's branding glyph. Approval, error, and interruption states keep
   * ARC's standard rendering. See docs/api_to_audit.md.
   */
...
export interface PluginAgentConfigurationContext {
  /** Immutable context for an admitted turn in an existing conversation.
   * Only its owner receives this value. It does not change thread origin or
   * permanent execution context. Owner configuration must succeed. */
  experimental_turnContext?: ExperimentalOwnedTurnIdentity | null;
  thread: {
    id: string;
    title: string | null;
    parentThreadId: string | null;
    sourceThreadId: string | null;
    /** Immutable context reference supplied on spawn, persisted before first
     * dispatch. Present only for this thread's owning plugin; other plugins
     * receive null. Resolve it synchronously and throw if missing or invalid.
     * A bound owner's configuration failure stops dispatch. */
    experimental_executionContextId?: string | null;
  };
  project: {
    id: string;
...
export interface PluginAgentConfiguration {
  /** Immutable skill trees supplied by the owning execution-context plugin. Shared skills remain available; these snapshots take precedence by name. */
  experimental_skillSnapshots?: import("./assigned-skill-contract.js").ExperimentalAssignedSkillSnapshot[];
  ...
  skills: string[];
  /** Optional dynamic instructions. Legacy contributions are truncated to
   * 4096 characters. A context-bound owning plugin may return up to 65,536
   * characters; exceeding that limit fails dispatch instead of truncating. */
  instructions?: string;
}
...
/**
 * Permission modes a provider can run a session in — ARC's own permission
 ...
 * slash-command typeahead is universal — ARC injects skills into every
...
export interface PluginProviderCapabilities {
  ...
  /** The provider keeps its own thread archive, so ARC mirrors archive and
   * unarchive onto it instead of tracking the state only in ARC's own rows. */
  supportsThreadArchive: boolean;
  /** The provider stores a thread name of its own, so ARC forwards renames to
   * it. */
...
export interface PluginProviderOptionsContext {
  ...
  model: string;
  /** ARC's permission mode for this command (already clamped to the host). */
   ...
   * mode is an ARC prompt mode, so the bridge maps it onto whatever the agent
   * calls it natively.
   */
  promptMode?: "plan";
  /**
...
/**
 * One provider this plugin contributes to ARC's provider registry.
 *
 ...
 * artifact ARC ships to hosts. Declaring a provider in a plugin with no
...
export interface PluginAgents {
   ...
   * or more than 256 tool/skill ids fails closed for this plugin only. For a
   * thread with `experimental_executionContextId`, its owning plugin must be
   * enabled and provide a successful callback; owner failures abort dispatch.
   * Resolve and validate the immutable context against `project.id` and
   * `thread.id`, and return its instructions here before the first provider
   * call. The opaque identifier is not an authorization credential. It is not
   * inherited by forks or children and cannot be changed by thread updates.
   *
   ...
   * next constructed. Skill changes follow ARC's environment runtime policy:
...
export interface PluginMentionItem {
  ...
  subtitle?: string;
  icon?: string;
  /** A version-pinned recipient. Selecting it only changes the draft; dispatch occurs on Send. */
  experimental_recipient?: ExperimentalRecipientIdentity;
}

export interface ExperimentalAddressedDispatchContext {
  projectId: string;
  threadId: string;
  operationId: string;
  prompt: readonly PromptInput[];
  recipients: readonly ExperimentalAddressedRecipient[];
}

export type ExperimentalAddressedDispatchHandler = (
...
export interface PluginUi {
  /** Own explicit addressed sends for this plugin. Validate recipient scope, pinned revisions, and operation idempotency before effects. A successful result suppresses ordinary provider dispatch. */
  experimental_registerAddressedDispatch(
    handler: ExperimentalAddressedDispatchHandler,
  ): void;
...
export interface PluginServerApi {
  ...
  /**
   * This ARC server's own loopback base URL (e.g. "http://127.0.0.1:38886"),
...
/**
 * What a plugin's AI service does. `inference` answers ARC's server-side helper
 ...
 * the ARC server; this contract is what plugin `server.ts` files compile
 * against.
 */
export interface BbPluginApi {
  ...
  /** Durable owner-bound workers with a sealed first turn and explicit release. */
  readonly experimental_threads: ExperimentalPreparedThreads;
  /** Prepare an immutable, idle-only turn in an existing conversation. The
   * owning plugin must retain its execution context before preparation and
   * explicitly release it. Observations retain exact dispatch/native IDs;
   * an allocated request is never replaced under the same operation. */
  readonly experimental_turns: ExperimentalPreparedTurns;
  ...
  /**
   * The full ARC SDK, bound to this server over loopback (design §4.1).
   ...
   * An `experimental_executionContextId` additionally requires this plugin's
   * ownership; specifying another origin or owner is rejected. Persist the
   * immutable context before spawning, then validate it in `agents.configure`.
   */
  readonly sdk: BbSdk;
  /**
