...
export interface CollectedPluginAppRegistrations {
  ...
  threadViews: ExperimentalThreadViewRegistration[];
  threadListAnnotations: ExperimentalThreadListAnnotationsRegistration[];
 ...
 * Both the ARC app and the public test harness use this implementation so a
...
export function collectPluginAppRegistrations(
  ...
  const seenIds = {
    ...
    threadView: new Set<string>(),
    threadListAnnotations: new Set<string>(),
  ...
  definition.setup({
    slots: {
      ...
      experimental_threadView(registration) {
        const kind = "slots.experimental_threadView";
        const id = requireSlotId(kind, registration?.id);
        requireUniqueId(kind, seenIds.threadView, id);
        collected.threadViews.push({
          id,
          label: requireNonEmptyString(kind, "label", registration.label),
          component: requireComponent(kind, registration.component),
        });
      },
      experimental_threadListAnnotations(registration) {
        const kind = "slots.experimental_threadListAnnotations";
        const id = requireSlotId(kind, registration?.id);
        requireUniqueId(kind, seenIds.threadListAnnotations, id);
        collected.threadListAnnotations.push({
          id,
          component: requireComponent(kind, registration.component),
        });
      },
