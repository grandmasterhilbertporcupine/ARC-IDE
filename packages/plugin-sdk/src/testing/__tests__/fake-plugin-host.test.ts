...
describe("fixtures", () => {
  it("keeps internal transport private, owner-bound and withdrawn on disposal", async () => {
    const target = createFakePluginHost({ pluginId: "target" });
    const caller = createFakePluginHost({
      pluginId: "caller",
      experimental_internalRpc: (args) =>
        target.harness.behavior.experimental_callInternalRpc(
          args.method,
          args.input,
          args.context,
        ),
    ...
    const contract = defineRpcContract({
      owner: { input: z.string(), output: z.string() },
    });
    target.bb.rpc.experimental_registerInternal(contract, {
      owner: (input, context) => `${context.callerPluginId}:${input}`,
    });
    const client = caller.bb.rpc.experimental_client({
      pluginId: "target",
      contract,
    });
    expect(await client.call("owner", "work")).toBe("caller:work");
    await expect(
      target.harness.behavior.callRpc("owner", "work"),
    ).rejects.toThrow();
    await caller.harness.dispose();
    await expect(client.call("owner", "work")).rejects.toThrow();
    await target.harness.dispose();
  });
...
describe("sdk", () => {
  it("rejects execution context attribution to another plugin before invoking the SDK", async () => {
    const { bb, harness } = createFakePluginHost({
      pluginId: "arc",
      sdk: { threads: { spawn: async () => ({ id: "bound" }) } },
    });
    const request = {
      projectId: "p1",
      prompt: "test",
      environment: { type: "project-default" as const },
      experimental_executionContextId: "ctx-1",
    };
    expect(() =>
      bb.sdk.threads.spawn({ ...request, originPluginId: "other" }),
    ).toThrow("owned by the spawning plugin");
    expect(() => bb.sdk.threads.spawn({ ...request, origin: "sdk" })).toThrow(
      "owned by the spawning plugin",
    );
...
describe("agent tools", () => {
  it("accepts pinned skills only from the bound execution owner", async () => {
    const skills = [
      {
        name: "reader",
        description: "Read relevant sections.",
        files: [
          {
            path: "SKILL.md",
    ...
    const owner = createFakePluginHost({ pluginId: "arc" });
    owner.bb.agents.configure(() => ({
      tools: [],
      skills: [],
      experimental_skillSnapshots: skills,
    }));
    const bound = makePluginAgentConfigurationContext({
      origin: { pluginId: "arc" },
      thread: { experimental_executionContextId: "ctx-1" },
    });
    expect(
      (await owner.harness.resolveAgentConfiguration(bound))
        .experimental_skillSnapshots,
    ).toEqual(skills);
    const unbound = await owner.harness.resolveAgentConfiguration(
      makePluginAgentConfigurationContext(),
    );
    expect(unbound.experimental_skillSnapshots).toBeUndefined();
  ...
  it("delivers a bound context only to its owner and rejects incomplete owner configuration", async () => {
    const bound = makePluginAgentConfigurationContext({
      origin: { pluginId: "arc" },
      thread: { experimental_executionContextId: "ctx-1" },
    });
    const { bb, harness } = createFakePluginHost({ pluginId: "arc" });
    await expect(harness.resolveAgentConfiguration(bound)).rejects.toThrow(
      "configuration is unavailable",
    );
    bb.agents.configure((ctx) => {
      expect(ctx.thread.experimental_executionContextId).toBe("ctx-1");
      return { tools: [], skills: [], instructions: "x".repeat(65_536) };
    });
    const result = await harness.resolveAgentConfiguration(bound);
    expect(result.instructions).toHaveLength(65_536);
    const other = createFakePluginHost({ pluginId: "other" });
    other.bb.agents.configure((ctx) => {
      expect(ctx.thread.experimental_executionContextId).toBeNull();
      return { tools: [], skills: [], instructions: "x".repeat(5_000) };
    });
    expect(
      (await other.harness.resolveAgentConfiguration(bound)).instructions,
    ).toHaveLength(4_096);
  });

  it("rejects failed and oversized bound configuration instead of silently omitting instructions", async () => {
    const bound = makePluginAgentConfigurationContext({
      origin: { pluginId: "arc" },
      thread: { experimental_executionContextId: "ctx-1" },
    });
    const oversized = createFakePluginHost({ pluginId: "arc" });
    oversized.bb.agents.configure(() => ({
      tools: [],
      skills: [],
      instructions: "x".repeat(65_537),
    }));
    await expect(
      oversized.harness.resolveAgentConfiguration(bound),
    ).rejects.toThrow("65536-character limit");
    const failed = createFakePluginHost({ pluginId: "arc" });
    failed.bb.agents.configure(() => {
      throw new Error("Unknown immutable snapshot");
    });
    await expect(
      failed.harness.resolveAgentConfiguration(bound),
    ).rejects.toThrow("Unknown immutable snapshot");
  });
