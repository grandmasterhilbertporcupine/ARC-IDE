...
} from "../assigned-skill-contract.js";
...
import {
  ...
  PLUGIN_AGENT_BOUND_INSTRUCTIONS_MAX_CHARS,
...
/**
 * `createFakePluginHost` — an in-process stand-in for the ARC server's plugin
...
export interface FakePluginRegistrations {
  ...
  addressedDispatch:
    | import("../backend-contract.js").ExperimentalAddressedDispatchHandler
    | null;
...
/** Deterministic inputs that stand in for behavior normally driven by ARC. */
export interface FakePluginBehaviorDrivers {
  experimental_callInternalRpc(
    method: string,
    input: unknown,
    context: import("../rpc-contract.js").ExperimentalInternalRpcContext,
  ): Promise<unknown>;
   ...
   * selected. Callback failures are logged and return empty selections, except
   * bound owner failures which reject as they do on the server. */
  resolveAgentConfiguration(context: PluginAgentConfigurationContext): Promise<{
    tools: FakeAgentToolRecord[];
    skills: string[];
    experimental_skillSnapshots?: ExperimentalAssignedSkillSnapshot[];
    instructions: string | null;
  }>;
  resolveProviderEnv(
    providerId: string,
...
export interface CreateFakePluginHostOptions {
  experimental_preparedThreads?: import("../prepared-threads-contract.js").ExperimentalPreparedThreads;
  experimental_preparedTurns?: import("../prepared-turns-contract.js").ExperimentalPreparedTurns;
...
function normalizeAgentConfiguration(args: {
  ...
  skillIds: string[];
  assignedSkillSnapshots: ExperimentalAssignedSkillSnapshot[];
  instructions: string | null;
} {
  if (
  ...
  if (
    args.boundExecutionContext &&
    typeof output.instructions === "string" &&
    output.instructions.length > PLUGIN_AGENT_BOUND_INSTRUCTIONS_MAX_CHARS
  ) {
    throw new Error(
      `Execution-context instructions exceed the ${PLUGIN_AGENT_BOUND_INSTRUCTIONS_MAX_CHARS}-character limit`,
    );
  }
  ...
  if (
    output.experimental_skillSnapshots !== undefined &&
    !args.boundExecutionContext
  )
    throw new Error(
      "Only the owning execution-context plugin may assign skill snapshots",
    );
  return {
    assignedSkillSnapshots: experimental_assignedSkillSnapshotsSchema.parse(
      output.experimental_skillSnapshots ?? [],
    ),
...
function createFakePluginHostInternal(
  ...
  const internalRpcHandlers = new Map<string, FakeRpcRecord>();
  const internalLifecycle = new AbortController();
  function registerRpc(
    contract: import("../rpc-contract.js").PluginRpcContract,
    handlers: object,
    target: Map<string, FakeRpcRecord>,
  ): void {
    assertLive();
    if (
    ...
    for (const [name, contractValue] of contractEntries) {
      ...
      pending.push([
        name,
        {
          ...
          handler: handler as FakeRpcRecord["handler"],
        },
      ]);
    }
  ...
  const rpc: PluginRpc = {
    register(contract, handlers) {
      registerRpc(contract, handlers, rpcHandlers);
    },
    experimental_registerInternal(contract, handlers) {
      registerRpc(contract, handlers, internalRpcHandlers);
    },
    experimental_client({ pluginId: targetId, contract }) {
      assertLive();
      return {
        async call(method, input, callOptions) {
          assertLive();
          const signal = AbortSignal.any([
            internalLifecycle.signal,
            ...(callOptions?.signal ? [callOptions.signal] : []),
          ]);
          signal.throwIfAborted();
          const schema = readRpcMethodContract(method, contract[method]);
          const parsed = await validateRpcValue(schema.input, input, "input");
          assertLive();
          signal.throwIfAborted();
          if (!options.experimental_internalRpc)
            throw new Error("No fake internal RPC transport configured");
          const result = await options.experimental_internalRpc({
            pluginId: targetId,
            method,
            input: normalizeRpcJsonResult(parsed),
            context: { callerPluginId: pluginId, signal },
          });
          const output = await validateRpcValue(
            schema.output,
            result,
            "output",
          );
          ...
          return normalizeRpcJsonResult(
            output,
          ) as import("../rpc-contract.js").PluginRpcResult<
            (typeof contract)[typeof method]
          >;
        },
      };
    },
  ...
  let addressedDispatch:
    | import("../backend-contract.js").ExperimentalAddressedDispatchHandler
    | null = null;
  const ui: PluginUi = {
    experimental_registerAddressedDispatch(handler) {
      assertLive();
      if (addressedDispatch !== null)
        throw new Error("Addressed dispatch is already registered");
      if (typeof handler !== "function")
        throw new Error("Addressed dispatch requires a handler");
      addressedDispatch = handler;
    },
    requestInput,
  ...
  const bb: BbPluginApi = {
    ...
    rpc,
    experimental_turns: {
      prepare(request, callOptions) {
        assertLive();
        if (!options.experimental_preparedTurns)
          throw new Error("No fake prepared turn implementation configured");
        return options.experimental_preparedTurns.prepare(request, callOptions);
      },
      getPreparation(request, callOptions) {
        assertLive();
        if (!options.experimental_preparedTurns)
          throw new Error("No fake prepared turn implementation configured");
        return options.experimental_preparedTurns.getPreparation(
          request,
          callOptions,
        );
      },
      startPrepared(request, callOptions) {
        assertLive();
        if (!options.experimental_preparedTurns)
          throw new Error("No fake prepared turn implementation configured");
        return options.experimental_preparedTurns.startPrepared(
          request,
          callOptions,
        );
      },
      interrupt(request, callOptions) {
        assertLive();
        if (!options.experimental_preparedTurns)
          throw new Error("No fake prepared turn implementation configured");
        return options.experimental_preparedTurns.interrupt(
          request,
          callOptions,
        );
      },
    ...
    experimental_threads: {
      prepare(request, callOptions) {
        assertLive();
        if (!options.experimental_preparedThreads)
          throw new Error("No fake prepared thread implementation configured");
        return options.experimental_preparedThreads.prepare(
          request,
          callOptions,
        );
      },
      getPreparation(request, callOptions) {
        ...
        if (!options.experimental_preparedThreads)
          throw new Error("No fake prepared thread implementation configured");
        return options.experimental_preparedThreads.getPreparation(
          request,
          callOptions,
        );
      },
      startPrepared(request, callOptions) {
        ...
        if (!options.experimental_preparedThreads)
          throw new Error("No fake prepared thread implementation configured");
        return options.experimental_preparedThreads.startPrepared(
          request,
          callOptions,
        );
      },
  ...
  async function disposeHost(cleanupStorage: boolean): Promise<void> {
    if (disposed) return;
    internalLifecycle.abort(new Error("Plugin disposed"));
    disposed = true;
  ...
  const harness: FakePluginHarness = {
    ...
    registrations: {
      ...
      mentionProviders,
      get addressedDispatch() {
        return addressedDispatch;
      },
    ...
    async experimental_callInternalRpc(method, input, context) {
      assertLive();
      const record = internalRpcHandlers.get(method);
      if (!record) throw new Error(`Unknown internal RPC method ${method}`);
      const signal = AbortSignal.any([
        internalLifecycle.signal,
        context.signal,
      ]);
      const parsed = await validateRpcValue(
        record.inputSchema,
        jsonRoundTrip(input, "internal RPC input"),
        "input",
      );
      assertLive();
      signal.throwIfAborted();
      const result = await record.handler(parsed as never, {
      ...
      assertLive();
      signal.throwIfAborted();
      return normalizeRpcJsonResult(output);
    },
    ...
    async resolveAgentConfiguration(context) {
      const executionContextId =
        context.thread.experimental_executionContextId ?? null;
      const boundExecutionContext =
        executionContextId !== null && context.origin.pluginId === pluginId;
      if (agentConfigurationProvider === null) {
        if (boundExecutionContext)
          throw new Error(
            "Execution-context owner configuration is unavailable",
          );
        return {
          tools: [...agentTools],
      ...
      try {
        const normalized = normalizeAgentConfiguration({
          ...
          pluginId,
          value: agentConfigurationProvider({
            ...context,
            thread: {
              ...context.thread,
              experimental_executionContextId: boundExecutionContext
                ? executionContextId
                : null,
            },
          }),
        ...
        return {
          ...
          ...(normalized.assignedSkillSnapshots.length
            ? { experimental_skillSnapshots: normalized.assignedSkillSnapshots }
            : {}),
          instructions: normalized.instructions,
        };
      } catch (error) {
        emitLog("warn", `agent configure failed: ${errorMessage(error)}`);
        if (boundExecutionContext) throw error;
