...
The typed facade ARC plugin authors compile against. The root preserves the
complete `BbPluginApi` and `BbSdk` contract; `./app` is the frontend runtime
that `bb plugin build` replaces with ARC's shared implementation.
...
Use `UrlLink` for a real anchor that applies ARC's current
...
newly opened page cannot control ARC by accident. The frontend harness records
...
thread-storage file. Ordinary activation opens the shared ARC preview and its
...
app-wide floating React UI. ARC mounts the component once per app window through
...
imports ARC workspace packages or source TypeScript at runtime. Install the SDK
...
and disposal order. HTTP runs through Hono but does not enforce ARC's local or
...
scope, quote, mention, focus, and clear behavior. It does not reproduce ARC
layout, CSS, persistence, routing, host authentication, crash boundaries, or
multi-plugin arbitration; use a live ARC test for those boundaries.

## Declaration surface

The complete root declaration flattens the unpublished ARC workspace contracts.
