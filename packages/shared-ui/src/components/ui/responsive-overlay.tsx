...
const PERSISTENT_DRAWER_EASING = "cubic-bezier(0.2, 0, 0, 1)";
...
export function PersistentResponsiveDrawerShell({
  ...
  const durationMs = prefersReducedMotion ? 0 : motionDurationMs;
  const transition = `transform ${durationMs}ms ${PERSISTENT_DRAWER_EASING}`;
  const backdropTransition = `opacity ${durationMs}ms ${PERSISTENT_DRAWER_EASING}`;
  ...
  React.useEffect(() => {
    ...
    const timeout = window.setTimeout(
      () => reportSettled(open),
      durationMs === 0 ? 0 : durationMs + 50,
    );
    return () => window.clearTimeout(timeout);
  }, [durationMs, open, reportSettled]);
