...
export function listLastRequestedModelsByThreadIds(
  db: DbQueryConnection,
      ...
      db
        .select({
          ...
          model: sql<string | null>`(
            SELECT CASE
              WHEN json_type(request.data, '$.execution.model') = 'text'
                AND length(json_extract(request.data, '$.execution.model')) > 0
              THEN json_extract(request.data, '$.execution.model')
              ELSE NULL
            END
            FROM events AS request
            WHERE request.thread_id = threads.id
              AND (
                request.type = 'client/turn/requested'
                OR (
                  request.type IN ('client/thread/start', 'client/turn/start')
                  AND json_type(request.data, '$.input') IS NOT NULL
                )
              )
