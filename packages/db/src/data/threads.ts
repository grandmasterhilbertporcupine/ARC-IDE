...
import { cancelThreadPreparation } from "./thread-preparations.js";
import type {
    ...
    (tx) => {
      const createdThread = tx
        ...
        .values({
          ...
          experimental_executionContextId: input.experimental_executionContextId ?? null,
          visibility,
          lastReadAt: now,
  ...
  return db.all<ThreadSearchMatchRow>(sql`
    ...
    ranked_threads AS (
      ...
      WHERE t.deleted_at IS NULL
        ...
        ${args.projectId === undefined ? sql`` : sql`AND t.project_id = ${args.projectId}`}
      GROUP BY threadId
...
export function deleteThread(
  ...
  cancelThreadPreparation(db, id, "Prepared thread was deleted");
  const existing = db.select().from(threads).where(eq(threads.id, id)).get();
...
export function markThreadDeleted(
  ...
  cancelThreadPreparation(db, args.threadId, "Prepared thread was deleted");
  const updated = db
    .update(threads)
    .set({
