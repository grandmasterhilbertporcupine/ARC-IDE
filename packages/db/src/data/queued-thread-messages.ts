...
import { assertPreparedThreadMutable, getThreadPreparation } from "./thread-preparations.js";
import { assertPreparedTurnQueueMutable, assertPreparedTurnRequestMutable, getOpenThreadTurnPreparation, getPreparedTurnQueueIds, preparedTurnQueueCanDispatch } from "./thread-turn-preparations.js";
...
function partitionQueuedMessageGroups(
  ...
  const ownedIds = getPreparedTurnQueueIds(db, queuedMessages.map(row=>row.id));
  ...
  while (index < queuedMessages.length) {
    ...
    let current: QueuedThreadMessageRow[] = [];
    for (const row of queuedMessages.slice(index, index + size)) {
...
export function createQueuedThreadMessageInTransaction(
  ...
  admission?: { preparedOperationId: string } | { preparedTurn: {ownerPluginId:string;operationId:string} },
) {
  if (admission && "preparedTurn" in admission) {
    const preparation = getOpenThreadTurnPreparation(tx,input.threadId);
    if (preparation?.ownerPluginId !== admission.preparedTurn.ownerPluginId || preparation.operationId !== admission.preparedTurn.operationId || preparation.state !== "prepared" || preparation.queuedMessageId !== null) throw new Error("Preparation cannot admit this turn queue row");
  } else if (admission) {
    const preparation = getThreadPreparation(tx, input.threadId);
    if (preparation?.operationId !== admission.preparedOperationId || preparation.state !== "prepared" || preparation.queuedMessageId !== null) throw new Error("Preparation cannot admit this queue row");
  } else {
    assertPreparedThreadMutable(tx, input.threadId);
  }
  if (input.payload.kind === "retry") assertPreparedTurnRequestMutable(tx,input.payload.retryOfTurnRequestId);
  const now = Date.now();
...
export function updateQueuedThreadMessage(
  ...
  const result = db.transaction(
    (tx): UpdateQueuedThreadMessageResult => {
      assertPreparedThreadMutable(tx, input.threadId);
      assertPreparedTurnQueueMutable(tx, input.id);
...
export function claimQueuedThreadMessage(
  ...
  const claimedQueuedMessage = db.transaction(
    (tx) => {
      assertPreparedTurnQueueMutable(tx, id);
      const existing = tx
        .select()
        .from(queuedThreadMessages)
        .where(eq(queuedThreadMessages.id, id))
...
function claimQueuedThreadMessageIdsInTransaction(
  ...
  if (ids.length === 0 || !preparedTurnQueueCanDispatch(tx,ids)) return null;
...
export function claimQueuedThreadMessageGroup(
  ...
  const claimedQueuedMessages = db.transaction(
    (tx) => {
      ...
      const group =
        partitionQueuedMessageGroups(tx, queuedMessages).find((rows) =>
          rows.some((row) => row.id === id),
        ) ?? null;
      if (group === null) {
        return null;
      }
      if (policy.kind === "explicit-send") for (const row of group) assertPreparedTurnQueueMutable(tx,row.id);
      if (
...
export function reorderQueuedThreadMessage({
  ...
  for (const id of [queuedMessageId,previousQueuedMessageId,nextQueuedMessageId,groupBoundaryQueuedMessageId]) if (id !== null && id !== undefined) assertPreparedTurnQueueMutable(db,id);
...
export function setQueuedThreadMessageGroupBoundary({
  ...
  assertPreparedThreadMutable(db, threadId);
  for (const id of [...expectedGroupedPrefixQueuedMessageIds,groupBoundaryQueuedMessageId]) if (id !== null) assertPreparedTurnQueueMutable(db,id);
  const result = db.transaction(
    (tx) =>
...
export function deleteQueuedThreadMessage(
  ...
  const existing = db.transaction(
    (tx) => {
      ...
      assertPreparedThreadMutable(tx, existing.threadId);
      assertPreparedTurnQueueMutable(tx, id);
