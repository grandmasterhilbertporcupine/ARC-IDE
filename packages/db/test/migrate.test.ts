...
const executionContextMigrationWhen = 1789009163396;
...
function dropExecutionContextColumn(db: DbConnection): void {
  db.$client.exec("DROP TABLE IF EXISTS thread_turn_preparations");
  db.$client.exec("DROP TABLE IF EXISTS thread_preparations");
  const columns = db.$client
    .prepare<[], TableInfoRow>("PRAGMA table_info(threads)")
    .all();
  if (columns.some((column) => column.name === "execution_context_id")) {
    db.$client.exec("ALTER TABLE threads DROP COLUMN execution_context_id");
  }
}

function dropSurfaceStyleColumn(db: DbConnection): void {
  const columns = db.$client
    .prepare<[], TableInfoRow>("PRAGMA table_info(app_theme)")
    .all();
  if (columns.some((column) => column.name === "surface_style")) {
...
describe("migrate", () => {
  it("adds execution context to an existing database without changing existing threads", () => {
    ...
    try {
      ...
      const { project } = createProject(db, noopNotifier, {
        ...
        source: { type: "local_path", hostId: host.id, path: "/tmp/existing" },
      });
      ...
      db.$client
        ...
        .run(executionContextMigrationWhen);
      dropSurfaceStyleColumn(db);
      migrate(db);
      expect(getThread(db, existing.id)).toMatchObject({
        id: existing.id,
        title: "Existing thread",
        projectId: project.id,
      ...
      const bound = createThread(db, noopNotifier, {
        ...
        providerId: "codex",
        originPluginId: "arc",
        experimental_executionContextId: "snapshot-after-upgrade",
      });
      migrate(db);
      expect(getThread(db, bound.id)?.experimental_executionContextId).toBe(
    ...
    try {
      db.$client.exec(`
        ...
        INSERT INTO marketplaces VALUES
          ('bb-official', 'git', 'https://github.com/ymichael/bb.git', 'main', '{"schemaVersion":1,"name":"bb-official","displayName":"Included with ARC","plugins":[]}', 10, 20, NULL, 1, 20),
