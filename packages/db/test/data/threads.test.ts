...
describe("threads", () => {
  it("persists execution context before notifying creation and retains it across thread updates", () => {
    const { db, project } = setup();
    const contexts: Array<string | null> = [];
    const notifier: DbNotifier = {
      ...noopNotifier,
      notifyThread(threadId) {
        contexts.push(getThread(db, threadId)?.experimental_executionContextId ?? null);
      },
    };
    try {
      const thread = createThread(db, notifier, {
        projectId: project.id,
        providerId: "codex",
        originPluginId: "arc",
        experimental_executionContextId: "ctx-revision-1",
        status: "pending",
      });
      expect(contexts).toEqual(["ctx-revision-1"]);
      updateThread(db, noopNotifier, thread.id, { title: "Renamed" });
      expect(getThread(db, thread.id)?.experimental_executionContextId).toBe("ctx-revision-1");
      const ordinary = createThread(db, noopNotifier, { projectId: project.id, providerId: "codex" });
      expect(ordinary.experimental_executionContextId).toBeNull();
    } finally {
      db.$client.close();
    }
  });
