...
describe("thread search data", () => {
  it("scopes content matches before group totals and limits while retaining global search", () => {
    const { db, project } = setup();
    try {
      const host = upsertHost(db, noopNotifier, { name: "other", type: "persistent" });
      const other = createProject(db, noopNotifier, {
        name: "other",
        source: { type: "local_path", hostId: host.id, path: "/tmp/other" },
      }).project;
      const own = Array.from({ length: 3 }, () => createThread(db, noopNotifier, {
        projectId: project.id, providerId: "codex", title: "Unrelated title",
      }));
      upsertThreadSearchSegments(db, {
        segments: own.map((thread) => ({
          threadId: thread.id, sourceKind: "user_message", sourceKey: "1",
          sourceSeq: 1, text: "Needle in the conversation",
        })),
      });
      archiveThread(db, noopNotifier, own[2].id);
      for (let i = 0; i < 55; i++) {
        const foreign = createThread(db, noopNotifier, {
          projectId: other.id, providerId: "codex", title: "Needle",
        });
        if (i < 2) archiveThread(db, noopNotifier, foreign.id);
      }
      const scoped = searchThreadsWithPendingInteractionState(db, {
        query: "needle", limitPerGroup: 1, projectId: project.id,
      });
      expect(scoped.active.total).toBe(2);
      expect(scoped.archived.total).toBe(1);
      expect(scoped.active.results).toHaveLength(1);
      expect(scoped.archived.results[0]?.thread.id).toBe(own[2].id);
      expect(scoped.active.results[0]?.thread.projectId).toBe(project.id);
      expect(scoped.active.results[0]?.matches[0]?.sourceSeq).toBe(1);
      const global = searchThreadsWithPendingInteractionState(db, {
        query: "needle", limitPerGroup: 1,
      });
      expect(global.active.total).toBe(55);
      expect(global.archived.total).toBe(3);
      expect(searchThreadsWithPendingInteractionState(db, {
        query: "needle", limitPerGroup: 50, projectId: "missing",
      })).toEqual({ active: { total: 0, results: [] }, archived: { total: 0, results: [] } });
    } finally {
      closeConnection(db);
    }
  });
