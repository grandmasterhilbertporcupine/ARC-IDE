...
describe("skills/configure", () => {
  it("rejects required skill configuration when the bridge has not advertised support", () => {
    const adapter = makeAdapter();
    completeHandshake(adapter, {});
    expect(() =>
      adapter.buildCommandPlan({
        ...
        required: true,
        skillRoots: [{ id: "assigned", path: "/skills", skills: [] }],
      }),
    ).toThrow(/required_skills_unsupported/);
  });
