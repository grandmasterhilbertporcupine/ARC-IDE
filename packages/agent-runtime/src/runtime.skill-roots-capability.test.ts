...
describe("skills/configure handshake capability", () => {
  ...
  it("sends skills/configure before thread/start to a bridge that declares it", async () => {
    ...
    const runtime = withBridgeLaunch(
      createAgentRuntime({
        workspacePath,
        env: record.env,
        requiredSkillConfiguration: true,
        skillRoots: [
  ...
  it("refuses required skills before starting a thread on an unsupported bridge", async () => {
    const record = createScriptedEchoRequestRecord();
    const runtime = withBridgeLaunch(
      createAgentRuntime({
        workspacePath,
        ...
        skillRoots: [
          {
            id: "assigned",
            path: stageSkillRoot(),
            skills: [{ name: "demo", description: "Required role skill" }],
          },
        ],
        onEvent: () => {},
        onToolCall: async () => ({ contentItems: [], success: true }),
    ...
    ).rejects.toThrow(/required_skills_unsupported/);
    expect(runtime.hasThread("required")).toBe(false);
    expect(record.read().map((entry) => entry.method)).not.toContain(
      "thread/start",
    );
  });
});
