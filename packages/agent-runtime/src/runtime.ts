...
import { RuntimeTurnRequestState } from "./runtime-turn-request-state.js";
import type {
...
export function createAgentRuntime(options: AgentRuntimeOptions): AgentRuntime {
  ...
  if (options.requiredSkillConfiguration && skillRoots.length === 0)
    throw new Error(
      "required_skills_unavailable: No assigned skill roots were staged",
    );
  ...
  const turnRequestState = new RuntimeTurnRequestState();
  ...
  const providerProcesses = new RuntimeProviderProcessManager({
    ...
    onProviderThreadDetached: (threadId) => {
      turnRequestState.detach(threadId);
    ...
    skillRoots,
    requiredSkillConfiguration: options.requiredSkillConfiguration ?? false,
  ...
  function resolveProviderRequestThreadId(
    ...
    if (!resolvedThreadId) {
      sendJsonRpcError({
        child: args.proc.child,
        id: args.parsedId,
        message: `Unable to resolve ARC thread id for ${args.requestKind} on provider thread "${args.providerThreadId}"`,
      });
      return null;
    }
    if (args.threadIdHint && args.threadIdHint !== resolvedThreadId) {
      sendJsonRpcError({
        child: args.proc.child,
        id: args.parsedId,
        message: `${formatProviderRequestKindForSentence(args.requestKind)} thread hint "${args.threadIdHint}" did not match resolved ARC thread "${resolvedThreadId}" for provider thread "${args.providerThreadId}"`,
      });
      return null;
    }
  ...
  function emitTranslatedEvents(args: EmitTranslatedEventsArgs): void {
    ...
    for (const event of args.events) {
      ...
      turnRequestState.observe(normalizedEvent);
  ...
  const runtime: AgentRuntime = {
    ...
    async runTurn({
      ...
      return runThreadOperation({
        ...
        work: async () => {
          ...
          turnRequestState.begin(threadId, clientRequestId);
    ...
    async stopTurnIfCurrent(args) {
      let observed = turnRequestState.inspect(
        args,
        turnState.getActiveTurnId(args.threadId),
      );
      if (
        observed.status === "unknown" &&
        observed.clientRequestId === args.expectedClientRequestId &&
        pendingTurnStarts.has(args.threadId)
      ) {
        await turnState.waitForActiveTurn({
          threadId: args.threadId,
          timeoutMs: 5_000,
        });
        observed = turnRequestState.inspect(
          args,
          turnState.getActiveTurnId(args.threadId),
        );
      }
      if (observed.status !== "current")
        return { ...observed, providerCheckpointId: null };
      const result = await runtime.stopThread({ threadId: args.threadId });
      turnRequestState.settle(args, observed.turnId);
      return { ...observed, ...result, status: "stopped" };
    },
