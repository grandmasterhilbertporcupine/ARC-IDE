...
describe("@bb/sdk", () => {
  ...
  it("sends a complete appearance selection through the theme transport", async () => {
    const appearance = {
      ...
      surfaceStyle: "liquid-glass" as const,
      resolvedCodeTheme: {
        dark: "nord",
        light: "nord",
        files: {},
  ...
  it("preserves the favicon color and surface style for the theme-id shorthand", async () => {
    const current = {
      appearance: {
        themeId: "dracula",
        customCss: null,
  ...
  it("forwards project-scoped search and preserves requested-model projections", async () => {
    const thread = {
      id: "thr_123",
      lastRequestedModel: { providerId: "custom-provider", model: "model-x" },
    };
    const result = {
    ...
    expect((await sdk.threads.list())[0]?.lastRequestedModel).toEqual(
      thread.lastRequestedModel,
    );
    expect(
    ...
    await sdk.threads.search({ query: "needle" });
    expect(new URL(queue.requests[1].url).searchParams.get("projectId")).toBe(
      "proj_123",
    );
    expect(
      new URL(queue.requests[1].url).searchParams.get("limitPerGroup"),
    ).toBe("3");
    expect(new URL(queue.requests[2].url).searchParams.has("projectId")).toBe(
      false,
    );
  });
  ...
  it("sends immutable plugin execution context on the initial thread creation request", async () => {
    const queue = createFetchQueue([
      {
        body: { id: "thr_1", experimental_executionContextId: "ctx-1" },
        status: 201,
      },
    ]);
    const sdk = createBbSdk({
    ...
    expect(thread.experimental_executionContextId).toBe("ctx-1");
  });
