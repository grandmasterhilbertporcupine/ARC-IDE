...
export class BbRequestTimeoutError extends Error {
  constructor(timeoutMs: number) {
    super(
      `ARC request timed out after ${formatRequestTimeoutDuration(timeoutMs)}.`,
    );
...
export async function resolveResponse<TResponse extends SdkResponseLike>(
  ...
  try {
    ...
    if (isTypeErrorWithCauseCode(error, "ECONNREFUSED")) {
      throw new Error(
        "Cannot connect to ARC server. Ensure it is running and BB_SERVER_URL is correct.",
      );
    }
    throw error;
  }
...
function validateRequestTimeoutMs(timeoutMs: number): void {
  if (!Number.isFinite(timeoutMs) || timeoutMs < 0) {
    throw new RangeError(
      "ARC request timeout must be a non-negative finite number.",
    );
  }
}
