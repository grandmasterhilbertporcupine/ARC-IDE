...
export class BbRealtimeClient implements BbRealtime {
  ...
  private connectSocket(): Promise<void> {
    ...
    if (!websocketFactory) {
      throw new Error(
        "ARC SDK realtime requires a WebSocket implementation. Pass websocket when creating the transport.",
      );
    }
  ...
  private ensureSocketReadyPromise(): Promise<void> {
    ...
    if (!this.socketReadyPromise) {
      throw new Error("ARC SDK realtime socket readiness was not initialized.");
    }
