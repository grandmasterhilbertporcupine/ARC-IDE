...
export interface ThreadsArea {
  /** Retry the server-retained addressed Send with its original attachments, recipients and operation identity. Agent callers must preserve senderThreadId; owned workers cannot dispatch or retry addressed work. */
  experimental_retryAddressed(args: {
    threadId: string;
    operationId: string;
...
export function createThreadsArea(args: CreateSdkAreaArgs): ThreadsArea {
  ...
  return {
    ...
    async experimental_retryAddressed(input) {
      return transport.readJson(
        transport.api.v1.threads[":id"]["retry-addressed"].$post({
          param: { id: input.threadId },
          json: {
