...
export interface FilesArea {
  experimental_refreshPreview(args: {
    previewId: string;
    signal?: AbortSignal;
  }): Promise<RefreshFilePreviewResponse>;
...
export function createFilesArea(args: CreateSdkAreaArgs): FilesArea {
  ...
  return {
    experimental_refreshPreview: ({ previewId, signal }) =>
      transport.readJson(
        transport.api.v1["file-previews"][":id"].refresh.$post(
          { param: { id: previewId }, json: {} },
