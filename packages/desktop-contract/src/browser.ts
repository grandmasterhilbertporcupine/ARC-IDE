...
export interface BbDesktopBrowserApi {
  captureFeedback?(
    request: BbDesktopBrowserFeedbackRequest,
...
export const bbDesktopBrowserElementSchema = z
  .object({
  ...
  .strict();
export type BbDesktopBrowserElement = z.infer<
  typeof bbDesktopBrowserElementSchema
>;
export const bbDesktopBrowserFeedbackRequestSchema = z
  .object({
    tabId: z.string().min(1),
    threadId: z.string().min(1),
    mode: z.enum(["capture", "select", "cancel"]),
  })
  .strict();
export type BbDesktopBrowserFeedbackRequest = z.infer<
  typeof bbDesktopBrowserFeedbackRequestSchema
>;
export const bbDesktopBrowserFeedbackSchema = z
  .object({
    url: z.string().max(4096),
    title: z.string().max(1024),
    screenshot: z.string().max(12_000_000),
    element: bbDesktopBrowserElementSchema.nullable(),
    console: z
      .array(
        z.object({ level: z.string().max(30), message: z.string().max(1000) }),
      )
      .max(50),
    network: z
      .array(
  ...
  .strict();
export type BbDesktopBrowserFeedback = z.infer<
  typeof bbDesktopBrowserFeedbackSchema
>;
