...
export const createThreadRequestSchema = z
  ...
  .superRefine((value, ctx) => {
    if (value.senderThreadId !== undefined && !value.experimental_addressing) {
      ctx.addIssue({
        code: "custom",
        message: "senderThreadId on creation requires experimental_addressing",
        path: ["senderThreadId"],
      });
    }
    ...
    if (
      value.experimental_executionContextId !== undefined &&
      value.origin !== "plugin"
    ) {
      ctx.addIssue({
        code: "custom",
        message: 'experimental_executionContextId requires origin "plugin"',
        path: ["experimental_executionContextId"],
      });
    }
...
export const sendMessageResponseSchema = z.discriminatedUnion("delivery", [
  z.object({
    ...
    experimental_addressed:
      experimentalAddressedDispatchResultSchema.optional(),
  }),
  z.object({
    ok: z.literal(true),
...
export const editMessageRequestSchema = sendMessageRequestBaseSchema
  .omit({ mode: true, sendAt: true, experimental_addressing: true })
  .extend({
