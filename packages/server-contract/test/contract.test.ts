...
const OPTIONAL_SERVER_FIELD_GROUPS: readonly OptionalServerFieldGroup[] = [
  ...
  {
    reason:
      'originPluginId is present exactly when origin is "plugin"; plugin creation may optionally bind an immutable execution context. Both ownership constraints are enforced by refinement.',
    fields: [
      "createThreadRequestSchema.originPluginId",
      "createThreadRequestSchema.experimental_executionContextId",
    ],
  },
  {
    reason:
...
describe("server-contract canonical schemas", () => {
  ...
  it("parses request contracts", () => {
    ...
    ).toMatchObject({ type: "local_path", path: "C:\\Users\\michael\\bb" });
  ...
  it("accepts a bounded execution context only on plugin-owned creation", () => {
    const base = {
      projectId: "proj_123",
      origin: "plugin",
      originPluginId: "arc",
    ...
    expect(
      createThreadRequestSchema.parse(base).experimental_executionContextId,
    ).toBe("ctx-1");
    for (const override of [
      { origin: "app", originPluginId: undefined },
      { originPluginId: undefined },
      { experimental_executionContextId: " " },
      { experimental_executionContextId: "x".repeat(257) },
    ]) {
      expect(
        createThreadRequestSchema.safeParse({ ...base, ...override }).success,
      ).toBe(false);
    }
  });
