...
interface PluginDevLoopDeps {
  ...
  filterChanges?: (paths: readonly string[]) => Promise<readonly string[]>;
  debounceMs?: number;
  now?: () => number;
}
...
export function createPluginDevLoop(deps: PluginDevLoopDeps): PluginDevLoop {
  ...
  async function runCycle(files: readonly string[]): Promise<void> {
    if (disposed) return;
    if (deps.filterChanges !== undefined) {
      try {
        files = await deps.filterChanges(files);
      } catch (error) {
        deps.log(
          `source read failed: ${errorMessage(error)} — fix and save to retry`,
        );
        return;
      }
