...
import { omitNpmScriptPolicyEnv, spawnPortableOutputProcess, stopProcessGroupLeaderFirst, supportsProcessGroups } from "@bb/process-utils";

async function run(command: string, args: string[], options: { maxBuffer: number; env: NodeJS.ProcessEnv }): Promise<void> {
  const child = spawnPortableOutputProcess({ command, args, env: options.env, detached: supportsProcessGroups() });
  let output = "";
  let exceeded = false;
  const stop = () => { void stopProcessGroupLeaderFirst({ child, timeoutMs: 1000, killGraceMs: 1000 }); };
  await new Promise<void>((resolve, reject) => {
    const timeout = setTimeout(() => { stop(); reject(new Error("Plugin toolchain installation exceeded five minutes.")); }, 300_000);
    const collect = (chunk: Buffer) => {
      output = (output + chunk.toString("utf8")).slice(-options.maxBuffer);
      if (Buffer.byteLength(output) >= options.maxBuffer) { exceeded = true; stop(); }
    };
    ...
    child.once("close", (code) => {
      ...
      else reject(new Error(exceeded ? "Plugin toolchain installer exceeded its output limit." : `Plugin toolchain installer exited ${code}: ${output.slice(-5000)}`));
    });
  });
}
