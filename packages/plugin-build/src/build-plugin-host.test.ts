...
describe("plugin host build", () => {
  ...
  it("rejects private ARC workspace imports from host entries", async () => {
    ...
    await expect(
      buildPluginHost(dir, "0.9.0-test", await testToolchain()),
    ).rejects.toThrow(/cannot import private ARC workspace package/u);
  });
  ...
  it("rejects relative type imports into private ARC workspace packages", async () => {
