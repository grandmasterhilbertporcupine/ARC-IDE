...
function privateBbImportError(specifier: string): string {
  return `host entries cannot import private ARC workspace package "${specifier}"; use @get-bb/plugin-sdk, Node APIs, or a regular plugin dependency`;
}
...
async function readPluginHostConfig(rootDir: string): Promise<{
  ...
  if (!isWithinDirectory(rootDir, hostEntry)) {
