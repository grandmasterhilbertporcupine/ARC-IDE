...
async function shimModuleSource(
  ...
  return [
    ...
    `  throw new Error(${JSON.stringify(
      `Cannot load "${specifier}": this bundle must be loaded by the ARC app, which provides the shared plugin runtime (globalThis.__bbPluginRuntime).`,
    )});`,
    `}`,
...
async function readPluginAppConfig(rootDir: string): Promise<PluginAppConfig> {
  ...
  if (!isWithinDirectory(rootDir, appEntry)) {
