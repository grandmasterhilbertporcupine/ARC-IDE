...
const ONLINE_RPC_RESPONSE_RESULT_FIXTURES: OnlineRpcResponseResultFixtures = {
  "terminal.probePreview": { state: "ready", statusCode: 200, reason: null },
  "terminal.stopPreview": { state: "stopped", exitCode: 0, reason: null },
...
const ADDITIONAL_ONLINE_RPC_RESPONSE_ROUND_TRIP_CASES: OnlineRpcResponseRoundTripCase[] =
  [
    {
      name: "preview probe unavailable",
      commandType: "terminal.probePreview",
      result: {
        state: "unavailable",
        statusCode: null,
        reason: "Host cannot inspect listeners",
      },
    },
    {
      name: "preview probe failed",
      commandType: "terminal.probePreview",
      result: {
        state: "failed",
        statusCode: null,
    ...
    {
      name: "preview probe starting",
      commandType: "terminal.probePreview",
      result: {
        state: "starting",
        statusCode: 503,
        reason: "Waiting for HTTP readiness",
      },
    },
    {
      name: "preview stop unconfirmed",
      commandType: "terminal.stopPreview",
      result: {
        state: "unavailable",
        exitCode: null,
        reason: "Process exit not acknowledged",
      },
    },
    {
...
const ONLINE_RPC_RESPONSE_MISMATCH_CASES: OnlineRpcResponseMismatchCase[] = [
  {
    name: "preview stop cannot confirm a null exit",
    commandType: "terminal.stopPreview",
    result: { state: "stopped", exitCode: null, reason: null },
  },
  {
    name: "preview stop cannot accept probe readiness",
    commandType: "terminal.stopPreview",
    result: { state: "ready", statusCode: 200, reason: null },
  },
  {
    name: "preview probe cannot accept a stop acknowledgement",
    commandType: "terminal.probePreview",
    result: { state: "stopped", exitCode: 0, reason: null },
  },
  {
...
describe("host-daemon command schemas", () => {
  ...
  it("requires exact conditional stop identity and rejects unbound terminal acknowledgements", () => {
    const command = {
      type: "turn.stop-if-current",
      environmentId: "environment-1",
      threadId: "thread-1",
      expectedClientRequestId: CLIENT_REQUEST_ID,
      expectedTurnId: null,
    };
    ...
    expect(
      hostDaemonCommandSchema.safeParse({
        ...command,
        expectedClientRequestId: "unbound",
      }).success,
    ).toBe(false);
    const schema = hostDaemonCommandResultSchemaByType["turn.stop-if-current"];
    expect(
      schema.safeParse({
...
describe("host-daemon session schemas", () => {
  ...
  it.each(["terminal.probePreview", "terminal.stopPreview"])(
    "round-trips %s RPC failures without a fabricated success result",
    (commandType) => {
      const message = {
        type: "host-rpc.response",
        ...
        errorMessage: "Host disconnected before acknowledgement",
      };
      const roundTrip = JSON.parse(JSON.stringify(message));
      expect(hostDaemonOnlineRpcResponseMessageSchema.parse(roundTrip)).toEqual(
        message,
      );
      expect(hostDaemonDaemonWsMessageSchema.parse(roundTrip)).toEqual(message);
    },
  );
