...
export const turnStopIfCurrentCommandSchema = hostDaemonThreadTargetSchema
  .extend({
    type: z.literal("turn.stop-if-current"),
    expectedClientRequestId: clientTurnRequestIdSchema,
    expectedTurnId: z.string().min(1).nullable(),
  })
  .strict();
...
const threadStopResultSchema = z
  ...
  .strict();
const turnStopIfCurrentResultSchema = z
  .object({
    status: z.enum(["stopped", "already-settled", "not-current", "unknown"]),
    clientRequestId: clientTurnRequestIdSchema.nullable(),
  ...
  .superRefine((result, ctx) => {
    if (
      ...
      (result.clientRequestId === null || result.turnId === null)
    )
      ctx.addIssue({
        code: "custom",
        message:
          "Settled conditional stops require exact request and turn identity",
      });
  });
...
export const terminalPreviewProbeResultSchema = z
  .object({
    state: z.enum(["ready", "starting", "failed", "unavailable"]),
    statusCode: z.number().int().nullable(),
    reason: z.string().nullable(),
  })
  .strict();
export type TerminalPreviewProbeResult = z.infer<
  typeof terminalPreviewProbeResultSchema
>;
export const terminalPreviewStopResultSchema = z.discriminatedUnion("state", [
  z
    .object({
...
export type TerminalPreviewStopResult = z.infer<
  typeof terminalPreviewStopResultSchema
>;

export const hostDaemonCommandRegistry = {
  "terminal.stopPreview": defineHostDaemonCommandDescriptor({
    type: "terminal.stopPreview",
    schema: z
      .object({
        type: z.literal("terminal.stopPreview"),
        terminalId: z.string().min(1),
      })
      .strict(),
    resultSchema: terminalPreviewStopResultSchema,
    transport: "onlineRpc",
    retryable: false,
    flushEventsBeforeResult: false,
    envLane: null,
  }),
  "terminal.probePreview": defineHostDaemonCommandDescriptor({
    type: "terminal.probePreview",
    schema: z
      .object({
        type: z.literal("terminal.probePreview"),
        terminalId: z.string().min(1),
        url: z.url().max(4096),
      })
      .strict(),
    resultSchema: terminalPreviewProbeResultSchema,
    transport: "onlineRpc",
    retryable: true,
    flushEventsBeforeResult: false,
    envLane: null,
  ...
  "turn.stop-if-current": defineHostDaemonCommandDescriptor({
    type: "turn.stop-if-current",
    schema: turnStopIfCurrentCommandSchema,
    resultSchema: turnStopIfCurrentResultSchema,
    transport: "settled",
    retryable: false,
    flushEventsBeforeResult: true,
    envLane: null,
