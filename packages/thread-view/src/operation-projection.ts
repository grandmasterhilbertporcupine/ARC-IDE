...
function mergeThreadOperationMessage(
  ...
  const addressed =
    incoming.threadOperation?.rawOperation === "addressed_dispatch" &&
    existing.threadOperation?.rawOperation === "addressed_dispatch";
  const mergedStatus = addressed
    ? (incoming.status ?? "pending")
    : mergeLifecycleStatus(
        existing.status ?? "pending",
        incoming.status ?? "pending",
      );
  const shouldUseIncomingLifecycle =
    addressed || existing.status !== mergedStatus;
