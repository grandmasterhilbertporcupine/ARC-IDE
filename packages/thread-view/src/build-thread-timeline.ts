...
function buildGenericOperationSystemRow({
  ...
  const result =
    message.threadOperation?.operation === "other" &&
    message.threadOperation.rawOperation === "addressed_dispatch"
      ? message.threadOperation.metadata?.result
      : null;
  const path =
    result &&
    ...
    /^\/plugins\/[a-zA-Z0-9_-]+\/[^\s\\]+$/.test(result.path)
      ? result.path
      : null;
  return {
    ...(message.threadOperation?.operation === "other" &&
    message.threadOperation.rawOperation === "addressed_dispatch" &&
    message.threadOperation.status === "failed" &&
    message.threadOperation.metadata?.retryAddressed === true &&
    typeof message.threadOperation.operationId === "string"
      ? { experimental_addressedRetry: message.threadOperation.operationId }
      : {}),
    ...(path ? { experimental_link: { path, label: "Open run" } } : {}),
    ...base,
    kind: "system",
