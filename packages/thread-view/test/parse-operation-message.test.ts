...
describe("parseOperationMessage operation titles", () => {
  it.each(["child-completed", "child-failed", "child-interrupted"])(
    "renders a delivered %s notice without claiming a provider turn",
    (kind) => {
      const row = factory().systemOperation({
        operation: "owned_child_notice",
        status: "completed",
        message: "The assigned child has an update.",
        metadata: { delivery: "owner-controlled", kind },
      });
      const { event, meta } = decodeThreadEventRow(row);
      expect(
        ...
        title: "Child update",
        status: "completed",
        detail: "The assigned child has an update.",
      });
    },
  );
