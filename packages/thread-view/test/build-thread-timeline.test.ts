...
describe("buildThreadTimelineFromEvents", () => {
  it("replaces a failed addressed Retry action with the recovered run receipt", () => {
    const operationId = "223e4567-e89b-42d3-a456-426614174000";
    const failed = systemOperationEvent({
      operation: "addressed_dispatch",
      operationId,
      seq: 1,
      status: "failed",
      message: "Configure providers",
      metadata: { retryAddressed: true },
    });
    const before = buildTimelineRows([failed]);
    expect(before).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ experimental_addressedRetry: operationId }),
      ]),
    );
    const after = buildTimelineRows([
      ...
      systemOperationEvent({
        ...
        status: "completed",
        message: "Started",
        metadata: { result: { path: "/plugins/arc/workspace/run-1" } },
      }),
    ]);
    expect(after.some((row) => "experimental_addressedRetry" in row)).toBe(
      false,
    );
    expect(after).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          experimental_link: {
            path: "/plugins/arc/workspace/run-1",
            label: "Open run",
          },
        }),
      ]),
