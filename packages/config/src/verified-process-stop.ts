...
async function readWindowsProcessField(
  pid: number,
  field: "command" | "elapsed",
): Promise<string | null> {
  if (!Number.isSafeInteger(pid) || pid <= 0) return null;
  const expression =
    field === "command"
      ? "$processRecord.CommandLine"
      : "[math]::Floor(([DateTime]::Now - $processRecord.CreationDate).TotalSeconds)";
  try {
    const result = await execFileAsync(
      ...
      [
        ...
        "-NonInteractive",
        "-Command",
        `[Console]::OutputEncoding = [System.Text.UTF8Encoding]::new(); $processRecord = Get-CimInstance Win32_Process -Filter 'ProcessId = ${pid}'; if ($null -ne $processRecord) { ${expression} }`,
      ],
  ...
  return {
    ...
    kill(pid, signal) {
      if (process.platform === "win32") {
        if (!Number.isSafeInteger(pid) || pid <= 0 || pid === process.pid) {
          throw new Error(
            "Refusing to stop an invalid or current process tree",
          );
        }
        ...
        if (result.status !== 0 && isProcessRunning(pid)) {
          throw new Error("Could not stop the verified Windows process tree", {
            cause: result.error,
          });
        }
        return;
    ...
    readCommand: (pid) =>
      process.platform === "win32"
        ? readWindowsProcessField(pid, "command")
        : readPsField(pid, "command="),
    async readElapsedSeconds(pid) {
      if (process.platform === "win32") {
        const rawElapsed = await readWindowsProcessField(pid, "elapsed");
        return rawElapsed !== null && /^\d+$/u.test(rawElapsed)
          ? Number(rawElapsed)
          : null;
      }
