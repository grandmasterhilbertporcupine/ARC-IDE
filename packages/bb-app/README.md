...
| `pi`           | Install [Pi](https://github.com/earendil-works/pi/tree/main/packages/coding-agent) with `npm install -g @earendil-works/pi-coding-agent` (0.84.0 or newer) and authenticate per its docs; ARC can run the install from Settings.          |
...
ARC indexes the documented native skill roots for Codex, Claude Code, Pi,
...
`bb skill list` show native skills for Claude Code, Codex, and Cursor. ARC also
reads configured Pi, omp, Grok, and Hermes skill directories, plus enabled
provider plugin skills.

ARC reads Pi's global `~/.pi/agent` files and each workspace's `.pi` files.
...
ARC loads project resources only after Pi's saved or global trust policy approves
the workspace. An unresolved `ask` decision stays untrusted because ARC has no Pi
trust prompt.
...
ARC lists these sources as read-only skills. ARC injects them into Codex, Claude,
Pi, and ACP threads. This permits one physical skill collection for ARC and a
