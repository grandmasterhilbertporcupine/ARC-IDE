...
describe("bb-app launcher", () => {
  it("runs the bundled CLI through its current runtime without relying on PATH", async () => {
    const directory = mkdtempSync(join(tmpdir(), "ARC's bundled CLI Δ "));
    const context = { ...createTestStartContext(), daemonBundleDir: directory };
    const resultFile = join(directory, "arguments.json");
    try {
      writeFileSync(join(directory, "package.json"), '{"type":"module"}');
      writeFileSync(
        join(directory, "bb"),
        'import { writeFileSync } from "node:fs"; writeFileSync(process.argv[2], JSON.stringify({ args: process.argv.slice(3), reexec: process.env.BB_CLI_REEXEC })); process.exitCode = 7;',
      );
      const env = createServerEnv({
        context,
        env: { ...process.env, PATH: "" },
      });
      expect(env.BB_CLI).toBe(
        join(directory, process.platform === "win32" ? "bb.cmd" : "bb"),
      );
      if (process.platform === "win32")
        expect(env.BB_CLI_RUNTIME).toBe(process.execPath);
      const args = ["O'Reilly Δ", "with spaces", "x&y", 'a"b'];
      expect(
        await runBundledCliCommand({
          args: [resultFile, ...args],
          context,
          env,
        }),
      ).toBe(7);
