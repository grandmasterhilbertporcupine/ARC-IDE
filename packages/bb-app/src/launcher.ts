...
function requiredHostArtifactPaths(context: BbAppStartContext): ArtifactPath[] {
  ...
  if (process.platform === "win32") {
    artifacts.push({
      kind: "file",
      label: "bundled Windows bb CLI shim",
      path: join(context.daemonBundleDir, "bb.cmd"),
    });
  }
  return artifacts;
}
...
export function createServerEnv(args: CreateServerEnvArgs): NodeJS.ProcessEnv {
  return {
    ...
    ...(process.platform === "win32"
      ? { BB_CLI_RUNTIME: process.execPath }
      : {}),
...
export async function runBundledCliCommand(
  ...
  const bundledCliPath = join(args.context.daemonBundleDir, "bb");
  const cliPath = bbCliOverride ?? bundledCliPath;
  const bundledShimPath = join(args.context.daemonBundleDir, "bb.cmd");
  const useBundledRuntime =
    bbCliOverride === undefined ||
    resolve(bbCliOverride) === resolve(bundledCliPath) ||
    (process.platform === "win32" &&
      resolve(bbCliOverride).toLowerCase() ===
        resolve(bundledShimPath).toLowerCase());
  const childProcess = spawn(
    useBundledRuntime ? process.execPath : cliPath,
    useBundledRuntime ? [bundledCliPath, ...args.args] : args.args,
    {
      cwd: process.cwd(),
      env: createCliEnv({
        ...
        env: {
          ...args.env,
          ...(useBundledRuntime ? { BB_CLI_REEXEC: "1" } : {}),
        },
      }),
      stdio: "inherit",
      windowsHide: true,
