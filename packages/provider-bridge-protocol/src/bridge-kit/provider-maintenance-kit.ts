        ...
        .find(
          (line) =>
            ...
            (process.platform !== "win32" ||
              /\.(?:exe|com|cmd|bat)$/iu.test(line.trim())),
        )
        ?.trim() ?? null
    );
  } catch {
...
async function executeMaintenanceCommand(
  ...
  const execution = execFileAsync(
    windowsScript ? resolveWindowsPowerShell() : resolvedCommand,
    windowsScript
      ? [
          "-NoLogo",
          "-NoProfile",
          "-NonInteractive",
          "-Command",
          `& ${[resolvedCommand, ...args].map((part) => `'${part.replaceAll("'", "''")}'`).join(" ")}; exit $LASTEXITCODE`,
        ]
      : [...args],
...
export async function commandOutput(
  ...
  try {
    const { stdout, stderr } = await executeMaintenanceCommand(
      command,
      args,
