// Builds the ARC plugin component registry (plugin design §5.5): shadcn
...
for (const [itemName, relPath] of [...fileByItem.entries()].sort()) {
  ...
  const item = {
    ...
    type,
    title: itemName,
    description: `ARC ${type.replace("registry:", "")} "${itemName}" — vendored from the ARC app's own source (version-matched to this ARC release).`,
    ...(dependencies.size > 0
...
generatedFiles.set(
  ...
  JSON.stringify(
    {
      $comment:
        "ARC plugin component registry index. Install via: npx shadcn add @bb/<name> (see the bb-plugin-authoring skill).",
