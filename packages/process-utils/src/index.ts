...
export function spawnPortableSyncProcess(
  request: PortableSpawnRequest,
): SpawnSyncReturns<Buffer> {
  return crossSpawn.sync(request.command, request.args, {
    cwd: request.cwd,
    env: request.env,
    stdio: request.stdio,
    windowsHide: true,
...
export function killProcessGroup(args: KillProcessGroupArgs): void {
  if (process.platform === "win32" && args.child.pid !== undefined) {
    if (
      !Number.isSafeInteger(args.child.pid) ||
      args.child.pid <= 0 ||
      args.child.pid === process.pid
    ) {
      throw new Error("Refusing to stop an invalid or current process tree");
    }
    if ("exitCode" in args.child && args.child.exitCode !== null) {
      return;
    }
    const result = spawnSync(
      ...
      ["/PID", String(args.child.pid), "/T", "/F"],
      { windowsHide: true, stdio: "ignore", timeout: 5_000 },
    );
    if (result.status === 0 || !isProcessAlive(args.child.pid)) {
      return;
    }
    throw new Error("Could not terminate the owned Windows process tree", {
      cause: result.error,
    });
  }
...
export function sanitizeInheritedChildProcessEnv(
  ...
  for (const [key, value] of Object.entries(args.env)) {
    ...
    if (comparisonKey === "NODE_ENV" || comparisonKey.startsWith("BB_")) {
      continue;
    }
    sanitizedEnv[comparisonKey === "PATH" ? "PATH" : key] = value;
  }
...
export function resolveWindowsPowerShell(
  ...
  const candidates = [
    ...searchPath
      ...
      .filter(Boolean)
      .map((directory) =>
        win32.join(directory.replace(/^"|"$/gu, ""), "pwsh.exe"),
      ),
    win32.join(
