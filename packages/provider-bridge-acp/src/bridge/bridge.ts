...
async function handleFsWriteTextFile(
  ...
  if (
    ...
    responder.error(
      -32000,
      `File writes outside the workspace are denied by ARC's accept-edits permission mode: ${parsed.data.path}`,
    );
    return;
  }
...
async function handleRequest(
  ...
  switch (request.method) {
    ...
    case "provider/health": {
      ...
      const result = await getAcpProviderHealth({
        ...
        result.supported
          ? {
              ...
              health: {
                ...result.health,
                loginCommand:
                  result.health.loginCommand ??
                  launchSpec?.experimental_loginCommand ??
                  null,
              },
            }
          : result,
