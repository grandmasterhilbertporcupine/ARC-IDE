...
async function resolveLifecycleScriptPath(
  ...
  if (process.platform === "win32" && scriptName.endsWith(".ps1")) {
    const posixScript = path.join(
      workspacePath,
      scriptName.replace(/\.ps1$/u, ".sh"),
    );
    if (await pathExists(posixScript)) {
      throw new WorkspaceError(
        "setup_script_failed",
        `This workspace has a POSIX lifecycle hook. Add ${scriptName} for native Windows execution.`,
      );
    }
  }
  return null;
...
function buildWindowsLifecycleScriptCommand(
  args: BuildLifecycleScriptCommandArgs,
): LifecycleScriptCommand {
  if (!args.scriptPath.endsWith(".ps1")) {
    throw new WorkspaceError(
      "setup_script_failed",
      "POSIX shell hooks are not supported on Windows. Provide a .ps1 lifecycle hook.",
    );
  }
  return {
    ...
    text: `PowerShell ${path.win32.basename(args.scriptPath)}`,
  };
}

function lifecycleScriptName(name: string): string {
  return process.platform === "win32" ? name.replace(/\.sh$/u, ".ps1") : name;
}
...
export function runSetupScript(
  ...
  return runLifecycleScript({
    ...args,
    kind: "setup",
    scriptName: lifecycleScriptName(DEFAULT_ENV_SETUP_SCRIPT_NAME),
  });
}
...
export async function runTeardownScript(
  ...
  try {
    return await runLifecycleScript({
      ...
      onProgress,
      kind: "teardown",
      scriptName: lifecycleScriptName(DEFAULT_ENV_TEARDOWN_SCRIPT_NAME),
    });
  } catch (error) {
    if (!failureReported) {
      emitStep({
