...
function serverEntrySource(packageName: string): string {
  ...
  return `// ${packageName} — an ARC plugin backend entry.
//
// The default export is a factory that receives the plugin API. ARC supplies
...
export default async function plugin(bb: BbPluginApi) {
  ...
  // Declarative settings — rendered in ARC's settings UI and editable with
  ...
  // argv is plugin-owned; \`commands\` is metadata ARC renders into help and
...
function appEntrySource(packageName: string): string {
  const id = derivePluginId(packageName);
  return `// ${packageName} — an ARC plugin frontend entry.
//
// Compiled by \`bb plugin build\` into dist/app.js + dist/app.css. React and
// @get-bb/plugin-sdk/app are provided by the ARC app at load time (never bundled),
// so this file must be loaded by ARC, not imported directly.
//
// The components under components/ui/ are YOURS: vendored source (shadcn
// model), edit freely. Add more from the ARC registry with
// \`npx shadcn add @bb/<name>\` (see components.json) — dropdowns, tables,
// the full shadcn set, version-matched to this ARC install. Run
...
/** The dashed box ARC's own list pages use for loading and empty states. */
...
// (scrolling page, centered column) matches ARC's own nav-panel pages.
function TodosPage() {
...
// The default export must be definePluginApp(...); ARC interprets it after
...
The ${name} plugin keeps one todo list. The Example todos page in the ARC sidebar and
...
The todos live in this plugin's own storage on the ARC server, one list per
...
An ARC plugin that keeps a todo list. It shows every surface a plugin can own:
...
- \`skills/example-todos/SKILL.md\` — a skill that tells agents how to keep the list
  with \`bb ${id}\`. ARC imports it into agent threads automatically.
...
files freely — they never update out from under you. Add more from the ARC
component registry (the full shadcn set, version-matched to your ARC install
...
npm deps bundle into your dist. React, and ARC-shimmed packages like the
radix portal primitives and \`sonner\` (\`import { toast } from "sonner"\`
reaches ARC's own toaster), are provided by the ARC app at runtime and never
...
- \`bb.skills\` — skill roots; omitted here, so ARC reads \`skills/\`. Each
  directory with a \`SKILL.md\` is one skill, named after the directory.
- \`bb.name\` and \`bb.description\` — required human-facing identity.
- \`bb.branding\` — required; declare \`icon\` as an ARC icon name or a
...
- \`engines.bbPluginSdk\` — the lowest plugin SDK you need (scaffold:
  \`>=${PLUGIN_SDK_VERSION}\`). ARC reads this as a floor, not a ceiling: a later
  SDK in the same major still loads your plugin.
- \`dependencies\` — every package your source imports that ARC does not provide.
  ...
  is for types and tooling only (ARC shims React, the portal primitives, and
...
The submission to the public Community marketplace requires the file. Keep
...
exact version in \`devDependencies\` (\`${PLUGIN_SDK_VERSION}\` — the SDK of the ARC
...
The SDK surface grows with every ARC release, so the pin has to track the ARC you
actually run:

\`\`\`
bb plugin types          # sync this plugin's SDK surface to the running ARC
bb plugin types --check  # CI: fail when it does not match
\`\`\`

Ask ARC to write plugins for you: the \`bb-plugin-authoring\` skill documents
the whole surface with examples.

Confused by the API, or need something the types don't explain? Clone the ARC
    ...
    JSON.stringify(
      {
        ...
        bb: {
          ...
          description: "An ARC plugin with an example todo list.",
