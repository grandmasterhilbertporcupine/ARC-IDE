...
Making your repo work with bb:
  ...
  Native Windows uses tracked .bb-env-setup.ps1 and .bb-env-teardown.ps1 files.
  ARC runs them with PowerShell -NoProfile -NonInteractive -File and respects
  Windows execution policy. A POSIX-only hook fails native setup and asks for
  its .ps1 counterpart. The lifecycle and sanitized environment below are the
  same on both platforms.

  Commit a .bb-env-setup.sh script at the repo root when new ARC worktrees need
  repo-specific setup. After ARC creates a new managed worktree environment, it
  ...
  ARC runs the hook as `env bash .bb-env-setup.sh` with cwd set to the new
  ...
  variable are removed, and ARC does not inject BB_PROJECT_ID, BB_ENVIRONMENT_ID,
  or BB_SOURCE_PATH.
  ...
  A non-zero exit, timeout, signal, or cancellation fails provisioning and ARC
  ...
  resources outside the managed worktree. ARC runs the hook as
  ...
  signal reports failure in the destroy transcript, but ARC removes the
  worktree regardless. The teardown hook runs only when ARC destroys managed
  ...
  negate an earlier pattern. ARC copies each untracked file in the source
  ...
  ARC copies files only. It follows no symlinks, and it replaces nothing that
  ...
  that matches nothing, or a file ARC cannot read, is reported in the
  ...
  deleted, ARC destroys the environment: it stops the agent process, then
...
Remote access (`bb connect`):
  ...
  Expose this ARC server at <handle>.getbb.app so you can reach it from any
  ...
  Pairing returns immediately: the ARC SERVER redeems the code, stores the
  credential, and holds the tunnel itself — so it stays up as long as ARC is
  running and reconnects on restart (no foreground process).
  The upstream npm distribution can also pair:
  ...
  win, so the dev ARC can also pair with getbb.app.
  ...
  bb connect servers                      List every ARC on this account (handle, url, live)
  bb connect machine-code                 Mint a one-time code that pairs the getbb.app mobile app
  ...
  The getbb.app mobile app pairs with a paired ARC through `bb connect`. Turn on the
