...
remembered defaults apply. If the project has no remembered choice, ARC uses
...
an active turn. These settings are separate from ARC's optional Memory plugin,
an official plugin bundled with the app.

Provider-native subagents can also be disabled on those provider pages. For
Codex, ARC turns off the native multi-agent feature and caps V2 sessions at the
root thread so remote session policy cannot start a child. For Claude Code, ARC
...
quiescence while keeping the ARC thread attached and resumable. This defaults
...
Claude Code runs without its Claude in Chrome browser tools under ARC by
...
ARC indexes the native user and project skill roots for Codex, Claude Code, Pi,
...
ARC applies the selected model to the ACP session before the first prompt.
...
session mode, not a model. ARC does not select OpenCode agents; configure the
...
provider id), a model id, and an optional displayName. ARC skips an invalid
...
command. ARC derives provider id acp-<id> from the slug id. The id is permanent.
The id cursor is reserved because ARC always lists that agent. The ids opencode,
...
the vendor side channels ARC reads. Use nativeSkillRoots to add native skills to
the composer.
...
ARC hides the /compact command for agents that do not declare it. The plugin
re-registers its providers as soon as the setting changes, so no restart or
config refresh is needed.

The old customAcpAgents array in the app data-dir config.json is deprecated. ARC
...
setting does not accept: ARC drops that field when it reads the old array, and a
...
user and project paths use the same relative-path rules. ARC indexes these roots
as read-only sources. It then injects the selected skills into all providers.
The ARC user and project roots keep higher precedence than matching shared roots.
