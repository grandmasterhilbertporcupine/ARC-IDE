...
Addressed teams and agents:

  bb thread spawn --project <id> --prompt "..." --addressing-file <path>
  bb thread tell <id> "..." --addressing-file <path>

  The file contains {"operationId":"<UUID>","recipients":[{"pluginId":"arc","kind":"team","entityId":"<published-team-id>","versionId":1,"scopeKey":"project:<project-id>","label":"Team name"}]}.
  Use kind agent for an agent revision and scopeKey library for published library items.
  Reuse the same operationId and input when retrying one send; use a fresh UUID for new work.
  All recipients must use one coordinator. Selection alone never dispatches. Addressed sends do not support --send-at.
  The SDK equivalent is experimental_addressing on threads.spawn or threads.send.
  Retry a saved failed Send without reconstructing its attachments:
  bb thread retry-addressed <thread-id> --operation-id <original-UUID>
  SDK: threads.experimental_retryAddressed({threadId, operationId, senderThreadId?}). The server
  reloads the original request and deduplicates concurrent or completed retries.
  Addressed spawn, tell and retry preserve BB_THREAD_ID as sender context. SDK
  agent callers also supply senderThreadId; creation accepts it only with
  experimental_addressing. Owned workers cannot dispatch or retry addressed work;
  use the admitted run's message tools instead.

  Changing recipients preserves mandatory checks and supported check-based branch choices.
  Prior approval/release controls, nested conditions, or other decisions that cannot be
  safely replayed leave the saved follow-up action-required instead of dropping gates.
  Cancel that unadmitted follow-up in Workspace, restore the previous recipient versions,
  and send again to continue from the verified candidate with the existing budget and gates.
...
Spawning:
  ...
  bb thread spawn --project <id> --prompt "..." [options]
    ...
    --execution-context <id>       Bind a plugin's existing immutable execution context (experimental)
    --origin-plugin <id>           Owning plugin; required together with --execution-context
  ...
  remembered project defaults. With no remembered model, ARC uses the explicitly
  ...
  Omit --base-branch for ARC's default. Explicit values are exact; use
  origin/<branch> for a remote ref.

  --execution-context and --origin-plugin must be supplied together. The context
  identifier is trimmed, nonempty, and at most 256 characters. The owning plugin
  must already have persisted the context and must validate it in its agent
  configuration callback before the first provider call. Missing, disabled, or
  failing owner configuration stops dispatch. The identifier is not a credential
  and is not inherited by forks or children. Ordinary CLI creation is unchanged.
...
Listing:
  ...
  The table prints ID, Title, Project, Status, and Requested model. Title uses the thread
  ...
  personal project shows "-". Requested model shows provider/model from the
  latest retained execution request, or "-" when none is available. It does
  not claim which model the provider ultimately executed. Use --json for the
  full thread records, including lastRequestedModel: {providerId, model} or null.

  bb thread search <query> [--project <id>] [--limit <1-50>]
                                             Search threads and messages
  Search is global without --project. A project filter applies before the
  active/archived group totals and limits. Search records also include
  lastRequestedModel. SDK equivalents: threads.list({projectId}) and
  threads.search({query, projectId, limitPerGroup: "20"}).
...
Opening threads and files in the app:
  ...
  bb thread open <path>                    Open a file in the current ARC thread panel
  ...
  Inside an ARC thread, BB_THREAD_ID selects the current thread automatically and
  the thread ID argument is omitted for file-only opens. Pass an explicit thread
  ID with --split to open another thread. Outside an ARC thread, pass the thread ID
  ...
  Pane actions broadcast to connected ARC app windows and affect the matching
  ...
  artifacts you create for the user so they open in the ARC IDE.
...
Messaging:
  ...
  `thread clear` keeps the ARC thread, workspace, durable event history, and
...
Ownership:
  ...
  Model and reasoning updates stay within the thread's current provider. ARC
