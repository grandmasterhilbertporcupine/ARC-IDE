---
kind: instruction
title: ARC Guide Overview
summary: System overview and chapter index for the ARC CLI guide.
intent: Orient agents to ARC core concepts and help them find the right guide chapter.
editingNotes: Keep this concise. Concepts only — command details belong in chapter files.
---
ARC is an agent orchestration tool for managing multiple agents.
...
- Preview — a saved project launch command with an owned terminal, live URL and logs. Run `bb guide browser` for managed previews and native page inspection.
...
- BB_CLI_RUNTIME — on Windows, the Node-compatible runtime supplied by ARC for its bundled bb.cmd; a global Node installation is not needed inside ARC

Run `bb status` to see your current context (resolved project and thread IDs).
It also warns when an enabled plugin is not running (incompatible after a ARC
upgrade, failed to load, or missing); run `bb plugin list` for the detail.

All commands support --json for machine-readable output.

To make a repo work with ARC worktrees, run `bb guide environments` for the
