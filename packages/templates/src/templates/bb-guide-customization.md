---
kind: instruction
title: ARC Guide — Customization
summary: Command reference for customizing the ARC app color palette, keyboard shortcuts, and mobile push notifications.
...
Theming — the app-wide color palette and surface style

`bb theme` controls a set of CSS-variable overrides, persisted server-side and
applied live to every open window. Surface style is stored alongside the palette;
light/dark mode is a separate per-client setting. Custom themes live on
...
app uses ~/.bb/theme/…). The folder name is the theme id.
  ...
  bb theme set <id> [--favicon-color <color>]
                                 ...
                                 unless supplied; preserve the surface style
  ...
  bb theme style set <style>     Set default or liquid-glass; preserve palette/favicon
  bb theme style reset           Restore default surfaces; preserve palette/favicon
...
appearance values forward explicitly, including surface style. Liquid Glass is
optional and defaults off; `bb theme style set liquid-glass` enables it. Theme
reset changes the palette only. `bb theme show` includes the current surface style.

The SDK uses `sdk.theme.set({ themeId, faviconColor, surfaceStyle })` with
`surfaceStyle` set to `default` or `liquid-glass`. `sdk.theme.get()` reads the
selection. The theme-id shorthand preserves the favicon color and surface style.
...
On macOS it prevents system idle sleep while ARC is running; closing the lid or
...
false in packaged builds. Turn it on to show raw provider events ARC does not yet
...
`bb/`. ARC puts it in front of every branch name it creates for a worktree, so
the default gives `bb/fix-login-flow-thr_ab12cd34ef`. Set `sawyer/wt-` to get
`sawyer/wt-fix-login-flow-thr_ab12cd34ef`, or clear it for no prefix. ARC rejects
a prefix that cannot start a valid git branch name. The new prefix applies to
branches ARC creates after the change.
...
ARC releases restorable provider sessions after 30 idle minutes. The daemon
...
Reset removes an override and returns to ARC's current default; Clear explicitly
