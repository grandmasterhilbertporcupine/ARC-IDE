...
## Addressed composer dispatch

`sdk.threads.experimental_retryAddressed({threadId,operationId,senderThreadId?})` retries the server-retained original Send. It accepts no replacement prompt, attachments or recipients, and uses the same dispatch receipt and concurrency guards. Agent callers preserve their sender thread identity; core checks persisted preparation and execution context before receipt replay and refuses owned workers on both Send and Retry. The public HTTP API retains its existing trusted-local-process boundary: browser-origin checks do not authenticate a worker, so deliberately omitted or falsified sender identity is not a sandboxed principal. Audit failed first-thread admission, lost successful replies, unavailable saved attachments, cross-thread identity rejection and timeline retry state before stabilization.

`PluginMentionItem.experimental_recipient` pins a typed agent/team identity, published revision and scope. `bb.ui.experimental_registerAddressedDispatch` owns explicit sends with those recipients. Core validates one coordinator, persists user-visible operation receipts, rejects conflicting operation replays, prepares a new conversation workspace without a provider turn, and suppresses ordinary dispatch. The handler must revalidate project scope and versions and make external effects idempotent. `experimental_addressing` is available on SDK spawn/send and the CLI `--addressing-file` surface. Optional result paths must stay under the owning plugin's navigation path. Before stabilization, verify same-operation crash replay, first-send provisioning recovery, continuation budgets, active-turn history isolation, attachment handling and renderer/CLI parity. Addressed sends currently reject scheduling; provider skill invocation is not guaranteed by receipt creation.

## `PluginAgentConfiguration.experimental_skillSnapshots`

The owning execution-context plugin can supply immutable SKILL.md directories with UTF-8 instructions and exact base64 support files. Server validation enforces safe relative paths, unique names, matching YAML metadata, 128 files and 1 MiB per skill, at most 32 skills and 4 MiB total. Snapshots take precedence over same-named ARC catalog entries while unrelated shared skills remain available. Native provider discovery remains provider controlled.

The server registers content-addressed trees and marks them required in daemon protocol 190. A bridge without `skills.configure` rejects before starting the agent; a busy runtime with different required skill contents rejects instead of silently retaining the old catalog. The transport does not grant filesystem, network or tool permissions. Before stabilization, audit provider-native precedence, helper-file portability, cross-host staging, lifecycle cleanup, corrupt/missing content handling and owner disposal. Verify live Codex and Claude skill activation; do not equate availability with guaranteed invocation.

## `bb.experimental_turns` and native invocation ownership

Existing-conversation preparations retain immutable input, environment and execution under the actual plugin owner and operation. Release requires idle state and a revision check; the existing queue consumes one ungrouped input and records its client request atomically. A transient `experimental_turnContext` is delivered only to the owning agent configuration callback. The thread's origin and permanent execution context remain unchanged. Manual queue edits, Send-now, retry and owned message editing cannot replay the admission; active owned turns block additional inputs and history mutations.

Pre-request interruption cancels that operation permanently. Lifecycle loss before allocation requires explicit revision-based reconciliation; after a client request exists the operation cannot allocate another. Conditional interruption uses protocol 186's exact request/turn stop, preserves unknown outcomes, and reports terminal status only from retained native events.

Every native plugin tool invocation receives `experimental_invocation`, including exact provider thread, turn and call IDs plus `ownedTurn` with actual owner identity. A null ownership value means all mapped native inputs are proven unowned. Missing mappings, mixed owned inputs and stale native turns reject before invoking the tool. Foreign ownership is never hidden as manual input. Fake-host tests may inject `experimental_preparedTurns`; non-native contexts explicitly omit invocation identity.

Before stabilization, audit dispatch and mutation bypasses, owner disposal and callback races, conditional-stop lost replies and native restart ambiguity, and mapping fidelity across providers and steered inputs. Verify actual migrated SQLite and native provider evidence. Rename both experimental context fields and the API only after that audit.

## `bb.experimental_threads` and internal plugin RPC

`ExperimentalPreparedThreads` exposes durable owner/operation reservations,
sealed first input and execution, provisioning without a provider turn, and
revision/environment-checked first release. Ownership comes from the actual
plugin handle. Required `turnPolicy` is `single` or `conversation`; ARC workers
use single so later Send-now, retry, edit, clear and manual compaction cannot
bypass orchestration call limits. The queue and actual client request identity
are retained atomically. Unknown native outcomes and stale owner generations
require reconciliation rather than automatic replay. Managed named branch
semantics remain inherited; exact source claims require native host evidence.

Optional preparation `parentNotification: "owner-controlled"` seals passive
parent delivery for a non-null parent. Core resolves it from the retained
child preparation, records deduplicated `owned_child_notice` system operations,
and does not wake/steer the parent for direct or queued notifications. Explicit
worker-to-parent sends reject until a separate owner admission exists.
Omission keeps legacy delivery and canonical request hashes unchanged. Audit
mixed ordinary/owned children, all notification/send/queue paths, owner disposal
and duplicate outcomes; the option does not budget an ordinary parent task.

`bb.rpc.experimental_registerInternal` and `experimental_client` reuse Standard
Schema contracts with separate private registrations and actual caller identity.
Caller/target generation checks surround asynchronous boundary validation;
disposal withdraws registrations and aborts handlers before cleanup. Testing
adds `experimental_internalRpc`, `experimental_callInternalRpc`, and
`experimental_preparedThreads` to the fake host.

Before stabilization, verify crash gaps around reservation, provisioning and
queue consumption on real migrated databases; validate stale callbacks and
cross-owner refusal; confirm native acceptance/completion correlation across
providers; audit all alternate dispatch paths; and establish the intended
reconciliation UX before offering automatic recovery. Keep internal handlers
unreachable from public routes. Remove experimental prefixes only with the
project-wide API audit and rename.
...
## Fixed Context embeddings (`ExperimentalHostRpcContext.experimental_contextEmbeddings`)

Added experimentally for ARC Context on 2026-09-10. `ExperimentalHostContextEmbeddings` exposes `status({signal})`, `countTokens(input,{signal})` and `embed(input,{signal})`. The SDK exports the `ExperimentalContextEmbedding*` and `ExperimentalContextTokenCountResult` types and corresponding `experimental_contextEmbedding*` / `experimental_contextTokenCountResultSchema` validation schemas through its host and root entry points. `experimental_contextEmbeddingLimits` is the shared bound definition. The host test harness accepts an explicit `experimental_contextEmbeddings` fixture; its default reports unavailable.

The installed host chooses the fixed local model and packaged executable. Plugins cannot supply model, executable, module or cache paths. Status contains a verified manifest/model/runtime/tokenizer descriptor or a typed unavailable reason, without OS PID or installation paths. Count/embed inputs have exact request and item IDs, an expected manifest digest and required `expectedGeneration` from ready status, up to eight items, 32 KiB per UTF-8 item and 64 KiB per batch. Counts include special tokens and can exceed 256; embeddings reject larger inputs and return 384 finite normalized values. Only status can initialize a helper. Count/embed reject an absent or changed generation as `worker_stopped` before admission and recheck queued work at dispatch; they never initialize or migrate to a replacement helper. An unconfirmed stopping helper remains `stop_unresolved`. Successful results retain exact request/manifest/helper-generation bindings; failures retain a typed code and bounded message.

Every operation requires an abort signal and is also bound to the plugin worker lifecycle. A capability can support background indexing after its initiating RPC returns, but that work must retain the worker using an existing watch or lease. The daemon derives caller ownership from the actual plugin worker generation. One shared helper accepts one active operation and four queued operations; cancelling active inference can fail other queued Context requests explicitly. Cancellation waits for the owned helper to exit when a hard stop is necessary and never interrupts unrelated native effects by killing the plugin worker. Unresolved exit remains a failure and fences replacement inference. There is no automatic retry or first-use download.

Stabilization requires real packaged Windows model/tokenizer proof; owner/generation and request-binding isolation; queued, active and initialization cancellation with actual child exit; crash/reload/disable/daemon-shutdown cleanup; background retention; malformed/oversized/schema and stale-manifest failures; and the absence of a generic loader or provider fallback. Protocol 189 requires exact helper-generation admission; its plugin-worker bridge is private protocol 4 and the fixed inference child uses private IPC version 3. Existing top-level plugin-host call/cancel/dispose payload fields are unchanged. Retrieval quality, corpus freshness and document authority are ARC responsibilities and separate acceptance gates.
...
## `app.slots.experimental_threadView` and `experimental_threadListAnnotations`

**What they do.** The thread-view registration adds a label and component beside
the core thread chat. Core owns selection, the `threadView` query parameter,
back/forward navigation and fallback after plugin startup/removal. The main
chat and composer stay mounted; the companion mounts only when selected.
The required `mainElementId` is a stable unique DOM anchor for measuring that
preserved chat, not a persisted identity or permission to mutate core UI.
`viewState` and `onViewStateChange` expose one opaque, 512-character-bounded URL
value per plugin/view under `threadViewState.<pluginId>/<id>`. Host navigation
preserves other query/hash values and Chat switches retain the view state.
The public types are `ExperimentalThreadViewProps`,
`ExperimentalThreadViewRegistration`, `ExperimentalThreadListAnnotation`,
`ExperimentalThreadListAnnotationsProps` and
`ExperimentalThreadListAnnotationsRegistration`.

The separate annotation publisher receives a project and up to 500 visible or
expanded thread ids. Core mounts one publisher per registration/project,
validates the full replacement metadata list and excludes foreign thread ids.
Identity labels distinguish agents from groups; counters are nonnegative
integers. A local `viewId` resolves only within the publishing plugin. Core
clears prior publications after project/row-set/generation changes, removal or
crash and ignores callbacks from retired publishers.

**Audit before stabilizing.** Confirm multi-plugin header overflow and compact
companion sizing, split-pane URL selection behavior, accessibility and focus,
and whether the 500-row/eight-identity/eight-counter bounds meet real workloads.
Verify main composer/scroll/stream preservation and delayed publication cleanup
through hot reload and error boundaries. Metadata must remain display-only;
plugins retain responsibility for exact record association and freshness.
...
## Immutable plugin execution context

`experimental_executionContextId` on SDK thread creation persists a trimmed,
nonempty identifier of at most 256 characters on the thread before its first
dispatch. It requires plugin origin and an `originPluginId`; the plugin-bound
SDK rejects another origin or owner. The matching CLI flags are paired
`bb thread spawn --execution-context <id> --origin-plugin <id>`.

Only the owning plugin's `agents.configure` receives the identifier on
`context.thread.experimental_executionContextId`; other configure callbacks
receive null. The owner must be loaded, enabled, and supply a successful
configuration callback. Missing callbacks, thrown validation errors, and
malformed results abort dispatch. Bound owner instructions may contain up to
65,536 characters; oversized instructions reject instead of truncating. Other
plugin instructions retain the existing 4,096-character truncation behavior.

The plugin must persist and resolve its immutable record before spawning,
validate the actual project and thread identity, and return the definition in
the configure result. Core cannot inspect the plugin's private record store.
The identifier is opaque metadata, not a credential: thread reads expose it
alongside the owner, so native tools derive ownership from their real thread
context instead of trusting a model-supplied identifier. Threads cannot change
the binding through updates; forks and children do not inherit it. Existing
provider session instruction lifetime applies, and no daemon wire fields change.

Before stabilization, audit first-dispatch and resumed-session delivery across
real providers, plugin reload/disable races, context retention/deletion, cloned
session history, ownership checks in each consumer, CLI/SDK parity, and migration
of existing databases. Coverage must distinguish missing owners, invalid records,
oversized instructions, unchanged legacy limits, and optional unbound creation.

## Native provider process supervision

`experimental_spawnPortableProcess`, `experimental_killProcessGroup`, `experimental_supportsProcessGroups`, and `experimental_resolveWindowsPowerShell` from the provider-bridge entrypoint let provider plugins launch Windows command shims without a visible console, select PowerShell, and clean up an owned process tree. Unix process groups retain their existing behavior. Windows cleanup uses taskkill /T before the leader exits; it is normal shutdown supervision and does not provide crash containment or model sandbox isolation. Before stabilization, audit PID lifetime races, shell argument quoting, cancellation under heavy load, and packaged provider workflows on each supported platform.

`AcpLaunchSpec.experimental_loginCommand` carries a provider-declared, user-initiated authentication command to its health result. It is optional, preserved by launch normalization, and does not change the host wire schema. Existing launch specs remain valid. Audit provider ownership, escaping on each host shell, authentication cancellation, and settings UI behavior before stabilization.

## Project previews and native browser feedback

`sdk.experimental_previews.detach` requires the exact reviewed terminal ID, current configuration revision and `acknowledgeUnconfirmedProcess:true`. It is available only for unconfirmed cleanup, releases tracking without process operations, returns status `detached`, and preserves up to 20 bounded evidence records with explicitly unconfirmed exits. Audit criteria include stale-session rejection, acknowledgment enforcement, retained history after restart, no automatic launch and fresh process ownership checks after detach.

`sdk.experimental_previews` configures and starts/stops/restarts one owned persistent terminal per project. Configuration is versioned independently from the workspace and is saved without executing it. Preview operations preserve ownership through an exact recorded terminal title and ID; disconnected cleanup blocks replacement. `sdk.files.experimental_refreshPreview` renews a root-scoped file preview lease and reports metadata changes in rotating batches of 32 among up to 2,048 requested resources, with an explicit limit flag. No filesystem authority is added.

File-preview lease URLs and response shapes remain unchanged. Only validated live asset GET/HEAD responses allow credentialless cross-origin reads; mutation, renewal and unrelated API origin rules remain enforced. The configured root never expands. Hidden paths, private-key extensions and escaping/resolved private aliases are denied. Scripts can read permitted root contents. HTML, SVG, XHTML and XML remain sandboxed without same-origin privileges across preview, raw-file and uploaded-attachment responses, including conditional responses. Media/PDF behavior is retained; unfamiliar documents download. Stabilization requires native modules/dynamic imports, JSON, CSS, images/fonts and reload/renewal alongside navigation, credential, encoded-path and junction denial tests.

The internal `host.read_file.pathPolicy` capability requires a root and excludes Git-ref reads. Protocol 191 prevents older daemons from silently discarding these host-side constraints. It adds no public SDK/CLI configuration. The server selects Preview restrictions; the host validates requested and canonical root-relative paths.

ARC composition authorization is optional versioned server-owned snapshot metadata, not a public request member. Restricted admission checks every resolved constituent against the unchanged exact team-revision policy. Project, origin content hashes and complete team/member snapshots are bound together and validated across compilation, admission, continuation and reconciliation. Historical snapshots without this field keep their serialization and gain no composite authority. Audit criteria include forged requests, altered bindings, standalone/disallowed constituents, queued restart and changed-recipient verification retention.

The Node SDK exports `experimental_listDesktopBrowserTargets` and `experimental_evaluateDesktopBrowser` for existing private native-browser CDP leases, including Windows. Evaluation is bounded, uses an explicit target when multiple pages exist, and never discovers arbitrary debugging endpoints. Stabilization requires packaged Windows/local and enrolled-host lifecycle coverage, long-lived lease/file refresh tests, cancellation/takeover, output bounds, and SDK/CLI/UI parity.
