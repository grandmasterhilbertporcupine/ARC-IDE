...
## WNDR native Windows fork

WNDR adds native Windows 11 x64 paths, PowerShell terminals through ConPTY,
Windows provider command shims, native Parcel file watching, and Windows
process identity checks. The upstream macOS, Linux, and WSL paths remain.
Native Windows release certification requires the packaged installer and each
provider's live authentication and execution checks; source tests alone do not
certify a release.

Drive-letter project directories and UNC project directories are accepted.
Filesystem roots, share roots, drive-relative paths, device namespaces, and
control characters are rejected. UNC availability remains subject to the user's
Windows access rights and the selected provider's support.

PowerShell 7 is preferred when installed; Windows PowerShell is the fallback.
Windows managed-worktree hooks use `.bb-env-setup.ps1` and
`.bb-env-teardown.ps1`. A repository with only a POSIX hook fails setup with an
actionable request for its Windows counterpart instead of silently skipping it.

Owned live process trees are stopped using `taskkill.exe /PID <owned PID> /T /F`
before their leaders exit. Windows does not receive POSIX process-group signals.
This handles normal cancellation and shutdown, but does not guarantee cleanup
after an unexpected host crash or a leader that has already detached and exited.
Untrusted model execution requires the separate AppContainer and Job Object
boundary; ordinary IDE terminal process management is not a security sandbox.

## Supported host environments

- macOS persistent host
- Linux persistent host
- Native Windows 11 x64 in WNDR; upstream Windows via Ubuntu on WSL2
...
The preserved upstream WSL mode runs the Linux stack entirely inside WSL2:
...
- native Windows WNDR sessions instead use the native paths described above
...
- `/mnt/c/...` mounted paths are deliberately supported so WSL2 users can keep
  ...
  filesystem.
- WSL sessions use Linux paths; native Windows sessions use drive-letter or UNC
  paths belonging to that Windows host.
...
- upstream-only native Windows features outside WNDR's verified release gates
...
- macOS, Linux, and WSL2 use `.bb-env-setup.sh` and `.bb-env-teardown.sh`.
- Native Windows uses `.bb-env-setup.ps1` and `.bb-env-teardown.ps1` through
  PowerShell `-NoProfile -NonInteractive -File`, subject to Windows execution policy.
...
- Native Windows text files retain LF checkout; the Windows CLI shim is a `.cmd`
  file and PowerShell scripts use `.ps1`.
...
- Branch protection should require `Checks (ubuntu-latest, Node 22.x)`,
  ...
  required PR checks.
- WNDR adds native Windows CI and installer verification alongside these upstream
  gates. A successful Linux gate alone cannot certify native Windows behavior.
