...
## Approved MVP hardening milestone — September 16, 2026

Preserve the original checkout and build a healthy clone of ARC-IDE on `codex/mvp-hardening`, importing verified current source and intended deletions without build residue, credentials or profiles. Deliver an unsigned local `ARC-0.42.10-x64.exe` with desktop and bundled-app versions in agreement. Source pushes, tags and releases remain paused.

Close the shared untrusted-document boundary across leased previews, project/thread raw files and uploaded attachments. Retain opaque-origin HTML/SVG/XHTML/XML sandboxing, security headers on conditional responses, media/PDF behavior and downloads for unfamiliar documents. Permit credentialless assets only after lease and fixed-root validation. Deny hidden/private paths and aliases; preserve the general API origin guard. Keep visible Preview status fresh at five-second intervals even when stopped or disconnected.

Bind server-created composition authorization to validated constituent revisions, project and retained team/member snapshots. Preserve the policy allowlist, reject caller-injected metadata, disallow standalone constituents under restrictions, and validate admission, reconciliation and continuation. Legacy snapshots keep absent-field serialization and acquire no new authority.

Use shared serial Turbo verification in Windows CI and release preparation. Diagnose the bounded-delegation integration timeout with phase timing and cleanup checks; give only that test a 60-second ceiling. Include full ARC/Workflows tests, relevant typechecks and credential-free packaged team/Preview regressions. Freeze clean committed source, package into fresh output and compare the actual installed payload against its source/build manifest before finalizing assets. Reject mismatched installers and receipts even when versions agree.

Evidence must include source/payload manifests, installer SHA-256, signature status, logs, screenshots, guarded installation/reinstall/shortcut/uninstall results and a readiness report. Existing-installation safeguards cannot be bypassed. Clean-machine certification, cross-version upgrades, signing and authenticated efficiency comparisons remain separate. Inspection revealed that enforcing private aliases requires a generic host read-path constraint and protocol 191; this is an internal compatibility change, with no public Preview request or response change.
