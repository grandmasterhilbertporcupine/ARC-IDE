...
Packaged Windows runtime shells receive `BB_CLI` pointing to the bundled
`bb.cmd` and `BB_CLI_RUNTIME` pointing to the current Node-compatible runtime.
ARC supplies its own Electron executable in Node mode, so commands inside ARC
do not require Node on the user's PATH. These values are scoped to child
processes; the launcher does not install aliases or change the user's shell.
...
Settings → Appearance offers Liquid Glass independently of the color palette.
The server stores `appearance.surfaceStyle` as `default` or `liquid-glass`;
existing installations keep `default`. It applies to clients connected to that
server. Light/dark mode remains a separate client preference.

Use `bb theme style set liquid-glass` to enable it and `bb theme style reset`
to restore default surfaces. Both preserve the palette and favicon color.
Palette and favicon commands also preserve the selected surface style.
`bb theme show --json` reads all appearance values.

The SDK reads with `sdk.theme.get()` and writes with
`sdk.theme.set({ themeId, faviconColor, surfaceStyle })`.
`PUT /api/v1/settings/appearance` accepts the same selection; legacy requests
omitting `surfaceStyle` preserve its stored value. Responses always include it.

## Repository worktree hooks

Native Windows uses `.bb-env-setup.ps1` and `.bb-env-teardown.ps1`, executed by
PowerShell with `-NoProfile -NonInteractive -File`. Windows execution policy
applies. Keep both platform variants in cross-platform repositories. A POSIX-only
setup hook fails native provisioning with an actionable error.
...
Server-owned injected skills, including generated plugin command catalogs, are
snapshotted by content hash for the server process lifetime. Source edits do not
change an already registered snapshot. Each tree is limited to 1,000 files and
10 MiB; the registry retains at most 128 MiB of file contents, 2,048 hashes and
32,768 entries. Identical trees share a snapshot. Exhaustion rejects new
snapshots explicitly without evicting existing ones; restart the server to
clear this process-local registry.
...
### ARC provider authentication and API helpers
...
Codex subscription authentication is used for agent sessions. Auxiliary inference and voice require an explicit `OPENAI_API_KEY` in the host environment and use the public OpenAI API. Provider subscriptions do not fund these API helper calls. ARC does not extract tokens from provider credential files for helpers or quota checks.

## ARC agents, teams and runs

Agent Studio stores separate drafts and immutable published definitions. Agent execution settings can select a provider/model, reasoning effort, service tier and permission mode or inherit project defaults. When a project has no saved defaults, a team run can inherit its main conversation's resolved execution settings. The complete resolved settings and published versions are sealed when the run starts; later edits do not change active workers.

Team Builder stores teams in a personal library or project. Each member selects an exact published agent revision in that scope. Named/colorable groups can nest, but membership does not imply delegation or review permission; directed grants and execution connections are separate. The editor retains unfinished saved drafts with diagnostics. Publishing requires a structurally valid DAG and creates an immutable revision. Draft updates, publication, restore, archive and proposal review require the current `expectedDraftVersion`. History remains intact, and copying a published library team atomically copies its exact agent revisions and reference files into project-owned definitions.

The builder can edit the team's `name`, optional six-digit hex `presentation.color`, and each member's optional `modelOverride:{providerId,model,reasoningLevel,serviceTier}`. Team color accents member and workflow cards; group outlines keep their separate group colors. Omitted color follows existing group/theme styling. A model override requires all four non-null fields, with service tier `default` or `fast`, and applies only to that team's member. Omit the override to inherit the pinned agent configuration. Reusable agent defaults and permission mode are unchanged. Model choices affect the operational hash; team name and color only affect the content hash. `saveTeamDraft` through `bb arc teams rpc` or SDK plugin RPC uses the identical definition and draft-version guard. Published revisions and active run snapshots do not follow later edits automatically; active model changes require operational-rule review.

The graph includes agent tasks, parallel paths, all/selected joins, native checks, reviews, typed conditions with true/false connections, bounded repair, user approval, integration, release and bounded delegation. Required completion gates use all/any. `startTeamRun` seals a published project graph and resolved orchestration settings into Workflows V2; existing fixed runs retain V1 behavior. Release still requires the future factory capability. Publishing alone never dispatches work. Graph integration/native acceptance status is recorded in ARC-PROGRESS.

Orchestration settings store project defaults and explicit session overrides with immutable versions. The default is Collaborative, no preferred team, unrestricted otherwise eligible project teams, four active agents, 100 calls, three repair rounds and two active hours. Guided requests approval for assignments; Collaborative approves the initial plan; Autonomous follows sealed bounds. Preferred/restricted teams pin exact published project revisions. Inheritance, explicit no preference and an empty restriction are distinct. Settings do not change active run snapshots. `bb arc policy show|history --project <id> [--thread <id>]` and typed `policy rpc` operations expose the same settings as the UI. See [graph runtime contracts](ARC-GRAPH-RUNTIME.md) for field meanings, limits, decision recovery and remaining main-orchestrator continuation work.

Presentation coordinates, group names/colors and display text are persisted separately from operational hashing. Pinned agent revisions, tasks, dependencies and grants determine operational identity. Authoring limits are 200 nodes, 1,000 edges, 100 members and 1 MiB canonical JSON; a repair stage may declare one through three rounds. These limits do not imply that a published graph can execute.

Start the Team assistant from a saved draft. Its definition is snapshotted before spawn and bound to the actual project/conversation before the first configured turn. The assistant proposes exact-version changes for review and cannot directly apply or publish them. Applying edits the draft; publication remains separate. `bb arc teams list|show|history|rpc` and `sdk.plugins.callRpc({pluginId:"arc",method,input})` expose the same authoring/session operations, including `startTeamAssistant` and `listTeamSessions`. See the [agents skill](../plugins/arc/skills/agents/SKILL.md) for exact inputs and the [team design](ARC-TEAMS.md) for implemented contracts and remaining execution gates. New authoring browser/native acceptance is recorded only when completed in ARC-PROGRESS.

Graph runs select a published team, goal, main conversation and registered source. The retained fixed-run form configures one to four writers, a reviewer, repair agent and required executable/argument list. Its UI uses a ten-minute check timeout; V1 SDK/CLI requests accept 1,000–1,800,000 milliseconds. Graph commands use their published bounds. Pause/resume preserve counters. Pure user/CI waits exclude active time only when no native work remains active; dependency and reconciliation waits remain charged. New ARC workers report completion as passive parent history rather than automatically starting another provider turn. Manual messages in the main conversation remain separate BB activity. Workflows remains the only scheduler.

Use `bb arc runs` or the same ARC plugin RPC operations exposed by the Runs UI. See [ARC's agents skill](../plugins/arc/skills/agents/SKILL.md) for exact inputs, immutable operation IDs, evidence inspection and control semantics.

Run details offers explicit instruction updates for active V2/V3/V4 graphs. Review a newer published revision of the same project team, then pause and apply it. This slice changes agent instruction bodies only; it preserves metadata, references, graph/checks, policy and exact team restrictions. The entire team reruns from the freshly verified original source in a linked continuation with cumulative call, active-time and repair usage. Publishing alone leaves active runs unchanged. SDK/CLI expose preview, apply, poll, cancel and read-only lineage operations; see [instruction update contracts](ARC-INSTRUCTION-UPDATES.md).

The separate operational-rule review accepts saved project/session policy changes at their exact observed versions, plus compatible published changes to checks, gates, collaboration grants, delegation choices, repair ceilings and resolved agent execution. The current team revision supports policy-only changes. No-op and blocked previews cannot be applied. Explicit Apply reruns the team from verified original source with fresh approvals/results and cumulative usage against the reviewed totals; disabled repair stages retain their identities and consumed rounds. Both update types share one control lock and recovery engine. `getRunUpdateState` reads mixed lineage; `getRunReviewAuthority` reads semantic review eligibility without changing file-proof timestamps. Reviewers need directed grants for every other contributor in serial, merged, delegated and repaired candidates. See [operational-rule contracts](ARC-RULE-UPDATES.md) and ARC-PROGRESS for actual verification and remaining gates.

Main-composer orchestration adds `getOrchestratorContext` and `requestTeamRun` through `bb arc orchestrator show|rpc` and the same typed SDK. Context selects the registered source on the main conversation's host and returns exact eligible/preferred team revisions and policy versions. Direct user requests use the existing `startTeamRun` input; native main agents use `arc_orchestration_context` and `arc_team_run_request`, with core-derived invocation identity. ARC definition V3 retains V2 graph execution and adds one response in the actual main environment/model, counted in that run's limits. `reconcileOrchestratedRun({runId})` recovers an uncertain saved request; `discardOrchestratedRunRequest` retires an exact unused user request. Neither changes a retained snapshot. Completion cannot use the native ARC tool to start an independent run with fresh limits. Further automatic rounds require a separate finite Factory grant. Inherited provider shell/network access and direct trusted-user HTTP calls remain BB's trust boundary. V3 native acceptance passed; exact evidence and remaining Phase 4 gates are in ARC-PROGRESS.

Serial non-Git execution adds `getProjectRunSetup`, `getDirectoryRunSetup`, `requestDirectoryTeamRun` and `discardDirectoryRunRequest`. The Runs form explicitly inspects a folder and binds the returned inspection to one immutable V4 request. Repeated inspection polls and admission retries retain their identities. Native agents use `arc_directory_source_inspect` and `arc_directory_team_run_request`. V4 uses separate run-owned copies, exact directory snapshots, serial writing and one counted main response. Unsupported links, graphs and inventory limits produce explicit diagnostics. Long freshness scans remain durable Workflows dependencies; they do not charge an extra agent call or turn a historical scan into current authority. `resolveDirectoryRunControl` returns checking/resolved with the control and accepts the same exact decision fields as `resolveRunControl`; the UI polls its retained decision while checking. See [serial folder contracts](ARC-DIRECTORY-RUNTIME.md) for exact inputs, coverage, pause/recovery, the passed Codex native gate and remaining acceptance.

Open Workspace from a saved run to inspect the main composer and worker transcripts together. Wide layouts show up to four worker chats; focus and conversation selection make every retained worker accessible at smaller sizes. Worker chats show their pinned model and agent revision. The view follows actual native acceptance once and suppresses handoff motion on initial load, reconnection and reduced-motion displays. It does not assign invented team membership to earlier runs. `bb arc workspace show <run-id>` and the `getWorkspace` plugin RPC expose the same recorded data.

Graph runs also offer an optional saved-plan view beside the conversations. Search and inspect stages, view pinned member/revision details, and open an exact worker attempt without editing the published plan. Expand or collapse the graph while keeping the composer draft and visible chats mounted. Laptop Worker overview shows up to four chats with vertical scrolling; the conversation bar selects other workers. The SDK/CLI response includes `run.definition.team`, pinned `run.definition.members` and a nullable `graphNodeId` for each projected worker; V1 has no graph. Missing attempts in the bounded latest-100 window are not an execution outcome. Graph visibility and pane sizing are local view state and do not change run policy.

## Project Preview

The right-side Preview panel and `bb preview configure` save a launch command, explicit host, absolute working directory and optional HTTP(S) URL per project. Configuration lives in ARC's own settings database, separately from workspace files. Saving never executes a command. Start creates one owned persistent terminal; Stop and Restart only act on that recorded terminal. A disconnected host must reconnect before replacement so cleanup is not guessed. Use `bb preview show --project <id> --json` for the current revision, terminal, detected URL and bounded logs. Localhost belongs to the machine running the browser; remote projects require an explicitly reachable URL or existing authenticated port sharing.

Stop requires native exit acknowledgment. Unconfirmed cleanup blocks Start, Restart and configuration. If reconnecting and retrying cannot recover the native session, inspect or stop its processes on the owning host, then explicitly confirm **Detach lost session**, or run `bb preview detach --project <id> --terminal <exact-id> --acknowledge-unconfirmed-process`. Detach releases tracking without stopping processes or claiming cleanup succeeded. ARC preserves the 20 most recent detached sessions, including host/configuration, last URL, bounded logs and unconfirmed process-exit status. A fresh launch still verifies ownership of its listener; a previously running process cannot be adopted as the new preview.
