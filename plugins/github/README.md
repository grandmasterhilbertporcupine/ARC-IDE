...
GitHub issues and pull requests inside ARC, with one-click agent dispatch.

Install it from the Included with ARC catalog:
...
- **Send agent / Review with agent**: spawns an ARC worker thread on the issue
  (or a review thread on the PR) in the repo's ARC project. The issue/PR then
...
- Every ARC project source whose checkout has a GitHub `origin` remote
