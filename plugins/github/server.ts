...
export default async function plugin(bb: BbPluginApi) {
  const settings = bb.settings.define({
    extraRepos: {
      ...
      description:
        'Comma-separated "owner/repo" list to track in addition to repos discovered from ARC projects.',
    ...
    defaultProject: {
      type: "project",
      label: "Default ARC project",
      description:
        "Where agent threads spawn for repos that are not attached to an ARC project.",
    },
  });
  ...
  async function resolveProjectId(repo: string): Promise<string> {
    ...
    throw new Error(
      `No ARC project is attached to ${repo}. Create a project whose checkout has ` +
