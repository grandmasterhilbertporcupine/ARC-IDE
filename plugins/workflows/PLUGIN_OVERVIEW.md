...
- Explicit success/failure outcomes and required-success gates for work that must finish before the next step.

## How it works

A workflow is a JavaScript file. It runs in a sandbox with no file, shell, network, or clock access. Calls to `agent(...)` start normal ARC worker threads with their usual tools and permissions. Scripts can pipeline stages, run parallel barriers, and request a JSON Schema for a worker's output. Successful calls are cached, so a resumed or restarted run replays them and continues live from the first change.

Worker threads stay hidden from the sidebar. Expired runs are archived after the retention period.

Use `requireSuccess(await parallelSettled(...))` or `requireSuccess(await pipelineSettled(...))` for required work. Failure evidence remains attached to each outcome; a failed required stage prevents the next step. Optional work can be handled separately. The original `parallel()` and `pipeline()` helpers keep their legacy failure-to-null behavior.
...
The plugin is off by default. Commands must run inside an ARC project thread.
