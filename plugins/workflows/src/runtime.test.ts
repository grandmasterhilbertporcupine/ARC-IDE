...
describe("workflow QuickJS runtime", () => {
  ...
  it("retains parallel failure evidence separately from valid null and falsy results", async () => {
    const calls: string[] = [];
    const result = await executeWorkflowScript({
      args: null,
      body: `return await parallelSettled([
        () => agent("failure"),
        () => agent("nullable"),
        () => false,
        () => 0,
      ...
      capabilities: {
        async agent(prompt) {
          calls.push(prompt);
          if (prompt === "failure") throw new Error("Provider unavailable");
          return null;
        },
        log: vi.fn(),
        phase: vi.fn(),
    ...
    expect(calls).toEqual(["failure", "nullable"]);
    expect(result).toEqual([
      { status: "failed", error: "Provider unavailable" },
      { status: "succeeded", value: null },
      { status: "succeeded", value: false },
  ...
  it("requires all outcomes before advancing and waits for parallel siblings to settle", async () => {
    const calls: string[] = [];
    await expect(
      executeWorkflowScript({
        args: null,
        body: `
          requireSuccess(await parallelSettled([
            () => agent("review"),
            () => agent("check"),
          ]));
        ...
        capabilities: {
          async agent(prompt) {
            calls.push(prompt);
            if (prompt === "review") throw new Error("Review failed");
            await Promise.resolve();
            return "checked";
          },
          log: vi.fn(),
      ...
      "Required workflow work did not succeed: item 0: Review failed",
    );
    expect(calls).toEqual(["review", "check"]);
  });

  it("allows explicitly optional failed work while preserving successful null values", async () => {
    const result = await executeWorkflowScript({
      args: null,
      body: `
        const outcomes = await parallelSettled([
          () => { throw new Error("Optional research unavailable"); },
          () => null,
          () => false,
          () => 0,
        ]);
        return {
          omitted: outcomes.filter((outcome) => outcome.status === "failed"),
          values: requireSuccess(outcomes.filter((outcome) => outcome.status === "succeeded")),
        };
      `,
      capabilities: { agent: async () => null, log: vi.fn(), phase: vi.fn() },
    });
    ...
    expect(result).toEqual({
      omitted: [{ status: "failed", error: "Optional research unavailable" }],
      values: [null, false, 0],
    });
  });

  it("streams settled pipelines and skips later stages only for the failed item", async () => {
    const result = await executeWorkflowScript({
      args: null,
      body: `
        const events = [];
        ...
        const outcomes = await pipelineSettled(["slow", "fast", "failure", null],
          async (value, original, index) => {
            events.push("start:" + index);
            if (index === 0) await firstGate;
            if (index === 2) throw new Error("Check failed");
            return value;
          },
    ...
    expect(result).toEqual({
      ...
      outcomes: [
        {
          status: "succeeded",
          value: { value: "slow", original: "slow", index: 0 },
        },
        {
          status: "succeeded",
          value: { value: "fast", original: "fast", index: 1 },
        },
        { status: "failed", error: "Check failed", stageIndex: 0 },
        {
          status: "succeeded",
          value: { value: null, original: null, index: 3 },
        },
      ],
    });
  });

  it("records the failing stage and rejects missing callback results before downstream work", async () => {
    const calls: string[] = [];
    const result = await executeWorkflowScript({
      args: null,
      body: `
        const outcomes = await pipelineSettled(["failure", "missing", "ok"],
          (value) => value,
          (value) => {
            if (value === "failure") throw new Error("Second stage failed");
            if (value === "missing") return;
            return null;
          },
          (value, original) => agent(original),
        );
        return outcomes;
      `,
      capabilities: {
    ...
    expect(result).toEqual([
      { status: "failed", error: "Second stage failed", stageIndex: 1 },
      {
        status: "failed",
        error:
          "Workflow work returned undefined; return null for an empty result",
        stageIndex: 1,
      },
      { status: "succeeded", value: null },
    ]);
  ...
  it("rejects skipped and malformed required outcomes instead of dropping them", async () => {
    const result = await executeWorkflowScript({
      args: null,
      body: `
        const failures = [];
        for (const outcomes of [
          [{ status: "skipped", reason: "Tests not configured" }],
          [null],
          [{ status: "succeeded" }],
          [{ status: "succeeded", value: undefined }],
          [{ status: "running", value: "not complete" }],
        ]) {
          try { requireSuccess(outcomes); }
    ...
    expect(result).toEqual([
      "Required workflow work did not succeed: item 0 skipped: Tests not configured",
      ...Array(4).fill("requireSuccess outcome 0 must be a settled result"),
    ]);
  });

  it("validates settled helper collections before starting any callback", async () => {
    const result = await executeWorkflowScript({
      args: null,
      body: `
        let sideEffects = 0;
        const failures = [];
        const invalidCalls = [
          () => parallelSettled([() => sideEffects++, null]),
          () => parallelSettled(new Array(2)),
          () => parallelSettled(new Array(4097).fill(() => sideEffects++)),
          () => pipelineSettled([1], () => sideEffects++, null),
          () => pipelineSettled(new Array(2), () => sideEffects++),
          () => pipelineSettled(new Array(4097).fill(1), () => sideEffects++),
          () => requireSuccess(new Array(2)),
        ];
        for (const call of invalidCalls) {
          try { await call(); }
          catch (error) { failures.push(error.message); }
        }
        return { sideEffects, failures };
      `,
      capabilities: { agent: async () => null, log: vi.fn(), phase: vi.fn() },
    });
    ...
    expect(result).toEqual({
      sideEffects: 0,
      failures: [
        "parallelSettled entries must be functions",
        "parallelSettled thunks cannot be sparse",
        "parallelSettled thunks cannot exceed 4096 entries",
        "pipelineSettled stages must be functions",
        "pipelineSettled items cannot be sparse",
        "pipelineSettled items cannot exceed 4096 entries",
        "requireSuccess outcomes cannot be sparse",
      ],
    });
  });
