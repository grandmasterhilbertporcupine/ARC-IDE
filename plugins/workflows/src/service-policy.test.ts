...
describe("workflow service policy integration", () => {
  ...
  it("fails a required parallel gate without dispatching its release worker", async () => {
    const test = setup();
    ...
    try {
      ...
      test.service.onThreadFailed("child-1", "Review rejected the changes");
      test.service.onThreadIdle("child-2", "Checks passed");
      await eventually(() => {
        const finished = getRunRequired(test.db, run.id);
        expect(finished.status).toBe("failed");
        expect(finished.error).toContain(
          "Required workflow work did not succeed",
        );
        expect(finished.error).toContain("Review rejected the changes");
      });
      expect(test.childCount()).toBe(2);
      expect(getCall(test.db, run.id, 0)?.status).toBe("failed");
      expect(getCall(test.db, run.id, 1)?.status).toBe("succeeded");
    } finally {
      controller.abort();
      await worker;
    }
  ...
  it("retains optional worker failures while accepting schema-valid null results", async () => {
    const test = setup();
    harnesses.push(test.harness);
    const run = await test.start(
      source(`
      const outcomes = await parallelSettled([
        () => agent("optional research"),
        () => agent("nullable result", { schema: { type: "null" } }),
      ]);
      return {
        optional: outcomes[0],
        required: requireSuccess([outcomes[1]]),
      };
    `),
    );
    const controller = new AbortController();
    ...
    try {
      await eventually(() => expect(test.childCount()).toBe(2));
      test.service.onThreadFailed("child-1", "Research declined");
      await expect(
        test.service.submitStructuredResult("child-2", null),
      ).resolves.toEqual({ ok: true });
      await eventually(() => {
        const finished = getRunRequired(test.db, run.id);
        expect(finished.status).toBe("succeeded");
        expect(JSON.parse(finished.resultJson ?? "null")).toEqual({
          optional: { status: "failed", error: "Research declined" },
          required: [null],
        });
      });
      expect(getCall(test.db, run.id, 0)?.status).toBe("failed");
  ...
  it("fails a required pipeline gate and never runs the failed item's later stage", async () => {
    const test = setup();
    harnesses.push(test.harness);
    const run = await test.start(
      source(`
      const outcomes = await pipelineSettled(["work"],
        (item) => agent("build " + item),
        () => agent("test"),
        () => agent("release"),
      );
      return requireSuccess(outcomes);
    `),
    );
    const controller = new AbortController();
    const worker = test.service.runWorker(controller.signal);
    try {
      ...
      test.service.onThreadIdle("child-1", "Built");
      await eventually(() => expect(test.childCount()).toBe(2));
      test.service.onThreadFailed("child-2", "Tests failed");
      await eventually(() =>
        expect(getRunRequired(test.db, run.id).status).toBe("failed"),
      );
      expect(test.childCount()).toBe(2);
      expect(getRunRequired(test.db, run.id).error).toContain("Tests failed");
    } finally {
      controller.abort();
      await worker;
    }
