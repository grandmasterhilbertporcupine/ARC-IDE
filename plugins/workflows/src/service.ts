...
import { claimOwnedRun } from "./owned-data.js";
import { createOwnedRunOperations } from "./owned-execution.js";
import type {
  OwnedRunStartInput,
  OwnedContinuationKey,
  OwnedContinuationReserve,
  OwnedContinuationStart,
  OwnedRuleContinuationStart,
  OwnedAddressedContinuationStart,
  OwnedRuleContext,
  OwnedContinuationView,
  OwnedRunControl,
  OwnedRunView,
...
export function formatWorkflowNotification(
  ...
  const prefix = `[ARC workflow finished · ${run.id}]\n\nRun ${run.id} (${run.name}) ${run.status}.\n`;
  ...
  return utf8Prefix(
    `[ARC workflow ${run.id}] ${run.status} — run bb workflows status ${run.id}`,
    maximumBytes,
  );
}
...
export interface WorkflowService {
  startOwned(owner: string, input: OwnedRunStartInput): OwnedRunView;
  inspectOwned(owner: string, runId: string): OwnedRunView;
  controlOwned(owner: string, input: OwnedRunControl): Promise<OwnedRunView>;
...
export function createWorkflowService(
  ...
  const owned = createOwnedRunOperations({
    bb,
    db,
    controllers,
    publish: publishRunsChanged,
  });
  ...
  function childPrompt(
    ...
    const header = `[ARC workflow ${run.name} · run ${run.id}]`;
      ...
      instructions:
        call.status !== "running"
          ...
          : options.outputSchema === null
            ? null
            : `You are an ARC workflow worker. Submit your final value with bb_workflow_result. Required schema: ${JSON.stringify(options.outputSchema)}`,
    };
  }
  ...
  async function maintenanceTick(signal: AbortSignal): Promise<void> {
    const now = Date.now();
    ...
    await isolated("reconcile-owned", () => owned.maintain(signal));
  ...
  async function runWorker(signal: AbortSignal): Promise<void> {
    ...
    let preferOwned = true;
    while (!signal.aborted) {
      while (active.size < currentSettings.maxActiveRuns) {
        const claimOwned = () => {
          const run = claimOwnedRun(db, currentSettings.maxActiveRuns);
          return run === null
            ? null
            : {
                id: run.row.id,
                originThreadId: run.input.originThreadId,
                execute: (runSignal: AbortSignal) =>
                  owned.execute(run, runSignal),
              };
        };
        const claimLegacy = () => {
          ...
          return run === null
            ...
            : {
                ...
                execute: (runSignal: AbortSignal) => executeRun(run, runSignal),
              };
        };
        const claimed = preferOwned
          ? (claimOwned() ?? claimLegacy())
          : (claimLegacy() ?? claimOwned());
        if (claimed === null) break;
        preferOwned = !preferOwned;
        publishRunsChanged(claimed.originThreadId);
        const controller = new AbortController();
        controllers.set(claimed.id, controller);
        const abortRun = () => controller.abort();
        signal.addEventListener("abort", abortRun, { once: true });
        const execution = claimed.execute(controller.signal).finally(() => {
  ...
  return {
    startOwned: owned.start,
    inspectOwned: owned.inspect,
    controlOwned: owned.control,
    reserveOwnedContinuation: owned.reserveContinuation,
    inspectOwnedContinuation: owned.inspectContinuation,
    startOwnedContinuation: owned.startContinuation,
    startOwnedRuleContinuation: owned.startRuleContinuation,
    startOwnedAddressedContinuation: owned.startAddressedContinuation,
    inspectOwnedRuleContext: owned.inspectRuleContext,
    cancelOwnedContinuation: owned.cancelContinuation,
    start,
