...
function installAgentFunction(
  ...
  const finish = (
    ...
    if ("value" in settlement) {
      ...
      if (
        ...
        "stepFailure" in settlement.error
      ) {
        const failure = ownedStepFailureSchema.safeParse(
          settlement.error.stepFailure,
        );
        if (failure.success) {
          const value = jsonToHandle(vm, failure.data);
          vm.setProp(handle, "stepFailure", value);
          value.dispose();
        }
      }
  ...
  const fn = vm.newFunction("agent", (...handles) => {
    ...
    if (capabilities.step !== undefined)
      return rejectImmediately(
        "Owned workflows must use admitted step() calls",
      );
  ...
  if (capabilities.step !== undefined) {
    ...
    const stepFunction = vm.newFunction("step", (...handles) => {
      if (closed) return rejectImmediately("Workflow is no longer running");
      let ref: { nodeId: string; iteration: number };
      let input: JsonValue;
      try {
        ref = ownedStepRefSchema.parse({
          nodeId: handles[0] ? vm.dump(handles[0]) : undefined,
          iteration: handles[1] ? vm.dump(handles[1]) : undefined,
        });
        input = ownedJsonValueSchema.parse(
          handles[2] ? dumpJson(vm, handles[2], "step input") : undefined,
        );
      } catch (error) {
        return rejectImmediately(errorMessage(error));
      }
      const call: PendingAgentCall = {
        controller: new AbortController(),
        deferred: vm.newPromise(),
      ...
      void Promise.resolve()
        .then(() =>
          step(ref.nodeId, ref.iteration, input, call.controller.signal),
        )
        .then(
          (value) => {
            assertJsonValue(value, "step result");
            finish(call, { value });
          },
          (error) => finish(call, { error }),
        )
    ...
    vm.setProp(vm.global, "step", stepFunction);
    stepFunction.dispose();
  }
...
(() => {
  ...
  const validateSettledCollection = (values, name) => {
    if (!Array.isArray(values)) throw new TypeError(name + " must be an array");
    if (values.length > maximumCollectionSize) throw new RangeError(name + " cannot exceed 4096 entries");
    for (let index = 0; index < values.length; index += 1) {
      if (!(index in values)) throw new TypeError(name + " cannot be sparse");
    }
  };
  const failureMessage = (error) => error instanceof Error ? error.message : String(error);
  const settledValue = (value) => {
    if (value === undefined) throw new TypeError("Workflow work returned undefined; return null for an empty result");
    return value;
  };
  const parallelSettled = async (tasks) => {
    validateSettledCollection(tasks, "parallelSettled thunks");
    for (const task of tasks) {
      if (typeof task !== "function") throw new TypeError("parallelSettled entries must be functions");
    }
    return Promise.all(tasks.map(async (task) => {
      try { return { status: "succeeded", value: settledValue(await task()) }; }
      catch (error) { return { status: "failed", error: failureMessage(error) }; }
    }));
  };
  const pipelineSettled = async (items, ...stages) => {
    validateSettledCollection(items, "pipelineSettled items");
    validateSettledCollection(stages, "pipelineSettled stages");
    for (const stage of stages) {
      if (typeof stage !== "function") throw new TypeError("pipelineSettled stages must be functions");
    }
    return Promise.all(items.map(async (original, index) => {
      let current = original;
      for (let stageIndex = 0; stageIndex < stages.length; stageIndex += 1) {
        try { current = settledValue(await stages[stageIndex](current, original, index)); }
        catch (error) { return { status: "failed", error: failureMessage(error), stageIndex }; }
      }
      return { status: "succeeded", value: current };
    }));
  };
  const requireSuccess = (outcomes) => {
    validateSettledCollection(outcomes, "requireSuccess outcomes");
    const values = [];
    const failures = [];
    for (let index = 0; index < outcomes.length; index += 1) {
      const outcome = outcomes[index];
      if (outcome === null || typeof outcome !== "object" || Array.isArray(outcome)) {
        throw new TypeError("requireSuccess outcome " + index + " must be a settled result");
      }
      if (outcome.status === "succeeded" && Object.hasOwn(outcome, "value") && outcome.value !== undefined) {
        values.push(outcome.value);
      } else if (outcome.status === "failed" && typeof outcome.error === "string") {
        failures.push("item " + index + ": " + outcome.error);
      } else if (outcome.status === "skipped" && typeof outcome.reason === "string") {
        failures.push("item " + index + " skipped: " + outcome.reason);
      } else {
        throw new TypeError("requireSuccess outcome " + index + " must be a settled result");
      }
    }
    if (failures.length > 0) throw new Error("Required workflow work did not succeed: " + failures.join("; "));
    return values;
  };
  ...
  Object.defineProperties(globalThis, {
    ...
    parallelSettled: { value: Object.freeze(parallelSettled), configurable: false, writable: false },
    pipelineSettled: { value: Object.freeze(pipelineSettled), configurable: false, writable: false },
    requireSuccess: { value: Object.freeze(requireSuccess), configurable: false, writable: false },
...
export async function executeWorkflowScript({
  ...
  try {
    ...
    const budgetFunction = vm.newFunction("__bbWorkflowBudget", () =>
      jsonToHandle(vm, capabilities.budget?.() ?? scheduler.budget()),
    );
