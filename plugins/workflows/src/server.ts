...
import { findActiveOwnedThreadRuns } from "./owned-data.js";
...
const sourceInputFields = {
  script: z
    ...
    .min(1)
    .describe(
      "Self-contained workflow script. Must begin with `export const meta = { name, description, phases }` (pure literal, no computed values). Use agent()/parallelSettled()/pipelineSettled()/requireSuccess()/phase() for required work; legacy parallel()/pipeline() convert failures to null.",
    )
    .optional(),
  source: z
    .string()
...
const runInputSchema = z
  .object({
    ...
    resumeRunId: z
      ...
      .nullable()
      .describe(
        "Run ID of a prior ARC workflow to resume from. Calls in the causally safe, longest unchanged prefix return cached results; the first edited, new, or concurrent call and everything after it run live. The prior run must be terminal and from the same project and environment.",
      )
      .default(null),
  })
  .strict();
...
export default async function plugin(bb: BbPluginApi) {
  ...
  bb.rpc.experimental_registerInternal(ownedWorkflowRpcContract, {
    findActiveOwnedThreadRuns(input, { callerPluginId }) {
      return { runs: findActiveOwnedThreadRuns(db, callerPluginId, input) };
    },
    startOwnedRun(input, { callerPluginId }) {
      return { run: service.startOwned(callerPluginId, input) };
    },
    inspectOwnedRun({ workflowRunId }, { callerPluginId }) {
      return { run: service.inspectOwned(callerPluginId, workflowRunId) };
    },
    inspectOwnedRuleContext({ workflowRunId }, { callerPluginId }) {
      return service.inspectOwnedRuleContext(callerPluginId, workflowRunId);
    },
    async controlOwnedRun(input, { callerPluginId }) {
      return { run: await service.controlOwned(callerPluginId, input) };
    },
    reserveOwnedContinuation(input, { callerPluginId }) {
      return {
        continuation: service.reserveOwnedContinuation(callerPluginId, input),
      };
    },
    inspectOwnedContinuation(input, { callerPluginId }) {
      return {
        continuation: service.inspectOwnedContinuation(callerPluginId, input),
      };
    },
    startOwnedContinuation(input, { callerPluginId }) {
      return {
        continuation: service.startOwnedContinuation(callerPluginId, input),
      };
    },
    startOwnedRuleContinuation(input, { callerPluginId }) {
      return {
        continuation: service.startOwnedRuleContinuation(callerPluginId, input),
      };
    },
    startOwnedAddressedContinuation(input, { callerPluginId }) {
      return {
        continuation: service.startOwnedAddressedContinuation(
          callerPluginId,
          input,
        ),
      };
    },
    cancelOwnedContinuation(input, { callerPluginId }) {
      return {
        continuation: service.cancelOwnedContinuation(callerPluginId, input),
      };
    },
  });
  ...
  bb.agents.configure((context) => {
    ...
    if (context.origin.pluginId === bb.pluginId) {
      return {
        ...
        skills: [],
        instructions:
          "You are starting as an ARC workflow worker. Follow the workflow prompt. Your final text IS the return value, not a human-facing message. If the prompt requests structured output, call bb_workflow_result exactly once at the end of your response.",
      };
    }
    return {
