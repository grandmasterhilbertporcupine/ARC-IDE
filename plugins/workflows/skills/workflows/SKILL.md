---
name: workflows
description: "Author or run durable ARC workflows when the user requests workflow execution or multi-agent orchestration."
---
...
A workflow puts deterministic control flow around normal ARC worker threads. The
...
const results = await pipelineSettled(
  args.items,
...
Use pipelineSettled for independent multi-stage items. Use parallelSettled
only when the next stage needs all prior results. Require successful outcomes
before advancing mandatory work with requireSuccess. Handle optional failed
outcomes explicitly and report omitted work. A successful value can be null;
validate domain results such as test verdicts separately. The legacy pipeline
and parallel helpers convert failures into null and are not required gates.
...
- Required steps use settled outcomes and requireSuccess; optional failures
  are explicitly handled, never silently counted as completed checks.
