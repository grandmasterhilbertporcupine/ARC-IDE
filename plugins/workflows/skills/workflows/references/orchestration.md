...
## Required work and optional discovery

Use `pipelineSettled()` for required multi-stage work and `parallelSettled()`
for required barriers. Advance only after `requireSuccess(outcomes)` passes.
Failed outcomes retain their errors; valid null results remain successful
data. Validate domain verdicts separately before a review or test is accepted.

The discovery examples below retain the legacy `parallel()`/`pipeline()`
best-effort behavior, where errors become null. They apply only when individual
attempts are optional. Report failed or omitted coverage; do not copy their
null filtering into required review, test, integration, or release gates.
