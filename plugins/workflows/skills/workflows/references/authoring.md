...
- `parallelSettled(thunks)`: run tasks concurrently and await every outcome in
  input order. Each result is `{ status: "succeeded", value }` or
  `{ status: "failed", error }`. Throws and rejected agent calls retain their
  failure messages; a successful `null`, `false`, `0`, or empty string remains
  successful data. Use this for barriers that must distinguish failures.
- `pipelineSettled(items, stage1, stage2, ...)`: stream independent items through
  stages with `(prevResult, originalItem, index)`. A failed stage produces
  `{ status: "failed", error, stageIndex }`, where the index is zero-based, and
  skips that item's later stages. Other items continue. Successful `null`
  values pass to later stages normally. Callbacks must return a value;
  `undefined` fails the item, while `null` is an explicit empty result.
- `requireSuccess(outcomes)`: synchronously return the values of successful
  outcomes in input order. Throw if any outcome failed or is an explicit
  `{ status: "skipped", reason }`, or if the input is not a valid outcome list.
  Use `requireSuccess(await parallelSettled(...))` or the pipeline equivalent
  before required downstream work. This checks execution outcomes, not the
  meaning of returned data: inspect a test's `passed` field separately.
- `agent(prompt: string, opts?)`: spawn an ARC worker. Without `schema`, returns
...
- `pipeline(items, stage1, stage2, ...)`: run each item through all stages
  ...
  B is still in stage 1. This is the legacy best-effort helper. Wall-clock =
...
- `parallel(thunks: Array<() => Promise<any>>)`: run tasks concurrently. This is
  ...
  `.filter(Boolean)` before consuming successful results. This legacy helper
  cannot distinguish a failed call from a successful null result; prefer
  `parallelSettled()` for new workflows and never use it as a required gate.
...
For optional work, inspect the failed outcome and report the omission, then
pass only the required outcomes to `requireSuccess`. Do not filter failed
mandatory checks out of a required gate. An empty outcome list returns an
empty value list; the workflow must still declare and execute every required
check for its task.
...
Workers spawned by `agent()` are normal ARC threads and retain the tools and
workspace access allowed by their ARC permission mode. The QuickJS script itself
...
ARC validates the tuple against the live provider/model catalog immediately
...
Native `label` is an alias for ARC's existing `title`, and native `schema` is an
alias for ARC's existing `outputSchema`. Either spelling remains supported.
...
the structured output. ARC validates the value with Ajv. The initial invalid
