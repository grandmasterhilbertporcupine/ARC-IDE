...
The hub runs inside ARC and serves an Anthropic Messages endpoint and an OpenAI Responses endpoint. With routing on, ARC hands the Claude Code or Codex process a base URL that points at the hub and a token scoped to that machine, and the provider reports **Proxied** in its health row. An account is skipped for a request when it is at or above the switch threshold or in error. The threshold defaults to 98 percent of a window. Account secrets stay in the ARC data directory on the server machine, and the hub refreshes them in the background.
