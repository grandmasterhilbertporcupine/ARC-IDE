...
browser may be on a different machine from the ARC server. Newly added or
...
`CODEX_POOL_AUTH_TOKEN`; ARC applies them as in-memory app-server config.
...
`~/.codex/auth.json` on the ARC server host. OAuth quota refreshes on add or
