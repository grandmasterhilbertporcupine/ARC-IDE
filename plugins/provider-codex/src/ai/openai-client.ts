...
function readOpenAiApiKey(): OpenAiApiKey {
  const apiKey = process.env.OPENAI_API_KEY?.trim();
  if (!apiKey) {
    throw new AiServiceFailure(
      "auth_required",
      "openai_api_key_required",
      "OpenAI API helpers require OPENAI_API_KEY. Use your Codex subscription through agent sessions.",
    );
  }
  return { type: "apiKey", apiKey };
}

function createOpenAiHeaders(auth: OpenAiApiKey): Headers {
  const headers = new Headers();
  headers.set("Authorization", `Bearer ${auth.apiKey}`);
  headers.set("User-Agent", "arc-api-helpers");
  return headers;
}

function createOpenAiResponsesHeaders(auth: OpenAiApiKey): Headers {
...
export async function transcribeCodexVoice(
  ...
  const deadline = createCodexRequestDeadline(command.timeoutMs);
  const auth = readOpenAiApiKey();
  const response = await fetchOpenAiTranscription({ auth, command, deadline });
