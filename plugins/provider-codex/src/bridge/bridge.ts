...
const MISSING_CODEX_CLI_GUIDANCE =
  "ARC could not find the Codex CLI on this machine. Install Codex (https://developers.openai.com/codex/cli) or put `codex` on PATH, then retry.";
...
interface CodexBridgeSession {
  ...
  responseSettledCodexTurnIds: Set<string>;
...
function handleChildNotification(
  ...
  if (method === "turn/started" || method === "turn/completed") {
    const parsed = codexTurnResponseSchema.safeParse(params);
    if (
      parsed.success &&
      parsed.data.turn !== undefined &&
      session.responseSettledCodexTurnIds.has(parsed.data.turn.id)
    ) {
      return;
    }
  }
...
async function constructThreadSession(
  ...
  const session: CodexBridgeSession = {
    ...
    responseSettledCodexTurnIds: new Set(),
...
const codexTurnResponseSchema = z
  .object({
    turn: z
      .object({
        id: z.string().min(1),
...
function settleNativeTurnResponse(args: {
  ...
  if (
    ...
    currentSession(session.bbThreadId, session.serial) !== session ||
    session.responseSettledCodexTurnIds.has(turn.id)
  ) {
    return;
  }
  if (!session.openCodexTurnIds.has(turn.id)) {
    if (!prepared?.claim()) {
      return;
    }
  ...
  session.responseSettledCodexTurnIds.add(turn.id);
}
  ...
  try {
    if (isStandaloneBuiltinCompactCommand(input)) {
      ...
      const result = await connection.request({
        ...
        resultSchema: codexTurnResponseSchema,
        timeoutMs: CHILD_REQUEST_TIMEOUT_MS,
      });
      settleNativeTurnResponse({
        clientRequestId: params.clientRequestId,
        prepared,
        session,
        result,
...
async function handleRequest(
  ...
  switch (request.method) {
    ...
    case "provider/health":
      sendResult(
        request.id,
        await getCodexProviderHealth(withMaintenanceChild),
      );
      break;
    case "provider/usage":
      sendResult(request.id, await getCodexProviderUsage(withMaintenanceChild));
      break;
