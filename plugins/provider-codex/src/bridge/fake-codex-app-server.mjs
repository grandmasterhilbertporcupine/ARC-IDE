...
const LATE_TURN_START_DELAY_MS = 750;
...
async function handleRequest(message) {
  ...
  switch (method) {
    ...
    case "turn/start": {
      ...
      const responseOnlyStatus = {
        "/response-failed": "failed",
        "/response-interrupted": "interrupted",
        "/response-completed": "completed",
        "/response-completed-late-notifications": "completed",
      }[firstInputText(params.input)];
      if (responseOnlyStatus !== undefined) {
        turnCounter += 1;
        const turnId = `turn-fx-${turnCounter}`;
        const turn = {
          id: turnId,
          status: responseOnlyStatus,
          items:
            responseOnlyStatus === "completed"
              ? [
                  {
                    type: "agentMessage",
                    id: `item-fx-${turnCounter}`,
                    text: "Native terminal response",
                  },
                ]
              : [],
          error:
            responseOnlyStatus === "failed"
        ...
        if (
          firstInputText(params.input) ===
          "/response-completed-late-notifications"
        ) {
          setTimeout(() => {
            notify("turn/started", {
      ...
      if (firstInputText(params.input) === "/empty-acknowledgement") {
        respond(id, {});
        return;
      }
      if (firstInputText(params.input) === "/malformed-turn-response") {
        respond(id, { turn: { id: "turn-invalid", status: "unknown" } });
        return;
      }
