...
it("settles an explicit native completed response without streamed activity", async () => {
  ...
  expect(completed[0]).toHaveProperty("providerCheckpointId", "turn-fx-1");
  expect(
...
it("waits for native activity more than 250 ms after an in-progress acknowledgement", async () => {
  const providerThreadId = await startSession();
  harness.sendRequest(2, "turn/start", {
    threadId: THREAD_ID,
    providerThreadId,
...
it.each(["/empty-acknowledgement", "/compact"])(
  "does not infer completion from silence after %s",
  async (prompt) => {
    const providerThreadId = await startSession();
    harness.sendRequest(2, "turn/start", {
      threadId: THREAD_ID,
    ...
    await new Promise((resolve) => setTimeout(resolve, 500));
    expect(
      threadEvents().filter((event) => event.type.startsWith("turn/")),
    ).toEqual([]);
  },
  30_000,
);
...
it.each(["completed", "failed", "interrupted"] as const)(
  "preserves the native %s response and its evidence",
  async (status) => {
    const providerThreadId = await startSession();
    harness.sendRequest(2, "turn/start", {
      threadId: THREAD_ID,
      providerThreadId,
      input: [{ type: "text", text: `/response-${status}`, mentions: [] }],
      clientRequestId: "creq_a2b3c4d5e6",
      options: { ...sessionOptions },
    });
    const response = await harness.waitForResponse(2);
    ...
    const completed = events.filter((event) => event.type === "turn/completed");
    expect(completed).toHaveLength(1);
    expect(completed[0]).toMatchObject({ status });
    if (status === "failed") {
      expect(completed[0]).toMatchObject({
        error: { message: "Native turn failed" },
      });
...
it("does not duplicate lifecycle events after settling a native terminal response", async () => {
  const providerThreadId = await startSession();
  harness.sendRequest(2, "turn/start", {
    threadId: THREAD_ID,
    providerThreadId,
    input: [
      {
        type: "text",
        text: "/response-completed-late-notifications",
        mentions: [],
      },
    ],
    clientRequestId: "creq_a2b3c4d5e6",
...
it("rejects a malformed native turn status rather than completing it", async () => {
  const providerThreadId = await startSession();
  harness.sendRequest(2, "turn/start", {
    threadId: THREAD_ID,
    providerThreadId,
    input: [{ type: "text", text: "/malformed-turn-response", mentions: [] }],
    clientRequestId: "creq_a2b3c4d5e6",
    options: { ...sessionOptions },
  });
  const response = await harness.waitForResponse(2);
