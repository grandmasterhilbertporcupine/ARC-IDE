...
export function createCodexAppServerConnection(
  ...
  const child: ChildProcess = spawnPortableProcess({
  ...
  function writeLine(message: object): void {
    ...
    if (!stdin || stdin.destroyed || !stdin.writable) {
      closeForChildStdin(new Error("stdin is not writable"));
      return;
    }
  ...
  function closeForChildStdin(error: Error): void {
    if (finalized) return;
    stderrChunks.push(`Codex input closed: ${error.message}`);
    finalizeExit({ code: null, signal: null });
    killProcessGroup({ child, signal: "SIGKILL" });
  }

  child.stdin?.on("error", closeForChildStdin);
