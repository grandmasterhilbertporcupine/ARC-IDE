...
describe("codex error and warning translation", () => {
  it.each(["t1", null])(
    "preserves a native warning with thread association %s without completing its active turn",
    (threadId) => {
      const harness = createHarness();
      harness.translate(
        codexEvent("turn/started", {
      ...
      expect(
        harness.translate(codexEvent("warning", { threadId, message })),
      ).toEqual([
        expect.objectContaining({
          type: "provider/warning",
          scope: threadScope(),
          category: "general",
          summary: "Codex warning",
          details: message,
        }),
      ]);
      expect(
  ...
  it.each([
    { threadId: "t1", message: 7 },
    { threadId: 7, message: "Unexpected association" },
    { message: "Missing nullable association" },
  ])("retains malformed native warnings as unhandled events: %j", (params) => {
    const harness = createHarness();
    const event = { jsonrpc: "2.0" as const, method: "warning", params };
    expect(harness.translate(event)).toEqual([
      expect.objectContaining({
        type: "provider/unhandled",
        rawType: "warning",
        rawEvent: event,
