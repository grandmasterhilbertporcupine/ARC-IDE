---
name: acp-provider
description: "Configure or troubleshoot ACP agent discovery, custom models, skills, and compaction in ARC."
---
...
`.agents/skills`. ARC lists these linked skills as read-only under `cursor-project`.

ACP agents may reject unlisted model IDs. OpenCode requires models in its own
configuration; ARC discovers them there. OpenCode agents are session modes, not
models selectable through ARC's model field.
