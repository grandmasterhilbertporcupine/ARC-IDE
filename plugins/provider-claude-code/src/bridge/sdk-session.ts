...
export function spawnClaudeCodeProcess(args: {
  ...
  child.kill = (signal) => {
    ...
    if (typeof signal === "number") return originalKill(signal);
    killProcessGroup({
      child: { pid: child.pid, kill: originalKill },
  ...
  args.spawnOptions.signal?.addEventListener("abort", abort, { once: true });
  child.once("exit", () =>
    args.spawnOptions.signal?.removeEventListener("abort", abort),
  );
  if (args.spawnOptions.signal?.aborted) abort();
