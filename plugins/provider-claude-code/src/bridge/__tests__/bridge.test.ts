...
describe("bridge", () => {
    ...
    const policyCases = [
      {
        ...
        expected: {
          behavior: "deny",
          messageIncludes: "ARC's workspace sandbox allows work inside",
        },
      },
      {
  ...
  it("exposes the host HOME and CLAUDE settings cascade to the Claude SDK on thread/start", async () => {
    ...
    try {
      ...
      expect(queryOptions.env?.CLAUDE_CODE_ENTRYPOINT).toBeUndefined();
      expect(queryOptions.env?.CLAUDE_AGENT_SDK_CLIENT_APP).toBe("arc/0.1.0");
