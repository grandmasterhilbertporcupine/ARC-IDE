...
const READONLY_ASK_REASON =
  "ARC read-only mode requires approval before using tools that can modify state, run commands, access network, or perform non-read actions.";
...
export function buildReadonlyDenialMessage(): string {
  return "ARC read-only mode allows reading and analysis only. Continue with a read-only answer; do not modify files, run mutating shell commands, use network, or use mutating tools.";
}

export function buildWorkspaceWriteDenialMessage(): string {
  return "ARC's workspace sandbox allows work inside the current workspace only. Stay inside the workspace or explain why extra access is needed.";
}
  ...
  for (const searchDir of args.pathEnv.split(delimiter)) {
    ...
    const names =
      process.platform === "win32"
        ? [".exe", ".com", ".cmd", ".bat"].map(
            (extension) => args.executableName + extension,
          )
        : [args.executableName];
    for (const name of names) {
      const candidate = join(searchDir.replace(/^"|"$/gu, ""), name);
