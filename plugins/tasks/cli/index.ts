...
function resolveClientPath(cwd: string, candidate: string): string {
  if (posix.isAbsolute(candidate)) return posix.normalize(candidate);
  if (win32.isAbsolute(candidate)) {
    return posix.isAbsolute(cwd)
      ? win32.normalize(candidate)
      : win32.resolve(cwd, candidate);
  }
  return posix.isAbsolute(cwd)
    ? posix.resolve(cwd, candidate)
    : win32.resolve(cwd, candidate);
}
...
async function readFileOption(
  ...
  const path = resolveClientPath(ctx.cwd ?? process.cwd(), file);
  try {
...
async function defaultProject(
  ...
  if (!ctx.projectId) {
    if (required) {
      throw new CliError(
        "missing --project and no ARC project context is available",
      );
    }
    return undefined;
  }
  ...
  if (matches.length === 0) {
    throw new CliError(
      `no tracker project is linked to ARC project ${ctx.projectId}; pass --project or link one with bb tasks project update`,
    );
  }
  if (matches.length > 1) {
    throw new CliError(
      `multiple tracker projects are linked to ARC project ${ctx.projectId}; pass --project explicitly`,
    );
  }
  return matches[0];
}
...
function projectTable(
  ...
  return table(
    ["PREFIX", "NAME", "FOLDER", "ARC PROJECT", "ID"],
...
async function runProject(
  ...
  if (action === "show") {
    ...
    return detail([
      ...
      ["Color", project.color],
      ["Folder", folder?.name ?? "-"],
      ["ARC project", project.linkedBbProjectId ?? "-"],
...
async function runCreate(
  ...
  const attachPaths = options(args, "attach").map((path) =>
    resolveClientPath(ctx.cwd ?? process.cwd(), path),
  );
...
async function runAttachment(
  ...
  if (action === "add") {
    ...
    const sourcePath = resolveClientPath(
      ctx.cwd ?? process.cwd(),
      sourceOption,
    );
  ...
  if (action === "get") {
    ...
    const outPath = resolveClientPath(ctx.cwd ?? process.cwd(), outOption);
    const clientHostId = await resolveClientHostId(bb, domain, args, ctx);
          ...
          case "seed-demo": {
            ...
            stdout = args.flags.has("json")
              ...
              : detail([
                  ...
                  ["ARC project", result.linkedBbProjectId ?? "-"],
                ]);
            break;
          }
          default:
