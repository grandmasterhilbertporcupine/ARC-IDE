...
describe("bb tasks CLI", () => {
  ...
  it("defaults create and list to the tracker project linked to CLI project context", async () => {
    ...
    expect(missing).toMatchObject({
      ...
      stdout: "",
      stderr:
        "no tracker project is linked to ARC project proj_missing; pass --project or link one with bb tasks project update",
    });
  ...
  it.each([
    {
      ...
      cwd: "/remote",
      inputPrefix: "",
      descriptionPath: "/remote/notes.md",
      screenshotPath: "/remote/shot.png",
      downloadPath: "/remote/fetched/shot.png",
    },
    {
      platform: "Windows",
      cwd: "C:\\remote",
      inputPrefix: "",
      descriptionPath: "C:\\remote\\notes.md",
      screenshotPath: "C:\\remote\\shot.png",
      downloadPath: "C:\\remote\\fetched\\shot.png",
    },
    {
      platform: "Windows root-relative",
      cwd: "D:\\remote",
      inputPrefix: "\\",
      descriptionPath: "D:\\notes.md",
      screenshotPath: "D:\\shot.png",
      downloadPath: "D:\\fetched\\shot.png",
    },
    {
      platform: "UNC",
      cwd: "\\\\fileserver\\share\\remote",
      inputPrefix: "",
      descriptionPath: "\\\\fileserver\\share\\remote\\notes.md",
      screenshotPath: "\\\\fileserver\\share\\remote\\shot.png",
      downloadPath: "\\\\fileserver\\share\\remote\\fetched\\shot.png",
    },
  ])(
    "routes file flags to the invoking thread's machine and honors --machine ($platform paths)",
    async ({
      cwd,
      inputPrefix,
      descriptionPath,
      ...
      const remoteFiles = new Map<string, Buffer>([
        [descriptionPath, Buffer.from("remote description\n", "utf8")],
        [
          screenshotPath,
      ...
      await plugin(bb);
      const threadCtx = { threadId: "thr_remote_worker", cwd };
        ...
        stdout(
          await harness.runCli(
            [
              ...
              "Remote files",
              "--description-file",
              `${inputPrefix}notes.md`,
              "--attach",
              `${inputPrefix}shot.png`,
              "--json",
            ],
            threadCtx,
          ),
      ...
      expect(remoteFiles.get(downloadPath)).toEqual(
        remoteFiles.get(screenshotPath),
      );
      ...
      if (inputPrefix !== "") {
        ...
        expect(harness.sdk.callsTo("files.read").at(-1)![0]).toMatchObject({
          hostId: "machine-remote",
          path: descriptionPath,
        });
      }
  ...
  it("returns a friendly dispatch error when the task project is not linked", async () => {
    ...
    expect(result).toEqual({
      exitCode: 1,
      stdout: "",
      stderr: 'Task project "Unlinked CLI" is not linked to an ARC project',
    });
    ...
    expect(aliased.stderr).toBe(
      'Task project "Unlinked CLI" is not linked to an ARC project',
    );
