...
Tasks is a Linear-style tracker inside ARC for planning work, delegating it to
...
Install Tasks from the official plugins that ARC includes:
...
to create a tracker project. Link it to the ARC project where delegated agents
will run:
...
When the CLI runs inside a linked ARC project, `create` and `list` infer the
...
choose or create presets under **Manage → Presets**, and type `@` in the ARC
...
| `bb tasks attach <key-or-id>`                  | Attach the current ARC thread to a task when it was not delegated from Tasks.                                                              |
| `bb tasks detach <key-or-id>`                  | Detach the current ARC thread (or `--thread <id>`) from a task, for example a dead predecessor after a respawn.                            |
| `bb tasks threads <key>`                       | List the ARC threads attached to a task: live threads first, newest first.                                                                 |
...
Linking a Tasks project to an ARC project enables delegation. Open a task, choose
...
Delegation creates a worker thread in the linked ARC project, attaches that
...
Type `@` in the ARC composer and select **Tasks** to search by task key or title.
...
action contract as context. Tasks linked to the current ARC project rank first.
