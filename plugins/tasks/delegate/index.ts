...
export function buildSeedPrompt(input: SeedPromptInput): string {
  const sections = [
    ...
    markdownSection(
      "Project context",
      `- Name: ${input.project.name}\n- Linked ARC project: ${input.project.linkedBbProjectId ?? "Not linked"}`,
    ),
...
function requireLinkedBbProject(project: Project): string {
  ...
  throw new DelegationError(
    "project_not_linked",
    `Task project "${project.name}" is not linked to an ARC project`,
  );
}
...
async function presetSpawnEnvironment(
  ...
  if (hostId === null) {
    throw new DelegationError(
      ...
      "Could not create a worktree because ARC has no default machine",
    );
  }
  return {
    type: "host",
