Schedule work once or on a repeat, and let ARC run it while you do something else. An automation can start an agent thread with a prompt, or run a stored script on the ARC server.
...
Scripts run on the machine that hosts the ARC server. Agent runs use the providers you already have installed.
