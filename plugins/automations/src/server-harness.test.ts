...
describe("automations server plugin harness", () => {
  ...
  it("resolves the bound ARC server URL lazily for manual script execution", async () => {
    const runScript = vi
      .spyOn(scriptRunner, "executeStoredScript")
      .mockResolvedValue({ exitCode: 0, output: "done", timedOut: false });
    const host = createFakePluginHost({
      pluginId: "automations",
      sdk: {
        projects: {
    ...
    Object.defineProperty(host.bb.server, "loopbackBaseUrl", {
      get() {
        if (!bound) throw new Error("Server is not listening yet");
        return "http://127.0.0.1:38986";
      },
    });
    await plugin(host.bb as unknown as Parameters<typeof plugin>[0]);
    bound = true;
    try {
      const created = await host.harness.runCli([
        "create",
        "--project",
        PROJECT_ID,
        "--name",
      ...
      await host.harness.callRpc("automations_run", {
        projectId: PROJECT_ID,
        automationId: automation.id,
      });
      await vi.waitFor(() =>
        expect(runScript).toHaveBeenCalledWith(
          expect.objectContaining({ serverUrl: "http://127.0.0.1:38986" }),
        ),
      );
    } finally {
  ...
  it("creates a script automation through CLI and lists it through both CLI and RPC", async () => {
    ...
    expect(scriptUpdated.execution).toEqual({
      ...
      storedScriptPath: expect.stringMatching(
        new RegExp(String.raw`[\\/]scripts[\\/]${created.id}[\\/]script\.sh$`),
      ),
      interpreter: "bash",
      timeoutMs: 12_000,
      env: { CHANNEL: "qa" },
