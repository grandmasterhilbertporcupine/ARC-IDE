...
description: "Create or edit ARC color themes and inspect them in the Theme Preview panel."
---

# Authoring an ARC theme

A ARC theme is one CSS file that overrides the app's CSS custom properties. ARC
...
You only need to declare what you change; everything else derives from ARC's
...
How ARC uses them (from ARC's own components, so you can predict the result):
...
(ARC has no primary-filled button; `--primary` is links, focus and accents),
