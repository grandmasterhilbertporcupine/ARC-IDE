Give an agent the API keys it needs without pasting them into chat. The agent asks, you type the values into a masked form, and ARC writes them to a dotenv file on the thread's host.
...
The command must run from an ARC thread with a live host.
