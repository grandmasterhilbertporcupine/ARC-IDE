...
Opens files in ARC using [Monaco](https://microsoft.github.io/monaco-editor/),
the editor from VS Code, instead of ARC's read-only file preview.

It applies everywhere ARC opens a file: links clicked in chat, the secondary
...
Ships with ARC as a builtin; there is nothing to install.
...
config, and text formats. Binaries like `png` and `pdf` are left to ARC's own
preview, which renders them properly.

To change any file type back, use **Settings → File openers**, which offers
Automatic, ARC's built-in preview, or Monaco per extension. Right-clicking a
...
- **Hidden files and `node_modules`** never appear in the tree. ARC's path
...
- **Opening a file from the tree reuses the current tab,** so the tab title
  keeps naming the file it was opened with. A plugin cannot ask ARC to open a
  file or retitle its tab ([#2102](https://github.com/get-bb/bb/issues/2102)).
- **No "open in editor" button** like ARC's preview has; that capability is not
