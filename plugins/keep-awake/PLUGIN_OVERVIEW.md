Stop your Mac from going to idle sleep while ARC runs long agent work. Enable the plugin once, and ARC keeps the selected machines awake for as long as the app is running.
...
- Automatic reconnection. When a host connects again, ARC applies the setting again.

## How it works

The plugin runs on each selected host and holds an idle-sleep assertion while ARC is running. When ARC stops, the assertion stops with it. The display can still turn off. Closing the lid or choosing Sleep still puts the Mac to sleep.
