...
description: "Expose a local HTTP server through Remote access (getbb.app) and give the user its remotely accessible URL."
---

# Share local server links through Remote access
...
<port>`. It prints that host's share URL. Use `--host <name-or-id>` only
   ...
   sharing defaults to the machine running the ARC server.
