...
function helpText(): string {
  return [
    "Remote access via getbb.app — this ARC instance becomes reachable at https://<handle>.getbb.app.",
    ...
    "  bb connect servers             List every server on this account (from getbb.app)",
    "  bb connect machine-code        Mint a one-time code that enrolls the getbb.app mobile app (or another",
    "                                 device) as a connect machine for this ARC instance (needs the",
    '                                 "Mobile app" experiment in Settings → Experiments)',
    "",
    "The server holds the tunnel; it stays up while ARC is running.",
  ].join("\n");
}
...
function notPairedError(): string {
  return "this ARC instance is not connected to getbb.app — run `bb connect` for how to pair";
}
...
function formatMachineCode(payload: MobilePairingPayload): string {
  ...
  return [
    ...
    "",
    "Enter the code in the getbb.app mobile app when it asks to pair through Remote access (or",
...
export function registerConnectCli(args: {
  ...
  bb.cli.register({
    name: "connect",
    summary:
      "Expose this ARC instance at https://<handle>.getbb.app (pair with --code/--server from the dashboard)",
    commands: [
      {
        name: "status",
      ...
      {
        name: "servers",
        summary: "List every server on this account",
        usage: "bb connect servers [--json]",
      },
      {
        name: "machine-code",
        summary:
          'Mint a one-time code that enrolls the getbb.app mobile app as a connect machine (needs the "Mobile app" experiment)',
