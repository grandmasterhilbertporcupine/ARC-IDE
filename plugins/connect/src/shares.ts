...
export class ShareRegistry {
  ...
  async add(port: number, host: ShareHost): Promise<ShareListing> {
    ...
    if (
      ...
      throw new SharePortError(
        `Cannot share port ${validated}: that is the ARC server's own port — the bare handle URL already serves ARC`,
      );
    }
    const credential = this.options.getCredential();
    if (credential === null) {
      throw new SharePortError(
        "this ARC instance is not connected to getbb.app — run `bb connect` for how to pair",
      );
    }
  ...
  private async urlFor(host: ShareHost, port: number): Promise<string> {
    ...
    try {
      ...
      if (code === "connect_host_unenrolled") {
        throw new SharePortError(
          `${prefix}: this host has no remote-access machine credential. Enroll it via Connect in Settings > Machines.`,
        );
      }
