Open ARC from a phone or another computer. After you pair, this ARC instance answers at `https://<handle>.getbb.app` for anyone signed in to your getbb.app account.

## What you get

- Remote access to the full ARC app through a tunnel. ARC makes an outbound connection, so you do not open ports or change your router.
...
- Pairing for the getbb.app mobile app with a QR code or a one-time code.
...
When you view ARC remotely, agents are told to share servers with `bb connect expose <port>`. A localhost link would not open. The `share-server-links` skill explains the flow. Other commands: `bb connect status`, `bb connect unexpose <port>`, `bb connect shares`, `bb connect servers`, and `bb connect machine-code`.
