...
## ARC Scope And Alignment

- Follow [docs/ARC-PLAN.md](docs/ARC-PLAN.md) and record actual evidence in [docs/ARC-PROGRESS.md](docs/ARC-PROGRESS.md).
- Every two implementation work cycles, compare the diff and next step with the approved phase and acceptance gates. Report material drift; never silently omit accepted requirements.
- Keep the source WNDR checkout untouched. Preserve ARC's separate storage, profile and update identity. Historical WNDR verification is not ARC verification.
- Keep BB internal compatibility names; default to the BB dark theme. Extend the existing Workflows scheduler rather than adding another runtime.
